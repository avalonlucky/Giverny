...
## 2026-07-19 · v0.29.0（AI 运行治理与工作区权限底座）

- **AI 运行中心**：设置页新增统一运行视图，集中展示真实模型路线、成功与回退情况、近期请求、后台任务状态和持续学习样本，不再需要在多个页面分别排查 Agent。
- **后台任务统一处置**：月度复盘、趋势分析等后台任务统一汇总；失败或取消的任务可直接重试，进行中的任务可取消，并保留阶段、进度和错误原因。
- **学习效果可量化**：新增建议采用率、修改后采用率、学习场景分布及 AI 工时准确率摘要，开始用真实结果判断 AI 是否逐步贴近用户习惯。
- **多租户权限基础**：D1 新增工作区、成员关系及运行记录的 `workspace_id / principal_id` 上下文，为后续账号隔离和甲方成员权限预留数据边界；当前默认工作区行为保持不变，尚未开放多租户入口。
- **浏览器质量门禁扩展**：桌面与移动端回归新增 AI 运行中心、工作区上下文、后台失败任务与评测流量隔离校验；Agent 隔离评测继续覆盖 79 条读写、审批、CLI、后台分析与安全案例。
- **前端加载拆包**：React、Markdown 和图标依赖改为独立缓存分块，降低主应用包的重复下载与解析成本，并保持 PDF、Office 等重型预览器按需加载。
