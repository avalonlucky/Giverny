...
const taskOperations = readFileSync(join(root, 'src/hooks/useTaskOperations.ts'), 'utf8')
const overlay = readFileSync(join(root, 'src/components/AppOverlayLayer.tsx'), 'utf8')
...
try {
  ...
  assert.match(worker, /isDemoWorkspaceWritablePath\(path, request\.method\)/, '演示账号必须能管理 demo 工作区数据')
  assert.match(worker, /path\.startsWith\('\/api\/settlement-exports'\)[\s\S]+\['POST', 'PATCH', 'DELETE'\]/, '演示账号必须支持生成、锁定、分享管理和删除 demo 回单')
  assert.match(worker, /isDemoWorkspaceAiPath\(path, request\.method\)/, '演示账号必须能使用任务 AI、学习样本和 Agent 运行链路')
  assert.doesNotMatch(worker, /isDemoWorkspaceWritablePath[\s\S]{0,160}\/api\/settings|isDemoWorkspaceWritablePath[\s\S]{0,160}\/api\/tokens/, '演示工作区权限不得扩展到全站设置或访问口令')
  assert.match(app, /const canManageWorkspace = isAdmin \|\| isDemo[\s\S]+canDelete=\{canManageWorkspace\}/, '演示账号应显示任务、进展和附件的完整 demo 数据管理入口')
  assert.match(taskOperations, /const canAccept = isAdmin \|\| role === 'demo'[\s\S]+if \(!canAccept\)/, '演示账号必须能打开并确认验收进展')
  assert.match(overlay, /canAccept=\{isAdmin \|\| demoMode\}/, '任务详情中也必须向演示账号开放验收入口')
  assert.match(worker, /role === 'demo' && isDemoWorkspaceAiPath/, '演示账号必须能使用 Agent 与工作区 AI 运行链路')
