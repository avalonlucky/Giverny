...
try {
  ...
  assert.match(worker, /async function changeDemoPassword[\s\S]+principal\.role !== 'admin'[\s\S]+verifyAdminPassword[\s\S]+setDemoPassword[\s\S]+revokePrincipalSessions\(env, DEMO_PRINCIPAL_ID\)/, '演示密码只能由管理员验证后修改，并注销旧演示会话')
  assert.match(worker, /path === '\/api\/auth\/demo-password'[\s\S]+changeDemoPassword/, '必须注册演示账号密码修改接口')
