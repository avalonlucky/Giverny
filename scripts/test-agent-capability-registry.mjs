...
check(agentCapabilityAllows('generate-settlement-receipt', 'demo', 'POST'), '演示账号应能在 demo 工作区生成结算回单')
check(!agentCapabilityAllows('generate-settlement-receipt', 'collaborator', 'POST'), '协作者不能生成财务回单')
check(agentCapabilityAllows('manage-settlement-export-preview', 'demo', 'POST'), '演示账号应能管理 demo 工作区的回单')
check(agentCapabilityAllows('manage-settlement-export', 'demo', 'POST'), '演示账号应能执行已确认的 demo 回单操作')
check(!agentCapabilityAllows('configure-ai-route-preview', 'demo', 'POST'), '演示账号不能修改全站模型路由')
