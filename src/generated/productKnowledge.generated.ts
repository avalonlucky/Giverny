...
export const productKnowledgeDocuments: ProductKnowledgeDocument[] = [
  {
    ...
    "content": "最新说明：工作助手完成任务后会展示可核验的“分析过程”，包括如何理解用户目标、制定什么计划、查找了哪些依据、提取了哪些事实以及如何校验结论。面板不再显示“不经过本机 CLI”、英文工具名或模型 ID；技术路由仍保留在管理员运行审计中。产品问题会先查询官方产品知识，规划模型漏掉工具调用时编排层会自动补查；每次构建会自动把本手册、Agent 运营手册和完整 `CHANGELOG` 生成检索索引。\n\n> 网址：**https://mayeai.com**\n> 这份手册写给日常使用者（你自己），每次功能更新都会同步到文末的「更新记录」。\n> 技术细节和接口文档见 `handoff/HANDOFF.md`，开发改动明细见 `CHANGELOG.md`。\n\n---"
  },
  {
    "id": "使用手册.md#2",
    "sourcePath": "使用手册.md",
