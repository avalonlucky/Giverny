...
export const productKnowledgeDocuments: ProductKnowledgeDocument[] = [
  {
    ...
    "content": "最新说明：结算页统一使用「自定义导出」：选择任意开始与结束日期后，下方回单预览和顶部总工时、计费工时、结算金额、已验收四项数据都会实时按该范围更新。日期范围后的三个线性图标依次用于导出 Excel、生成分享链接和导出 PDF，鼠标悬停可查看名称；即使选择月初到月末，也仍属于同一套自定义导出流程。「导出记录」已拆为独立选项卡，记录默认未锁定；可单独设置永久有效、1 / 7 / 30 天、自定义到期时间或立即停用。未锁定记录可直接删除，锁定记录删除时必须输入当前管理员登录密码。\n\n工作助手也可以直接生成正式回单。例如输入“请帮我导出 6 月 1 日到 6 月 10 日的结算回单”，爱丽丝会按真实业务数据生成 Excel，在回答下方给出“预览 / 在线预览 / 下载”三个入口，文件使用 Microsoft Excel 官方图标标识。“预览”会在工作助手上方打开完整回单；右上角分别提供缩小、`1:1`、放大、适合窗口和关闭。鼠标滚轮只上下滚动，切换到 `1:1` 后使用横向滚动条查看整张表。\n\n合作伙伴打开分享链接后，可在右上角自选下载 Excel 或 PDF。项目卡片与时间线内的图片继续使用缩略图预览；PDF 文件点击后会读取完整源文件，不再只展示首页封面。\n\n最新说明：结算回单已完全采用《结算回单_2026年7月_美化兼容版_v2.xlsx》正式模板。后台预览、合作伙伴分享页和下载 Excel 共用同一套品牌头、四栏信息带、14 列明细、斑马纹、状态色与合计行；工时保留两位小数，单价、小计和合计继续使用可核对公式。回单不再额外插入人民币大写、系统备注、印章或剪切线。\n\n最新说明：工作助手标题可打开历史记录，搜索、切换、删除已保存对话或开始新对话。当前界面不提供独立的“项目”或“临时对话”入口。\n\n最新说明：工作助手在回答前分开展示“模型推理”和“执行过程”。模型推理只来自设置中所选模型实际返回的 reasoning/thought；对象解析、工具查询与证据审核属于可核验执行过程，不再合并为“已思考 N 步”。技术模型 ID、内部工具名和路由细节不在普通对话中堆叠，管理员可在运行审计中复核。每次构建会自动把本手册、Agent 运营手册和完整 `CHANGELOG` 生成产品知识索引。\n\n最新说明：生产文本对话统一由 Google ADK 语义主链编排。Scope Supervisor 先理解用户所问的主体、范围和意图，不会因为出现“版本”就默认在问 Giverny 版本；无法唯一绑定任务、期刊或附件时，必须请用户补充，不能用相似对象代答。工具证据先经确定性 claim-evidence 校验，再由独立 Evidence Auditor 审核，通过后才能发布回答。\n\n> 网址：**https://mayeai.com**\n> 这份手册写给日常使用者（你自己），每次功能更新都会同步到文末的「更新记录」。\n> 技术细节和接口文档见 `handoff/HANDOFF.md`，开发改动明细见 `CHANGELOG.md`。\n\n最新说明：工作助手具有完整的“生产保护”。Google ADK 协调模型的失败、超时或无效结果会显式停止，不用旧 Agent 或本机 CLI 伪装成成功回答。设置页“运行与质量中心”会显示回答成功率、事实验真率、响应时间和后台任务稳定性；突破红线时明确阻止发布。\n\n网站更新也不再一次性替换正式版本。系统会先上传候选版本，逐项核对健康接口、事实协议、版本号和前端资源；全部通过后才正式推广，任何一项失败都会自动恢复上一稳定版本。外部运行告警使用签名事件，并且不发送用户问题、任务、附件、金额或模型回答正文。\n\n---"
  },
  {
    "id": "使用手册.md#2",
    "sourcePath": "使用手册.md",
    ...
    "category": "产品手册",
    "heading": "企业记忆",
    "content": "在工作助手中打开“记录与任务 → 后台任务 → 企业记忆”，可以保存组织规则、合作伙伴长期偏好和项目约定。每条记录必须填写来源说明，可选择长期有效、30 天、90 天或 1 年。\n\n“纠正”会生成新版本并保留旧版本；“设为失效”或“删除”后，Agent 默认不再引用。也可以直接告诉爱丽丝“请记住这条规则”或“纠正某条合作伙伴记忆”，系统会先展示确认卡，确认后再写入。"
  },
  {
    "id": "使用手册.md#3",
    "sourcePath": "使用手册.md",
    ...
    "content": "工作助手现在由统一 Agent 编排层组织，不会把大模型的一次回答直接当成最终事实。它会先理解问题、制定步骤，再按当前登录工作区调用任务、附件、财务或产品说明工具，最后核对工具证据后回答。\n\n如果工具临时失败或验真发现缺少关键证据，Agent 会在同一业务目标下补充最小必要证据；仍无法取得可靠证据时会明确停止或请你补充，不会用猜测填空。管理员可在「设置 → 模型 → AI 运行中心 → Agent 执行审计」查看本轮设置模型、各编排角色的实际模型、工具状态、确定性证据和审核结果。审计不保存问题和回答正文，并按当前工作区隔离。\n\n- 查询金额、工资或工时：以网站计算结果为准，模型不能自行改数字。\n- 查询具体任务卡点或延期：读取任务详情和当前等待记录后回答。\n- 修改数据：先展示草稿，只有明确确认后才执行。\n- 模型边界：Google ADK 是编排框架，不是另一个模型。工作助手的语义主管、协调器、专家、回答格式化和证据审核必须全部使用设置中本轮选定的同一服务商与模型；配置缺失、调用失败或实际模型不一致时直接停止，不会暗中切换 Gemini 或备用模型。任务命名、文案、后台洞察等非 Agent 功能继续遵守各自的主备模型规则。\n- 本机 CLI：「设置 → 本机 CLI」仍可配对、扫描和诊断本机连接；配置生产 ADK 后，它不参与网站文本对话路由。\n\n---"
  },
  {
    "id": "使用手册.md#4",
    "sourcePath": "使用手册.md",
  ...
  },
  {
    ...
    "content": "**持续计划怎么用：** 可以直接说“为任务 #123 制定从当前进展到验收的持续计划”或“这个任务后续帮我一直跟进”。爱丽丝会把目标拆成 2–8 个可核对步骤并保存到 D1，而不是只在当前回答里输出一份临时清单。计划会出现在右上角「Agent 任务中心」；后续确认执行进展、等待、验收文件或完整验收时，对应步骤自动完成。\n\n**任务级长期记忆：** 每个任务都有独立记忆摘要，包含任务需求、近期进展、最近甲方反馈、等待情况和未解决事项。询问“这个项目还有什么没处理”或“甲方之前偏好什么”时，Agent 会先读取任务记忆，再以实时任务详情核对。记忆用于减少重复解释，不会覆盖任务原始数据，也不会把不同任务的上下文混在一起。\n\n**主动提醒规则：** 系统每天 9 点检查尚未结束的任务。以下情况会生成提醒：超过预计交付时间、进度 100% 但未验收、实际工时超过预估 25%、存在等待记录、已有验收文件但缺少验收备注。提醒只进入管理员的 Agent 任务中心，不会自动修改任务或发送给甲方。\n\n**主动事项怎么用：** 任务创建、修改和上传附件后会即时检查风险，定时扫描负责兜底。打开工作助手右上角「记录与任务 → 后台任务 → 主动事项」，可以看到按紧急、高、中、低排序的待办。展开后会显示事实依据和建议处理，可执行建议、查看任务、24 小时后提醒、标记已解决或标记无需处理。相同风险不会重复刷屏；处理后只有任务数据再次变化才会复发，风险自然消失时系统会自动闭环。顶部会显示待处理数、高风险数、解决率和平均响应时间。\n\n**确认卡编辑：** 所有 Agent 写入草稿均可在确认卡里点击「编辑草稿」。修改后系统会重新调用对应 preview 工具校验必填项、时间和附件归属，并生成新的确认凭证；旧 Workflow 自动取消。确认前修改不写入业务数据。\n\n**在聊天里上传文件：** 图片和文本仍可直接作为当前问题的理解材料。PDF、Office、压缩包、视频以及需要长期保留的图片，可以写明任务编号，例如“把这个 PDF 上传到任务 #123，并作为进展附件”；没有编号时，Agent 会先根据对话定位任务并返回上传接力。目标明确后，真实文件由当前登录浏览器直接上传 R2，写入 D1 后再供 Agent 查询或纳入验收；PDF/Office 等文件二进制不会进入模型上下文，明确要求识图时的图片仍会交给受控视觉模型理解。任务存在歧义时必须先让用户选择，不会生成无归属文件，API Key 也不会随附件进入模型。\n\n**在聊天里维护附件：** 可以要求爱丽丝重命名附件、调整标签 / 范围，或设置是否允许合作伙伴查看。系统先展示原值与新值，确认后才由 Workflow 执行；真实扩展名不能被改写，锁定结算月份不能修改，合作伙伴看不到未明确对外开放的附件。\n\n**结算、排期、提醒和模型设置：** 管理员可以让爱丽丝按日期范围导出结算 Excel/分享链接、管理快照锁定和分享有效期；成员可以检查排期冲突、确认后调整任务排期，并创建任务中心定时提醒。模型设置只允许管理员脱敏检查、测试已保存凭证和确认后修改路由，聊天中不能录入或读取明文 API Key。所有真实修改仍经过“预览 → 确认 → Workflow”，结算数据在预览后变化会使旧确认凭证失效。\n\n**失败学习如何工作：** 系统只记录匿名技术特征，如失败类别、意图类别、工具名、HTTP 状态和出现次数。相同失败指纹出现两次后进入 required，后续版本必须增加对应回归场景。系统不会把聊天原文、任务标题、回答正文或操作草稿保存到失败样本。"
  },
  {
    "id": "使用手册.md#21",
    "sourcePath": "使用手册.md",
    ...
    "category": "产品手册",
    "heading": "Agent 生产控制台",
    "content": "**管理持续计划：** 打开工作助手右上角的任务中心，在「计划与提醒」中展开计划。每一步都可以人工标记完成或重新打开；长期计划可暂停、继续或取消。暂停只影响计划自动推进，不会改变任务状态、进度或工时。\n\n**处理主动提醒：** 展开提醒后点击「执行建议」，爱丽丝会根据逾期、验收、等待或工时超限类型，先查询对应任务，再生成下一步确认草稿。该按钮不会跳过确认直接写入。\n\n**查看和纠正记忆：** 在任务中心切换到「任务记忆」，展开任务后可查看自动摘要和待处理事项。可以补充“甲方偏好”“项目约束”等人工纠正，也可以忽略已经解决的待办。发现记忆不应继续保留时，点击「停止并清空记忆」并二次确认；重新启用后会从任务权威数据重新生成。\n\n**查看失败回归：** 管理员进入「设置 → AI 模型设置 → Agent 运行质量」，展开“失败学习与回归”。同一匿名失败出现两次后标为“待回归”；修复并加入自动测试后可标记“已覆盖”，非产品问题可标记“忽略”。再次出现的已覆盖失败会重新升级为待回归。\n\n**查看质量和成本：** 同一页面展示 7 天 / 30 天真实运行次数、成功率、P95 响应、工具调用率、输入 / 输出 Token 估算及参考成本，并按模型比较运行次数、成功率、平均响应、Token 和成本。Token 和费用是趋势估算，不替代模型供应商账单。\n\n**费用审批红线：** 真实模型回归、正式部署、扩容、批量重放或开通付费服务等可能产生外部费用的开发 / 运维动作，必须先说明供应商、动作、最大调用或资源数量和预计费用，并取得负责人对本次动作的明确批准。一般性的“继续”或“修复”不视为费用授权；未批准时只能做本地静态检查和 Mock 测试。\n\n**单次调用上限：** 工作助手每个外层问题的规模由时间约束（默认 240 秒），调用次数上限只作为防打转开关；正常问答不会触发，包含主体理解、协调与专家、回答整理、证据审核，以及审核不通过时的一次修正与一次复核；达到上限会停止，不继续扇出。修正只有一轮，不会反复重试。网站会核对 Runtime 回报的上限声明，缺失或不一致时不发布答案。\n\n**什么时候调整模型：** 只有积累至少 7 天、30 次真实请求后，系统才根据失败率、回落率、P95 响应和消歧率给出调优建议。样本不足时先继续使用和修复明确失败，不应因为一两次回答就频繁换模型。系统只给建议，不会自动修改模型或 API Key。\n\n完整原理、安全边界和运营说明见 `docs/AGENT_PRODUCTION_OPERATIONS.md`。\n\n**Runtime 架构：** 生产工作助手的文本对话必须进入 Google ADK Root Coordinator，由语义主管动态选择工作区分析、产品支持、联网研究或事务预览专家。当 ADK 无结果或异常时，网站显式停止并告知用户，不回落到 Alice/LangGraph 旧编排、关键词事实策略或本机 CLI。D1/R2/Vectorize、身份权限、确认动作和业务写入仍始终由网站 Worker 管理。\n\n**需求人画像：** 需求人画像由 Worker 后台确定性计算，而不是由前端或模型临时猜测。`get_requester_profile` 会在当前工作区内读取该需求人的全部历史任务，聚合项目数、工时、验收率、准时率、工时偏差、改稿、等待和反馈特征。用户询问某人的用户画像、合作画像、历史偏好或报价排期建议时，Agent 编排层必须调用这个工具；模型漏调会被验真阶段补查，最终回答也会用工具事实校准。\n\n**产品能力库：** 快捷键帮助面板直接读取 `src/productCapabilities.ts`，`search_product_help` 工具也读取同一份结构化数据。每条能力包含标题、分类、入口、快捷键、适用角色、搜索别名、操作说明和权限边界。更新网站功能时应同步维护能力条目，而不是只改界面文案或在某个模型 Prompt 中追加答案。Google ADK 产品支持专家和网站帮助面板因此读取同一事实。\n\n**Runtime 身份：** 当用户询问“现在用哪个模型”时，网站应以设置中本轮选定的模型和 Google ADK 审计回报为准。语义主管、协调器、专家和证据审核器的服务商与模型必须完全一致；回答 JSON 由本地强类型协议解析，不额外调用另一模型。任一回报不同都阻止答案发布。本机 CLI 配对状态只说明连接是否可用，不代表当前网站对话正在使用该 CLI。\n\n**推理与执行过程：** “模型推理”只展示当前所选模型实际返回的 reasoning/thought 内容；对象解析、工具查询和证据审核单列为“执行过程”。系统不会用阶段模板或定时动画冒充模型思考。具名业务对象或归属不明确的对象会先查询工作区证据，再决定由哪个专家处理。"
  },
  {
    "id": "使用手册.md#22",
    "sourcePath": "使用手册.md",
  ...
  },
  {
    ...
    "category": "产品手册",
    "heading": "模型入口与路由规则",
    "content": "- **自动 · Google ADK**：先把设置中的文字主模型解析为本轮唯一模型，再由语义主管根据完整问题选择最小专家与工具集；“自动”只自动选择专家和工具，不允许自动更换模型，也不经过本机 CLI。\n- **工作助手模型**：设置中选中的服务商和模型就是 Google ADK 本轮所有角色的唯一模型。Runtime 必须回报实际服务商与模型，网站核对一致后才发布答案；失败时停止，不回退 Gemini、备用模型或其他服务商。\n- **图片问题**：对话中的图片、PDF 页面和交付件分析只有在当前所选模型支持相应能力时才执行；不支持时会明确提示改选模型，不静默切换视觉模型。非对话的附件命名和识图功能继续使用设置页的识图主备模型。\n- **写入操作**：无论由哪个模型理解意图，创建、修改、进展、反馈和验收都必须经过网站权限校验与确认流程。\n\nDeepSeek 官方直连模型使用 `deepseek-v4-flash` 和 `deepseek-v4-pro`。旧配置中的 `deepseek-chat`、`deepseek-reasoner` 会自动迁移为 V4 Flash，不需要手动清理历史设置。选择 DeepSeek 后，工作助手仍由 Google ADK 负责编排，但 ADK 内的所有模型角色都会通过兼容适配器实际调用所选 DeepSeek 模型；前台不得显示 DeepSeek、后台却运行 Gemini。新建任务 AI、工时建议、文案和其他非 Agent 文字能力继续按各自路由运行。\n\n桌面端点击工作助手后，会从页面右侧展开全高抽屉，保留左侧工作台作为当前操作上下文；手机与窄屏设备使用全屏模式，并在顶部操作区显示带文字的「工作助手」入口，避免图标快捷操作收起后无法打开助手。关闭按钮、历史对话、任务中心、附件和模型选择仍位于抽屉内部。\n\n工作助手输入框支持直接按 `Command + V` / `Ctrl + V` 粘贴截图。粘贴后图片先显示为附件缩略图，发送时由 Google ADK 编排受控视觉链路处理；图片和文件不会因本机 CLI 是否在线而改变对话路由。\n\n生产文本对话始终使用 Google ADK，包括产品问答、工作区业务查询、多步分析、图片理解和写入预览。Bridge 离线、版本较低、CLI 未登录或本机测试失败，只会影响「设置 → 本机 CLI」的连接状态，不会触发对话主链切换或云端回退。\n\n本机 CLI 获取的 Giverny 权限是 5 分钟一次性只读令牌，命令结束或取消后立即撤销。在本机生成的文件固定保存到 `~/.giverny/workspace`。完整安全模型见 `docs/LOCAL_CLI_BRIDGE.md`。\n\n**外部 Agent 只读工具接口：** Worker 同时通过 `/mcp` 暴露只读 MCP 工具。它使用独立的 `MCP 只读`访问口令，不能登录网站，也不能执行任何写入：\n\n- `GET /api/agent/tools/month-finance`：月份金额统计（总金额、计费工时、时薪、任务明细）\n- `GET /api/agent/tools/settlement-exports`：结算导出、锁定和分享有效期记录\n- `GET /api/agent/tools/search-tasks`：任务搜索\n- `GET /api/agent/tools/schedule-conflicts`：按起止时间查询排期重叠任务\n- `GET /api/agent/tools/search-attachments`：附件搜索与可验证文件地址\n- `GET /api/agent/tools/task-detail`：任务详情\n- `GET /api/agent/tools/context`：爱丽丝能力边界说明\n- `GET /api/agent/tools/product-help`：快捷键、功能入口、操作流程、模型路由和权限规则\n- OpenAPI 描述：`/api/agent/openapi.json`；支持 MCP 的客户端优先使用 `https://mayeai.com/mcp`。"
  },
  {
    "id": "使用手册.md#24",
    "sourcePath": "使用手册.md",
  ...
  },
  {
    ...
    "content": "平台启用了 **Cloudflare Workers AI**（边缘自带开源模型，无需任何外部厂商 Key），作为部分非 Agent 文字功能的「文字主模型 → 文字备用模型 → Workers AI」三层链路兜底；它不参与 Google ADK 工作助手对话。工作助手无论选择「自动」还是具体模型，都严格使用设置中解析出的本轮唯一模型，失败时停止，不进入 Workers AI。非 Agent 功能的默认兜底模型为 Llama 3.3 70B，可经环境变量 `WORKERS_AI_MODEL` 覆盖。计费按 Neurons（$0.011/1000 Neurons，每天免费 1 万 Neurons）。\n\n> 注意：Workers AI 提供的是 Cloudflare 自托管的开源模型（Llama、Qwen、Mistral 等），**不包含 GPT、Gemini 这类闭源商业模型**——后者仍需用各自厂商的 Key，经 AI Gateway 代理调用。"
  },
  {
    "id": "使用手册.md#27",
    "sourcePath": "使用手册.md",
  ...
  },
  {
    ...
    "category": "产品手册",
    "heading": "BAML 与多模型路由",
    "content": "项目已引入 BAML 工程结构，AI 提示词和输出类型会先维护在 `baml_src/ai_assistants.baml` 中，并通过 `npm run baml:generate` 生成 TypeScript client。\n\nBAML Runtime 用于新建任务辅助、工时建议等非对话结构化 AI 功能，不是工作助手的 Agent 编排层。这类功能在 BAML Runtime 不可用时可回退到 Cloudflare Worker 内的文字主模型 / 备用模型；任务、工时、附件和结算数据仍以 D1 / R2 的真实记录为准。"
  },
  {
    "id": "使用手册.md#31",
    "sourcePath": "使用手册.md",
  ...
  },
  {
    ...
    "category": "产品手册",
    "heading": "结算（月报核心流程）",
    "content": "月底结算的标准流程：\n\n1. 在「自定义导出」里选择要结算的开始日期和结束日期，核对回执单上的任务和工时\n2. 点日期范围后的 **分享图标** → 创建未锁定回单快照并复制链接；确认数据不再变动后，在导出记录中手动点击「锁定」\n3. 把 `/settlement-share/xxxx` 链接发给合作伙伴；对方可在线预览并下载同一份 Excel\n4. 需要 PDF 的话点日期范围后的 **PDF 图标** → 打印窗口选「另存为 PDF」（文件名自动带日期范围）\n\n**锁定规则：**\n- 生成链接只创建未锁定快照，不会阻止继续修改任务和金额\n- 任意日期范围都进入「导出记录」，可以复制链接、在线预览和下载\n- 确认数据不再变动后，在对应导出记录中手动点击「锁定」；锁定记录删除时需要管理员密码\n- 如果分享后数据又发生变化，重新生成一条新快照并发送新链接，旧链接继续保留原快照内容"
  },
  {
    "id": "使用手册.md#61",
    "sourcePath": "使用手册.md",
  ...
  },
  {
    ...
    "category": "产品手册",
    "heading": "合作伙伴分享页",
    "content": "- 不需要任何口令，凭链接直接看\n- 内容：分享时保存的日期范围、任务、工时、金额与验收备注快照\n- 可在线预览，并直接下载与预览相同的 Excel 回单\n- 合作伙伴只能看到分享页内容，不能进入后台，也不能新增、修改或删除任何数据"
  },
  {
    "id": "使用手册.md#63",
    "sourcePath": "使用手册.md",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 使用手册",
    "category": "产品手册",
    "heading": "2026-08-12 · v0.39.0 工作助手认识这个网站了",
    "content": "- **不再只靠搜索**。此前你说「结算回单」，它把这四个字当成一个陌生名字，去任务标题里模糊搜索——尽管「结算」就是左边导航栏里的一项。现在八个业务领域（工作台 / 任务 / 文件库 / 洞察 / 结算 / 收入 / 知识库 / 设置）是它的常识：每个域是什么、你会怎么称呼它、里面有哪些字段、该查哪里。\n- **说法不用统一**：「回单」「对账单」「导出记录」「结算单」都指向同一个地方，一共认 94 种说法。\n- **读不到的会直说**：洞察页面的诊断记录、知识库里的笔记，它没有读取权限。现在会如实告诉你读不到，而不是换关键词一直搜或者编一个。\n- 这份地图跟着代码走：改了导航、字段或工具而地图没同步，构建会直接失败，不会出现\"网站改了、助手还按老一套回答\"。"
  },
  {
    "id": "使用手册.md#81",
    "sourcePath": "使用手册.md",
    "sourceLabel": "Giverny 使用手册",
    "category": "产品手册",
    "heading": "2026-08-12 · v0.38.0 答案核对机制重做",
    "content": "- **回答变快**：事实核对不再需要额外跑一次模型。此前它已经想出答案了，还要把全部资料打包重新核一遍，15–25 秒后才决定给不给你看——这就是\"思考完了还要等很久\"的原因。现在核对是本地瞬时完成的。\n- **正确答案不会再被误拦**：此前核对依赖模型自己填写的证据编号，编号填错就算答案完全正确也会被整段拦下。现在直接核对答案里的日期、金额、版本号这些值在不在资料里。\n- **编造仍然拦得住**，而且更准：编造的日期或金额根本不会出现在资料里。\n- 问\"结算回单\"这类问题时，不会再拿回毫不相关的记录——对象识别此前只要搜到词就算\"认出来了\"。"
  },
  {
    "id": "使用手册.md#82",
    "sourcePath": "使用手册.md",
    "sourceLabel": "Giverny 使用手册",
    "category": "产品手册",
    "heading": "2026-08-12 · v0.37.10 修复上一版引入的回归",
    "content": "- 修复正常回答被误拦、显示「未声明有效的执行边界」的问题。这是上一版改动引入的，已补上能拦住同类问题的测试。\n- 对话里不再出现底层框架名和内部数字，报错统一改成人话。"
  },
  {
    "id": "使用手册.md#83",
    "sourcePath": "使用手册.md",
    "sourceLabel": "Giverny 使用手册",
    "category": "产品手册",
    "heading": "2026-08-12 · v0.37.9 放开额度与推理面板整理",
    "content": "- 检索类问题不会再因为\"额度用完\"而失败。次数上限提到正常用不到的高度，只保留防打转作用；真正的边界是单轮 240 秒时间。\n- 「模型推理」里不再夹杂英文——之前模型看到英文数据就会切换语言写分析。\n- 推理内容按段落显示，各阶段有独立小标题，不再是一大坨文字。\n- 回答完成后推理自动收起为一行「模型推理 · N 字，点击展开」，界面干净；生成过程中仍然实时展开，方便你看它在想什么。"
  },
  {
    "id": "使用手册.md#84",
    "sourcePath": "使用手册.md",
    "sourceLabel": "Giverny 使用手册",
    "category": "产品手册",
    "heading": "2026-08-12 · v0.37.8 预算耗尽不再报\"服务不可用\"",
    "content": "- 修好\"问到一半显示 Agent Runtime 暂时不可用\"：那不是服务故障，是这一轮的模型调用次数用光了。现在会如实说明\"这次检索范围太大\"并告诉你怎么缩小，同时列出已经检索过的数据，而不是弹一句英文报错。\n- 单个问题的调用上限从 9 次提到 11 次：检索类问题原本的余量不足，一次正常检索就会把额度用完。\n- 任何内部错误都不再把英文异常显示在对话里，只显示一句\"这一轮没能完成，请稍后再试一次\"。\n- 检索更省：覆盖面最广的搜索命中后，不再用窄范围搜索重复确认同一件事；没有精确匹配时直接告诉你\"没找到 + 最接近的候选\"。"
  },
  {
    "id": "使用手册.md#85",
    "sourcePath": "使用手册.md",
    "sourceLabel": "Giverny 使用手册",
    "category": "产品手册",
    "heading": "2026-08-11 · v0.37.7 回答稳定性",
    "content": "- 修好\"同一个问题有时答得出有时答不出\"：证据审核此前用默认温度运行，同一份输入时松时严；而它一旦判不通过，整段正确答案就被丢掉。现在审核改为零温度，判断标准也明确区分\"会让你拿到错误事实\"（拦）和\"措辞可以更好\"（只记录，不拦）。\n- 审核不通过时会按意见自动修正一次，修正稿必须重新通过审核才输出。此前是直接放弃整个答案。\n- 闸门没有放松：修正只有一轮，不会反复重试；修正稿没通过照样不输出；引用不存在证据这类硬伤不给修正机会。\n- 单个问题的模型调用上限从 7 次提到 9 次，新增的两次只在审核不通过时才发生。\n- 回答里的「最新」会写明是哪个维度（最新上传 / 最新提交 / 最新审批），不让你自己猜。\n- 「模型推理」面板恢复可读：内部字段名此前被统一替换成同一个词，句子读不通。\n- 新增统一的回答排版规范：结论先行、关键值加粗、三项以上数据用表格，金额/工时/日期格式与界面一致。"
  },
  {
    "id": "使用手册.md#86",
    "sourcePath": "使用手册.md",
    "sourceLabel": "Giverny 使用手册",
    "category": "产品手册",
    "heading": "2026-08-11 · v0.37.6 推理链落地与内部名词消毒",
    "content": "- 「模型推理」面板现在真的有内容：之前没有任何一处向模型申请推理输出，混合推理模型（如 DeepSeek V4）不打开开关就不返回推理，所以面板注定为空。现在对象判断、主协调与各专家都会申请推理，推理内容逐字实时出现。\n- 模型没被要求返回推理时，不再显示「等待模型返回真实推理内容」的空占位符，只展示真实的执行过程。\n- 推理内容里的接口名、内部角色名与框架名会被换成自然语言（例如 `search_attachments` 显示为「搜索任务附件」）；界面另有一层兜底，任何一层漏了都不会把技术细节端到你面前。\n- 修复偶发的「执行失败：请检查服务状态后重试」：流式文本拼装此前会吞掉与上一片相同的分片，丢一个字符后整轮结论就解析失败。\n- 证据审核阶段不申请推理：它的推理里带着还没通过校验的答案草稿，不展示，也不为此多付费用。\n- 单轮仍最多 7 次模型调用，全链路继续使用设置中选定的同一模型。推理输出会产生推理 token，同一问题的单轮费用比关闭推理时略高。\n- 需要你补充信息的回答变快了约三分之一：结论没成形时不再多跑一次复核，那次复核改变不了任何输出。这类回答的思考链也不再显示\"未通过，已阻止未验证结论\"这种和\"请你补充信息\"互相矛盾的说明。\n- 编排记录里的「参与环节」终于有内容：此前因为只读取委派工具名而不是专家名，这一栏永远是空的。\n- 超时改为按整轮计算（默认 240 秒），不再出现各阶段各等一遍、最长四分多钟才告诉你失败的情况。\n- 非网页方式发起的对话（脚本、外部集成）也不会再遇到 520 中断。\n- 让它写文案、改文字或闲聊时，不会再莫名收到\"我已查到相关资料，但未通过校验\"——这类请求本来就不需要查工作区数据。真正陈述业务事实时缺证据，仍然会照常拦下。\n- 回答里出现 `Logo 3`、`KV 1` 这类\"文字加数字\"时，不会再被误认成版本号而拦下正确答案。\n- 同名对象有多个候选时，产品自身的使用问题不会再被强行按工作区数据来回答。\n- 修掉两处会随使用时间累积的内存占用，长时间运行更稳。"
  },
  {
    "id": "使用手册.md#87",
    "sourcePath": "使用手册.md",
    "sourceLabel": "Giverny 使用手册",
    "category": "产品手册",
    "heading": "2026-08-11 · v0.37.5 结构化结果采集修复",
    "content": "- 修复 Root Coordinator 已经返回完整澄清答案，却被 ADK 临时事件覆盖后误报 `invalid AgentTurnOutput contract` 的问题。\n- Scope Supervisor、Root Coordinator 与 Evidence Auditor 现在只接受各自最后一个通过强类型校验的结果；专家过程输出不会冒充最终答案。\n- 兼容模型生成的澄清类 claim 标签。对象无法唯一匹配时应正常展示候选项并请用户确认，不再显示 Runtime 失败。\n- 后台保存的当前模型是 ADK 唯一模型来源；浏览器里的旧缓存不能覆盖后台选择。设置 A 时主管、协调、专家与审核全部使用 A，设置 B 时全部使用 B，DeepSeek 不作为默认修复或失败回退。\n- 单轮最多仍为 7 次模型调用，所有角色继续严格使用设置中选定的同一模型，不启用隐藏 Gemini 或其他回退。"
  },
  {
    "id": "使用手册.md#88",
    "sourcePath": "使用手册.md",
    "sourceLabel": "Giverny 使用手册",
    "category": "产品手册",
    "heading": "2026-08-11 · v0.37.4 真实模型推理与对象取证",
    "content": "- 思考面板中的“模型推理”只展示设置中所选模型实际返回的 reasoning/thought 内容，不再把阶段模板或定时动画当成思考链。\n- 对象解析、工具查询和证据审核改在独立的“执行过程”中展示；模型没有返回可展示推理时会明确说明，不伪造内容。\n- 询问具名业务对象或归属不明确的版本时，Scope Supervisor 会先查询工作区的任务、附件和对话证据，再决定专家范围；不能唯一绑定时要求选择，不用相似项目代答。\n- 修复 DeepSeek 已返回完整推理与 JSON、却因 `subject` 字段形态差异被误判为无结果的问题。\n- 单个外层问题最多仍为 7 次模型调用；对象取证替代原有格式化模型的调用预算，没有扩大单轮费用上限。"
  },
  {
    "id": "使用手册.md#89",
    "sourcePath": "使用手册.md",
    "sourceLabel": "Giverny 使用手册",
    "category": "产品手册",
    "heading": "2026-08-11 · v0.37.3 ADK Runtime 迁移 DMIT",
    "content": "- Google ADK 语义编排服务已从 Google Cloud Run 迁移到现有 DMIT VPS；生产对话不再依赖 Google Cloud 容器或 Gemini，因此不会因为容器常驻或隐藏 Gemini 调用产生 Google 账单。\n- ADK 是编排框架，不是模型供应商。工作助手实际使用的模型仍以「设置 → AI 模型」中的本轮选择为准；选择 DeepSeek 时，语义主管、协调器、专家与证据审核全部使用同一个 DeepSeek 模型，回答结构由本地协议解析。\n- Runtime 与 VPN 分开运行：ADK 使用独立 HTTPS 8443，原有 Xray、订阅、共享节点和 3x-ui 端口保持不变。\n- 管理员可访问 `https://mayeai.com/api/health?runtime=1` 检查 Worker 到 Runtime 的网络状态；该检查只读取健康信息，不调用模型，不产生 Token 费用。\n- 真实模型回归仍需提前说明供应商、最大调用次数和费用范围并取得明确批准；单个外层问题最多 7 次模型调用。"
  },
  {
    "id": "使用手册.md#90",
    "sourcePath": "使用手册.md",
    "sourceLabel": "Giverny 使用手册",
    "category": "产品手册",
    "heading": "2026-08-11 · v0.37.2 思考过程可见与长耗时问答修复",
    "content": "- 工作助手会边做边显示真实动作：先说明它判断你在问哪个对象和哪个维度，再逐条显示正在读取任务详情、搜索任务附件等步骤，最后复核结论与证据。\n- 模型思考期间，它的推理内容会实时逐字显示；即使某一步需要想几十秒，界面也不会停住不动。推理过程与最终答案是分开的，答案仍然必须先通过证据校验才会出现，不会提前显示未核实的结论。\n- 思考链里不再放写死的开场句，第一行就是模型对你问题的真实判断。\n- 复杂问题的编排可能需要一到两分钟。此前这段等待会被中途判定为链路异常并提示服务不可用，现在会持续保持连接直到答案返回。\n- 思考过程只显示自然语言步骤，不再出现底层框架名称或接口名；完整的工具调用记录仍可在「设置 → 模型 → AI 运行中心 → Agent 执行审计」查看。\n- 思考链展开时不再把同一句话显示两遍；摘要预览只在你手动收起后出现。\n- Google ADK 现在严格使用设置中本轮选定的同一服务商与模型完成语义主管、协调、专家回答和证据审核；任何角色模型不一致都会停止回答。真实模型测试、部署或扩容等可能产生费用的操作，必须先给出调用上限和预计费用并取得明确批准。"
  },
  {
    "id": "使用手册.md#91",
    "sourcePath": "使用手册.md",
    "sourceLabel": "Giverny 使用手册",
    "category": "产品手册",
    "heading": "2026-08-10 · v0.37.1 Google ADK 语义 Agent 主链",
    "content": "- Scope Supervisor 先理解完整问题的主体、范围和意图；“版本”不再默认指 Giverny 产品版本。\n- 查询《昂楷之道》B09 / B10 等具名对象时，只根据当前工作区任务与附件证据回答；无法唯一绑定时请用户补充，禁止用相似项目代答。\n- 回答先经过确定性 claim-evidence 校验，再由独立 Evidence Auditor 审核；证据不足时转为澄清或拒答。\n- 已配置 ADK 时，文本对话不回落 LangGraph/Alice 旧编排、关键词事实策略或本机 CLI；本机 CLI 配对与扫描界面继续保留。"
  },
  {
    "id": "使用手册.md#92",
    "sourcePath": "使用手册.md",
  ...
  },
  {
    ...
    "category": "运营手册",
    "heading": "3. 可控长期记忆",
    "content": "任务记忆按任务隔离，自动压缩需求、近期进展、合作伙伴反馈和未解决事项。任务详情仍是权威事实，记忆只用于减少重复解释。\n\n用户可以：\n\n- 增加人工纠正，例如“合作伙伴偏好简洁排版”。\n- 忽略已经解决或判断错误的待办。\n- 恢复此前忽略的待办，让系统重新计算。\n- 清空并停止某个任务的记忆；重新启用后从权威任务数据重新生成。\n\n记忆不会跨任务混用，也不能覆盖任务原始字段、工时或验收记录。\n\n任务中心另有“企业记忆”，用于保存跨任务延续的组织规则、合作伙伴偏好和项目决策。每条企业记忆必须标明来源，可设置有效期；纠正时保留旧版本，失效或删除后不再进入默认 Agent 查询。管理员和协作者可维护，只读成员仅可查询，合作伙伴与访客不可读取。Agent 写入前必须展示确认卡，不能把模型推测直接保存为已确认事实。"
  },
  {
    ...
    "category": "运营手册",
    "heading": "4. 主动提醒与快捷处理",
    "content": "主动工作不再只是每天生成一批普通提醒。任务创建、修改或上传附件后会即时检查；定时器继续作为漏扫兜底。逾期、今天/明天到期、进度 100% 未验收、实际工时超过预估 25%、活动等待和已有验收文件但缺验收备注，会进入独立的「主动事项」队列。\n\n每条主动事项保存事实证据、紧急/高/中/低优先级、建议动作、首次和最近发现时间。相同任务和信号只保留一条待办；解决或忽略后，任务源数据没有新变化时不会立刻重复出现。任务状态变化使风险消失时，系统自动标记为已解决。\n\n任务中心支持「执行建议」「查看任务」「24 小时后提醒」「已解决」和「无需处理」。顶部汇总待处理、高风险、解决率和平均响应时间；后台同时统计解决、自动解决和忽略比例，用于判断提醒是否有效或存在误报。\n\n点击“执行建议”后，系统会按提醒类型生成明确指令，例如检查逾期原因并生成进展草稿、核对资料并生成验收草稿。Agent 必须先读取真实任务数据，再返回可确认操作。\n\n管理员也可以直接对 Agent 说“明天 9 点提醒我核对任务 #123”。Agent 会先返回提醒草稿，确认后写入任务中心；它不会借此自动发送邮件、飞书或修改任务。"
  },
  {
    ...
    "sourceLabel": "Agent 运营手册",
    "category": "运营手册",
    "heading": "5. 结算、排期、附件与模型设置",
    "content": "- **结算**：管理员可让 Agent 查询导出记录，按任意日期范围生成回单 Excel 与分享链接，并确认后锁定快照或设置分享有效期。结算预览会绑定数据校验值；确认前数据发生变化时必须重新预览。\n- **排期**：成员可查询时间重叠，并在确认卡中核对任务、新旧开始/交付时间、预估工时和冲突任务后再调整。锁定结算月份不能修改。\n- **附件**：Agent 确定目标任务、附件范围和文件清单，真实文件由当前登录浏览器直接上传 R2。上传后可读取 OCR / 提取文字及分析结论，并用 `[attachment:ID:*]` 引用对应证据；可查询 missing、processing、completed、failed 等状态，对失败分析生成重试确认卡。\n- **附件维护**：成员可在确认卡中修改文件名、标签、范围和合作伙伴可见性。执行前会复核旧状态，文件扩展名不可伪造，锁定结算月份不可修改，合作伙伴始终看不到未明确设为对外可见的附件。\n- **模型设置**：仅管理员可查看脱敏路由、测试现有凭证或修改路由。Agent 不接受明文 API Key，只能使用设置页或环境中已经安全保存的凭证。"
  },
  {
  ...
  },
  {
    ...
    "sourceLabel": "Agent 运营手册",
    "category": "运营手册",
    "heading": "7. 质量、Token 与参考成本",
    "content": "设置页按 7 天或 30 天展示成功率、工具调用率、P95 / 平均响应、审批与消歧次数、模型回落和失败次数。Token 根据请求与结构化响应字符量近似计算；参考成本使用内置模型单价估算。\n\n参考成本只用于比较趋势和模型路线，不替代 DeepSeek、豆包、Gemini 等供应商账单。涉及财务核算时必须以供应商控制台为准。\n\n任何可能产生外部费用的开发或运维动作，包括真实模型回归、Google Cloud / Cloudflare 正式部署、扩容、批量重放和付费服务开通，执行前必须由负责人明确批准。申请时必须先说明供应商、动作、最大请求或资源数量及预计费用；“继续”“修复一下”等一般开发指令不构成费用授权。未获当前轮明确批准时，只能进行本地静态检查、Mock 测试、文档修改和不调用模型的健康检查。生产 ADK Runtime 运行在现有 DMIT VPS；更新该 Runtime 不得改动 Xray/x-ui 端口或擅自升级 VPS 套餐。"
  },
  {
  ...
  {
    "id": "docs/AGENT_PRODUCTION_OPERATIONS.md#10",
    "sourcePath": "docs/AGENT_PRODUCTION_OPERATIONS.md",
    "sourceLabel": "Agent 运营手册",
    "category": "运营手册",
    "heading": "9. 生产保护与受控发布",
    "content": "运行中心会把回答成功率、事实验真率、P95 响应、主模型完成率和后台任务失败率统一判定为正常、接近红线、突破红线或样本观察中。突破红线时直接显示“已阻止发布”，不再让管理员自己从多张统计卡猜测系统是否稳定。\n\nGoogle ADK 对话主链实行 100% 精确模型一致性：服务端持久化的当前模型设置是唯一权威，浏览器缓存或聊天请求携带的旧选择不得覆盖；Scope Supervisor、Root Coordinator、专家和 Evidence Auditor 必须全部使用该设置对应的同一服务商与模型。回答 JSON 使用本地强类型协议解析，不调用隐藏格式化模型。结构化采集只接受所属 Agent 最后一个通过协议校验的对象，ADK 临时 output、专家草稿或转交载荷不能覆盖有效正文。主模型超时、额度不足、鉴权失败、结果无效或能力不匹配时必须停止并报告，禁止启动 DeepSeek 或其他应急模型；Runtime 回报与设置不一致时，Worker 必须阻止答案发布。其他非 Agent AI 功能是否允许备用模型，以各自功能文档为准。\n\n获得本次费用授权后，正式发布才可通过 Cloudflare Versions 与 Deployments HTTP API 进行候选验证。候选版本必须通过健康接口、事实协议、版本号和静态资源一致性核对后才接管全部流量；失败时自动恢复上一稳定版本。数据库和 R2 不随代码回滚，迁移必须继续保持向后兼容。\n\n管理员可配置签名治理 Webhook 接收应急回退等运行事件。事件不包含问题、回答、任务、附件、金额或其他业务内容。完整规则见 `docs/AGENT_GOVERNANCE.md`。"
  },
  {
    "id": "docs/AGENT_PRODUCTION_OPERATIONS.md#11",
    "sourcePath": "docs/AGENT_PRODUCTION_OPERATIONS.md",
    "sourceLabel": "Agent 运营手册",
    "category": "运营手册",
    "heading": "安全原则",
    "content": "- 所有业务写入继续执行“预览 → 确认 → Workflow”。\n- 任务记忆可查看、可纠正、可清空、可停用。\n- 失败学习与质量指标不保存业务正文；生产力续接统计只保存匿名会话哈希，不保存原始会话 ID。\n- 模型调优建议只读，不自动改配置。\n- 正式任务数据、附件和结算永远以 D1 / R2 权威记录为准。"
  },
  {
    "id": "CHANGELOG.md#1",
    ...
    "category": "更新日志",
    "heading": "2026-08-12 · v0.39.0（Agent 认识这个网站了 · 领域地图）",
    "content": "这一版给 Agent 补的是**常识**，不是能力。\n\n- **问题**：你问「结算回单」——站内七大导航之一——Agent 把它当成一个不认识的对象名，拿任务解析器去模糊搜任务标题。它有专门的结算工具，但**决定去哪儿找的那一层压根不知道站内有「结算」这个域**，手里只有一把锤子。有些东西是要理解的，不是搜出来的。\n- **领域地图**：八个业务领域（工作台 / 任务 / 文件库 / 洞察 / 结算 / 收入 / 知识库 / 设置）现在写进 Agent 的常识：每个域是什么、用户会怎么称呼它（94 个别名）、里面有哪些对象和字段、该用哪个工具回答。命中领域就直接走对应工具，不再绕模糊搜索。\n- **它也知道自己读不到什么**：洞察页面自己生成的诊断记录、知识库里的笔记，都没有对应的读取工具。地图里写明了这一点，所以它会如实说\"读不到\"，而不是换着关键词一直搜，或者干脆编一个。\n- **字面命中兜底**：你把导航名说出口了（\"结算回单\"），即使模型漏判也会被补上。但只在唯一命中时兜底——命中多个说明问题本身跨域，那就交回给语义判断，字面匹配没资格覆盖它。\n- **地图是生成的，不是手抄的**：领域清单由导航定义（`AppView`）穷尽，工具由能力注册表约束，字段逐个回源核对，工具文案现取自注册表。改导航、改工具名、改字段名而地图没跟上，**构建直接失败**。新增只读工具没有领域认领它，也失败——必须要么归口，要么写明为什么不属于任何领域。这是为了不再出现\"三个月后地图和代码各说各话\"。\n- **反向也守**：地图只许指向只读工具。混进写工具的话，一个纯查询的问题可能触发业务写入。"
  },
  {
    "id": "CHANGELOG.md#2",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-08-12 · v0.38.0（换掉证据核对的承重墙 · 答完即答）",
    "content": "这一版不是补丁，是把答案核对的机制换掉。\n\n- **不再要求模型手抄证据编号**。原设计要模型把 `ev-cc1f894753e14a63` 这类哈希准确填进每条事实声明，再把整个闸门押在这件事上——这是能让模型做的最不可靠的事之一。线上真实案例：答案里的 07/28 23:09、¥3,293.75、14 个任务全都真实存在于证据中，却因为抄错了一个哈希被整段拦掉。\n- **改成内容对账**：直接拿答案里的日期、时间、金额、版本号、文件名、编号去证据里逐项核对。编造的值根本不会出现在证据里，躲不过去；而抄错编号不再影响判定，只记进审计。\n- **刻意不对账两类值**：派生值（\"距今 14 天\"是两个日期算出来的，证据里不会有这个数）和用户的猜测（问\"是 B09 还是 B10\"时，B10 只是候选不是事实）。对账它们会大量误拦，比原来更糟。\n- **答完即答**：事实值全部对上时直接输出，不再调语义审核。此前的尾部是——推理已经写出结论，系统还要把\"问题 + 候选答案 + 全部证据 + 核对报告\"打包再跑一次模型，15–25 秒后才决定发不发。这就是\"思考完了还要等很久\"的来源。\n- **闸门没有变松，判据变准了**：发布仍然要求确定性核对通过；只是核对的对象从\"模型声明的编号\"换成\"答案里的事实值\"。语义审核仍然保留，在值对不上（可能只是表述差异）或答案里没有可对账的值时介入。\n- **主体核对容许改写**：模型给主体起的概括标签常和用户措辞词序不同（\"结算回单导出记录\" vs \"导出结算回单\"），此前用子串匹配一比就判\"主体不存在\"。改成字符覆盖率。\n- **修掉一个上游误报**：对象解析工具此前只要全文搜索命中任何一条（哪怕只是用户自己的一条历史对话）就报告\"已识别\"，并把工作区里最新的那条无关记录当结果递回。问\"结算回单\"拿回硬封套版本信息就是这么来的。现在只有真正锁定对象才报告已识别，没锁定就不递回任何证据。"
  },
  {
    "id": "CHANGELOG.md#3",
    "sourcePath": "CHANGELOG.md",
    ...
    "category": "更新日志",
    "heading": "2026-08-12 · v0.37.10（修复上一版引入的回归）",
    "content": "- **修复\"未声明有效的执行边界\"**：v0.37.9 给回包新增了一个边界声明字段，但只加到了\"预算耗尽\"这条降级路径上，正常答完的回答漏了，于是被新加的校验自己拦死。这是上一版引入的回归，影响所有正常问答。\n- **补上真正能拦住这类问题的测试**：三道防线当时全都放过了——架构守卫只检查字段名在源码里存在（降级路径上有）、单元测试只覆盖降级路径、评测桩自己声明了这个字段所以评测也过。现在把 Worker 的校验逐条集中成一份契约，对四条真实路径（正常回答、需要澄清、审核未通过、预算耗尽）各跑一遍，并反向验证过：去掉字段测试立刻失败。\n- **报错不再出现框架名**：`Google ADK Runtime 未声明有效的执行边界：调用上限 39、单轮预算 缺失 秒` 这类文案会直接显示在对话里，既暴露了底层框架，又给出用户无法处理的内部数字。全部改为人话，技术细节进日志。"
  },
  {
    "id": "CHANGELOG.md#4",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-08-12 · v0.37.9（放开额度 · 推理面板不再是一堵墙）",
    "content": "- **调用额度实质放开**：检索类问题不会再因为\"额度用完\"而失败。次数上限从卡到刚好够用改成正常用不到的高位，只保留防止内部来回转交打转的作用。真正约束一轮规模的是时间——单轮 240 秒，而单次调用约 15–25 秒，一轮实际最多也就十几次，所以放开次数不会变成无限花钱。\n- **不设天花板才危险**：框架默认允许 500 次，真出现打转时会一直转到超时，让人白等四分钟拿到失败。所以保留高位开关，而不是完全取消。\n- **推理不再冒英文**：模型看到英文工具结果会切换语言，写出「Let me analyze the results carefully」这类内容。现在明确要求推理全程用中文——它是直接展示给用户的。\n- **推理不再是一堵墙**：推理文本此前整段塞进一个段落元素，换行被浏览器折叠，几百字挤成一坨。现在按段落渲染，各阶段（对象判断、分析与取证）单独标题。\n- **回答完成后自动收起推理**：跑的时候实时展开——那是等待期间反馈的全部意义；跑完自动折叠成一行「模型推理 · N 字，点击展开」，把位置让给真正有用的执行过程。要看细节点一下就有。\n- **Worker 的边界校验不再钉死数字**：改为校验\"Runtime 确实声明了有限边界、且收在 Worker 的 280 秒子请求上限之内\"。此前每次调整编排都要同步改一个魔数，任何不一致都会让整轮对话失败。"
  },
  {
    "id": "CHANGELOG.md#5",
    "sourcePath": "CHANGELOG.md",
    ...
    "category": "更新日志",
    "heading": "2026-08-12 · v0.37.8（预算耗尽不再报\"服务不可用\"）",
    "content": "- **修好\"半路显示不可用\"**：问一个工作区里没有精确匹配的对象时，界面会直接弹出 `Agent Runtime 暂时不可用：Max number of llm calls limit of 4 exceeded`。服务其实完好无损——是这一轮的模型调用预算用光了，而且结论其实已经查清（\"这本刊物在工作区里没有对应任务和附件\"），只是没预算写最后那份结论。\n- **调用预算不够用**：协调阶段原来只有 4 次，而一次正常的检索问答就要用掉 协调决策 + 专家检索 + 专家综合 + 协调成文 = 4 次，等于没有第二轮检索的余量。现在提到 6 次，单轮上限 9 → 11。\n- **用光预算改为软着陆**：不再抛异常，而是如实说明\"这次检索范围太大，没能收敛出结论\"，并告诉你怎么缩小范围，同时列出本轮已经检索过哪些数据。\n- **框架报错不再端给用户**：任何未预期的内部错误，界面只显示\"这一轮没能完成，请稍后再试一次\"；技术原因进日志与审计字段。此前是把异常原文直接渲染进聊天气泡。\n- **专家不再浪费调用**：覆盖面最广的搜索工具已经同时覆盖任务、附件和会话，命中后不再用窄范围工具重复确认同一件事；已取得的证据不重新取一遍；一轮没精确命中就如实汇报\"没有精确匹配 + 最接近的候选\"，不换关键词反复试探。"
  },
  {
    "id": "CHANGELOG.md#6",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-08-11 · v0.37.7（回答稳定性 · 审核不再一票否决正确答案）",
    "content": "- **修好\"有时对有时不对\"**：线上同一个问题连续三次是 拦 / 过 / 拦，三次的核心结论都正确，两次拒绝理由却完全不同。根因是证据审核员用默认温度运行，同一份输入时松时严；而它一说不通过，整段正确答案就被丢掉，你只能收到一句罐头拒答。\n- **审核员改为零温度**：它是判定器不是作者，不需要创造性。这一项直接消掉大部分随机波动。\n- **区分\"拦\"和\"提\"**：此前措辞偏好和事实错误混在同一个字段里，任何一条都能否决答案。现在只有四类会拦——答错了问题、主体不对、声明没有证据支持、\"最新\"混淆维度导致结论可能错；措辞、详略、排版、\"可以更严谨\"只作记录，永远不拦。\n- **审核不通过时给一轮修正机会**：把审核意见回喂给模型，让它只删改被指出的部分，修正稿**必须重新通过同一个审核员**才发布。此前的行为是整段扔掉。三次线上拦截里有两次都属于这一类，本可救回。\n- **闸门强度没有放松**：修正只有一轮、不得循环；修正稿没过审照样不发布；确定性校验不通过（引用了不存在的证据、主体绑不上）属于硬伤，不给修正机会。这三条都写进了架构守卫。\n- **单轮调用上限 7 → 9**：新增的两次（修正 1 次 + 复核 1 次）只在语义审核不通过时发生。治理文档已同步说明原因与边界。\n- **推理内容恢复可读**：协议字段名此前被统一替换成同一个\"内部流程\"，导致 `{\"entity_type\": ..., \"entity_id\": ...}` 变成两个同名字段，推理读不通。现在逐个给可读中文（对象类型、对象编号、已取得的证据等）。\n- **\"最新\"必须写明维度**：回答里出现\"最新/最后/当前\"时要写\"最新上传的版本是 X\"而不是\"最新版本是 X\"，避免读者自己从时间字段推断。\n- **不再断言内部处理状态**：附件分析进度、队列状态、`dead_letter`、重试次数这些既不是你问的，也几乎没有完整证据，一条多余断言就会让整个正确答案被拦。\n- **统一回答排版规范**：新增 `docs/AGENT_ANSWER_LAYOUT.md`。规范按数据形状分类（单值 / 2 项 / 3 项以上 / 有序步骤 / 并列要点 / 多维度冲突 / 层级 / 长说明），不按业务领域分类；取值格式与站内显示函数对齐（金额、工时、月份、日期）。"
  },
  {
    "id": "CHANGELOG.md#7",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-08-11 · v0.37.6（推理链落地 · 内部名词消毒 · 流式拼装修复）",
    "content": "- **思考链真的有内容了**：此前没有任何一处向模型申请推理输出。DeepSeek V4 属于混合推理模型，不显式打开开关就不返回推理内容，所以\"模型推理\"面板注定是空的，界面只能一直挂着\"等待模型返回推理内容\"。现在对象判断、主协调和各专家都会申请推理输出，推理内容逐字实时显示。\n- **拿不到推理时不再摆空壳**：Runtime 会在握手阶段如实声明本轮有没有申请推理。没有申请的模型不再显示\"等待模型返回推理内容\"占位符，界面只展示真实执行过程，不用占位符冒充进度。\n- **推理内容不再泄露技术细节**：模型的推理是自由文本，会照着提示词把接口名、内部角色和框架名念出来，而这条通道此前一个字符都没被检查过。现在 Runtime 出口会把它们换成自然语言（`search_attachments` → 「搜索任务附件」），界面再做一层兜底。模型自己写的意图判断同样会被消毒。\n- **修复随机出现的\"执行失败\"**：流式分片此前靠猜是增量还是全量，任何与上一片相同的分片都会被当成重复吞掉——JSON 里的 `\"\"` 和 `}}` 都会中招，丢一次字符后整轮结论就解析失败并报 502。现在按 ADK 的事件语义拼装：增量只拼接，段末聚合帧只覆盖本段，不再猜。（v0.37.4 记为\"正确合并 delta/cumulative\"的实现即为此处根因。）\n- **结论解析只在完整帧上进行**：不再对被截断的增量反复尝试解析，避免凑巧解析出一个字段缺失的\"合法\"结论被当成最终答案。\n- **审核阶段不申请推理**：证据审核是布尔闸门，它的推理里带着还没过闸的答案草稿，既不该展示给用户，也没必要为此多付推理 token。\n- **评测不再是空断言**：评测桩此前只下发干净的推理文案，界面消毒等于没被测过；现在桩会故意下发含接口名、专家名和框架名的推理，并新增\"不申请推理时不摆占位符\"的界面回归。\n- **费用边界不变**：单轮仍最多 7 次模型调用，全链路继续使用设置中选定的同一模型，无隐藏回退。注意推理输出本身会产生推理 token，同一问题的单轮费用比关闭推理时高。\n- **修复\"参与环节\"永远是空的**：委派专家用的工具名固定叫 `transfer_to_agent`，真正的专家名在参数里，此前只读工具名，所以编排记录里的参与环节和执行路径永远是空数组。\n- **澄清类回答不再多等一次模型调用**：结论没成形时（需要澄清或拒答），复核结论改变不了任何输出，那次调用是纯粹的等待与费用。现在直接跳过，这类回答快约三分之一、少一次调用。同时思考链不再显示\"未通过，已阻止未验证结论\"这种和澄清语义矛盾的说明。\n- **审核阶段彻底退出推理通道**：不只是不申请推理——即使供应商默认返回推理内容，审核阶段也不会再接入用户可见的推理面板。\n- **超时预算逐层收敛**：单轮总预算 240 秒（可配，硬上限 270），各阶段只能在剩余额度里取用。此前三个阶段各拿一份 150 秒，最坏能跑到 450 秒，而 Worker 280 秒就单方面掐断，用户白等四分多钟才看到失败。\n- **520 对所有调用方一起消失**：非 SSE 的对话调用（MCP、脚本、后台任务）此前仍走非流式请求，同样会被 Cloudflare 掐断合成 520。现在传输层只保留流式一条，没有接收端时在内部收完再整体返回。\n- **写文案、闲聊类请求不再被证据门误拦**：主体绑定校验此前对所有回答生效，而一次工具都没调过时证据集是空的，任何主体都判失败——用户会收到\"我已查到相关资料，但未通过校验\"，可系统根本没查过任何资料。现在这条只在本轮确实陈述业务事实（有工具证据或有事实声明）时才检查；一旦陈述事实而缺证据，照样拦。\n- **拦下来的说明不再说假话**：一次工具都没调过时，不再声称\"我已查到相关资料\"。\n- **\"Logo 3\" 不再被当成版本号**：版本识别此前接受任意 1–4 个字母加数字，于是 `Logo 3`、`AS 2`、`KV 1` 都会被视为版本结论，回答里出现而证据里没有原样字符串就会误拦。现在只认真正的版本前缀（v/ver/rev/rc/b）。\n- **候选不唯一时不再切断产品支持路线**：对象取证返回\"多个候选\"说明本身没定论，此前仍会把产品支持从可用范围里剥掉，导致真正在问 Giverny 自身的问题被强行送进工作区分析。现在只有确实解析到唯一工作区对象时才排除。\n- **修掉两处慢性内存泄漏**：会话闸门改为引用计数、用完即回收（此前每个新会话留一把锁，进程活多久攒多久）；模型适配器缓存加上 LRU 上限（此前 key 含模型、密钥摘要、角色与专家组合，只增不减）。Runtime 的 systemd 单元设了 `MemoryMax=1200M`，涨上去就是被 OOM kill。\n- **确定性校验的职责写清楚了**：主体一致性这条只拦\"主体是模型凭空造的\"（问题和回答里都不存在），语义上是否真正回应问题仍由独立的证据审核员判定，不用确定性规则去猜。"
  },
  {
    "id": "CHANGELOG.md#8",
    "sourcePath": "CHANGELOG.md",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-08-11 · v0.37.3（ADK Runtime 迁移 DMIT · Google 费用归零）",
    "content": "- **Google Cloud 不再承载对话 Runtime**：Google ADK 框架迁移到现有 DMIT VPS，以独立 systemd 用户运行；Google Cloud Run 与 Gemini 均不再是生产对话依赖。\n- **设置中的模型仍是唯一模型**：ADK 只负责语义编排，实际调用继续严格使用设置中选择的 DeepSeek 或其他服务商；所有角色共用同一精确模型，禁止隐藏 Gemini 与静默回退。\n- **VPN 完整隔离**：Runtime 独占 HTTPS 8443，Xray 的 443、订阅 2096、共享节点 24443 与 3x-ui 42989 保持不变；服务低优先级、单进程并设置 900MB 软限与 1.2GB 硬限。\n- **独立 TLS 与自动续期**：Runtime 使用 `179.253.249.92.sslip.io` 的 Let’s Encrypt 证书，证书续期只重启 ADK 服务，不重启 Xray 或 x-ui。\n- **无模型健康探针**：`/api/health?runtime=1` 只检查 Worker 到 ADK `/health` 的网络与模型策略声明，不提交用户问题、不调用 DeepSeek，发布验收不再靠真实模型请求试错。\n- **费用与调用上限继续锁定**：真实模型回归仍需单独审批；每个外层问题最多 7 次模型调用，Runtime 与 Worker 双端核验。"
  },
  {
    "id": "CHANGELOG.md#11",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-08-11 · v0.37.2（Agent 手册与生产主链对齐 · 思考过程可见）",
    "content": "- **等待不再变成报错**：编排要跑 60–150 秒，此前 Worker 用一次不产生任何字节的请求等结果，Cloudflare 会中途掐断并返回 520，云端已经算好的答案被丢掉。现在改为流式，空闲时持续发心跳，长耗时问答能正常拿到答案。\n- **思考过程是真的**：工作助手会边做边显示当前动作——先说明它判断你在问哪个对象和维度，再逐条显示正在读取任务详情、搜索任务附件等真实步骤，最后复核结论与证据，不再是一句静止不动的话。\n- **模型的推理逐字出现**：模型思考期间，它的推理内容会实时逐字显示，而不是等整段算完才蹦出来；即使某一步要想几十秒，界面也不会定住。推理内容与答案严格分开，答案仍然必须先通过证据校验才会出现，不会提前露出未核实的结论。\n- **不再有预设开场句**：思考链里不放任何写死的占位文案，第一行就是模型真实的判断；等待中的反馈由界面动效承担。\n- **不再暴露技术细节**：界面不会出现底层框架名、主链名称或 `search_attachments` 这类接口名；这些只留在管理员的执行审计里。\n- **同一句话不再显示两遍**：思考链展开时，摘要不再重复正文的最后一条；摘要预览只在你手动收起后出现。\n- **补上被测试放过的盲区**：评测桩此前秒回，真实的长耗时与流式路径完全没被覆盖；现在评测走与生产同一个流式端点，并新增两条界面回归用例。\n- **修正生产路由说明**：使用手册明确文本对话统一经过 Google ADK，不再把本机 CLI、Alice/LangGraph 旧编排或关键词策略写成当前主链。\n- **保留 CLI 连接边界**：「设置 → 本机 CLI」的配对、扫描和诊断能力继续保留，但连接状态不再被解释为生产对话正在使用 CLI。\n- **补齐 v0.37.1 用户说明**：手册新增 Scope Supervisor 主体理解、歧义澄清、《昂楷之道》B09 / B10 证据边界和 Evidence Auditor 双重验证。\n- **防止知识源再漂移**：Google ADK 架构守卫现在同时检查手册的主链描述、v0.37.1 条目和已知旧路由措辞，冲突内容会直接阻止构建。\n- **设置模型就是实际模型**：Google ADK 不再保留固定 Gemini；Worker 把设置中本轮选定的服务商、模型和 Base URL 传入 Runtime，语义主管、协调器、专家、回答整理与证据审核全部使用同一模型。Runtime 回报不一致时直接阻止答案，禁止暗中回退。\n- **费用审批成为项目红线**：真实模型回归、Cloud Run / Cloudflare 部署、扩容、批量重放或付费资源变更必须先说明最大调用量与预计费用并取得本轮明确批准；一般性的“继续”或“修复”不构成费用授权。\n- **单轮费用硬上限**：Google ADK 原生默认允许 500 次模型调用，生产链路现在将每个外层问题硬限制为最多 7 次，并由 Worker 核对 Runtime 声明；Cloud Run 固定 `min=0 / max=1`，避免空闲常驻和并发扩散。\n- **业务数据口径不变**：本轮不改金额、工时、验收日期或结算公式。"
  },
  {
    "id": "CHANGELOG.md#12",
    "sourcePath": "CHANGELOG.md",
    ...
    "category": "更新日志",
    "heading": "2026-08-10 · v0.37.1（Google ADK 语义 Agent 主链）",
    "content": "- **意图不再依赖关键词**：新增独立 Scope Supervisor 理解用户所问的真实主体、维度与操作意图，再动态限定 Root Coordinator 可见的专家；“版本”不再被默认理解为 Giverny 产品版本。\n- **《昂楷之道》回归用例修复**：查询 B09 / B10 时只允许工作区分析专家检索任务与附件证据，产品帮助、外网和本机 CLI 不再干扰；无法唯一绑定主体时明确请用户补充，禁止用相似项目代答。\n- **回答前双重验证**：Response Synthesizer 只能使用真实 evidenceId 组织结论，确定性校验再检查版本、主体和 claim-evidence 绑定，最后由独立 Evidence Auditor 决定发布、澄清或拒答。\n- **旧主链彻底隔离**：已配置 ADK 时，文本对话不再回落 LangGraph、Alice 旧编排、关键词事实策略或本机 CLI；ADK 无答案或异常时直接显式失败。\n- **安全写入边界保持**：模型只能生成 preview，签名执行工具对所有模型不可见；确认令牌仅由 Worker 的 D1 待确认状态保管，确认后才能执行并独立验收。\n- **浏览器回归可以连续跑完**：Playwright 关闭评测服务前改为先发 SIGTERM，隔离评测的清理逻辑因此真正执行；此前每组都会被强杀，在临时目录里各留下约 13MB 的 workerd D1/R2 残骸。\n- **分组之间不再抢端口**：每个「分组 × 重试」独占一对端口，重试轮次不会再撞上上一轮模型桩占用的端口。\n- **中断后不留残骸**：评测服务自带孤儿看护，Playwright 被强杀后会自行退出；分组启动器在每组结束后回收进程组、确认端口释放并清掉隔离目录，中断评测不再让下一组启动失败。"
  },
  {
    "id": "CHANGELOG.md#13",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-27 · v0.36.7（演示账号完整业务闭环）",
    "content": "- **demo 工作区权限补齐**：演示账号可完整操作自己工作区内的任务、进展、验收、附件和结算数据，包括作废、恢复、删除、回单锁定与分享管理。\n- **Excel 回单可正常导出**：结算页与 Agent 都可以使用 demo 虚构数据生成、预览和下载日期范围 Excel 回单。\n- **Agent 整月导出纠偏**：“请导出七月份的任务回单总结”等明确指令会直接生成 7 月 1 日至 7 月 31 日的回单，不再误查历史导出记录；同时保留“查询是否导出过”的只读语义。\n- **验收进展对齐正式站**：demo 可从任务列表、右键菜单、任务详情和右侧进展面板打开、编辑并确认验收进展。\n- **全站安全边界不变**：demo 始终固定在 `demo` 工作区；管理员密码、访问口令、AI Key / 模型路由、全站设置、工作区管理和基础设施权限仍仅管理员可用，不修改正式数据、金额、工时、验收日期或结算公式。"
  },
  {
    "id": "CHANGELOG.md#14",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-27 · v0.36.6（任务列表与选中态校正）",
    "content": "- **任务列表恢复原填充**：关闭「状态色」后恢复之前的中性浅底，不再整页使用浅绿色。\n- **导航选中态改为浅色**：主导航和账户入口复用文件库项目选中行的浅色矩形背景，不再使用深绿色实底。\n- **保留紧凑导航结构**：图标尺寸、图标与文字间距、左对齐和去除左侧竖线保持不变。\n- **业务数据口径不变**：本轮仅校正视觉样式，不修改金额、工时、验收日期、实际完成日期或结算公式。"
  },
  {
    "id": "CHANGELOG.md#15",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-27 · v0.36.5（导航与任务明细视觉收束）",
    "content": "- **导航更紧凑**：主导航图标统一缩小为 16px，图标与文字左对齐并保持 9px 间距，不再让图标抢过文字焦点。\n- **选中态改为整块填充**：移除主导航、账户入口和移动端导航的左侧竖线，选中项统一使用主题绿色矩形填充。\n- **逾期提示去描边**：逾期提醒保留整块浅色背景与文字层级，移除左侧强调竖线。\n- **任务明细关闭状态色更统一**：关闭「状态色」后，任务行统一使用与「知识科普」一致的浅绿色填充，不再出现灰暗的任务列表。\n- **业务数据口径不变**：本轮仅调整导航与任务列表视觉，不修改金额、工时、验收日期、实际完成日期或结算公式。"
  },
  {
    "id": "CHANGELOG.md#16",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-27 · v0.36.4（演示工作区全链路隔离）",
    "content": "- **Agent 只读演示数据**：结算查询、回单导出、需求人画像、后台分析和任务创建全部绑定 `demo` 工作区；演示单价统一为 ¥260，不再误用正式账号单价导致页面与 Agent 金额不一致。\n- **AI 学习样本隔离**：任务命名、需求整理、工时建议、采用偏好、样本反馈和偏差复盘均按工作区读取；演示空间无样本时返回空结果，禁止回退正式空间历史。\n- **新建任务彻底隔离**：演示账号使用独立岗位分类、草稿缓存和企业任务提示词，不再出现「展会类 / 品牌类」、正式人员、正式项目示例或平面设计专属分析。\n- **回单身份自然呈现**：演示回单改为「Giverny 结算回单 / 专业服务」，删除「多岗位协作」等解释性措辞；在线预览、分享页和 Excel 使用同一口径。\n- **日历恢复分类色彩**：岗位大类与子类都能命中对应颜色，产品、运营、数据、AI、HR、开发、UI 和影视任务不再全部回退为紫色。\n- **演示工时样本补齐**：为八类岗位各补充两条 4–5 月已验收历史任务，每类至少三条可追溯样本；样本不进入 6–7 月回单，不改变当前演示结算金额。\n- **启动缓存防串号**：本地秒开快照同时校验登录角色和工作区，管理员缓存不能在演示账号下短暂闪现；正式工作区金额、工时、验收和结算公式均未修改。\n- **业务路由白屏修复**：工作台、任务、文件、洞察、收入、结算和设置等叶子路由直接懒加载应用入口，不再因空 `<Outlet>` 渲染为白屏。"
  },
  {
    "id": "CHANGELOG.md#17",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-27 · v0.36.3（演示账号简化登录）",
    "content": "- **只保留一套密码**：演示账号固定为 `demo@mayeai.com`，登录时直接验证当前管理员密码，不再保存独立演示密码。\n- **自动跟随管理员密码**：管理员密码修改后，演示账号无需任何同步操作，立即使用新密码，同时注销旧演示会话。\n- **删除多余配置**：移除演示密码设置面板、额外修改接口、密码脚本和旧密码哈希，登录页仍然只有账号与密码。"
  },
  {
    "id": "CHANGELOG.md#18",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-27 · v0.36.2（演示账号密码自主管理）",
    "content": "- **管理员可自行修改**：在「设置 → 权限安全 → 账号安全」新增演示账号密码面板，不再需要通过脚本修改。\n- **可与管理员密码同步**：默认勾选「与当前管理员密码保持一致」，验证当前管理员密码后即可同步；也可取消勾选并单独设置演示密码。\n- **安全生效**：密码仅以 PBKDF2 哈希保存，更新后自动注销旧演示会话，不影响管理员当前登录和任何真实业务数据。"
  },
  {
    "id": "CHANGELOG.md#19",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-27 · v0.36.1（演示账号完整生产力）",
    "content": "- **演示不再公开进入**：删除登录页“一键进入演示”和公开演示接口；游客继续只看到空的安全花园，必须使用独立演示账号登录。\n- **演示账号可以真正操作**：新增受限 `demo` 角色，固定在 `demo` 工作区，可以新建和修改任务、记录进展、上传附件并触发正式附件分析；不能进入真实工作区、管理口令或修改全站模型与系统设置。\n- **工作助手纳入演示**：演示账号可打开 Giverny Agent，使用业务查询、任务草稿、确认执行、附件接力和后台分析；沿用站点首选模型但不能修改全站模型配置。\n- **交付件理解直接可看**：11 个演示附件全部预置完成态分析，覆盖摘要、提取内容、关键发现、需求匹配、风险和建议；不再显示“等待分析”。\n- **新上传附件自动分析**：演示账号上传交付件后沿用正式队列创建分析任务，前端持续轮询处理状态，完成后直接展示交付件理解。\n- **真实数据继续隔离**：游客、演示任务、演示 Agent、附件分析和写入操作均绑定独立工作区；没有修改真实金额、工时、验收日期、完成日期或结算公式。"
  },
  {
    "id": "CHANGELOG.md#20",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-27 · v0.36.0（独立演示空间）",
    "content": "- **一键进入演示**：登录页新增“进入演示”，服务端创建 HttpOnly 只读会话，不公开共用密码；演示账号不能创建、修改、删除、上传、验收、结算或调整设置。\n- **真实数据与演示数据物理隔离**：建立独立 `demo` 工作区，任务、进展、附件、月报、访问口令、R2 路径和向量检索均按 `workspace_id` 隔离；切换会话时先清空旧内存数据，杜绝真实内容短暂闪现。\n- **未登录不再读取真实工作区**：匿名访问只得到空的安全花园并自动打开登录页，不再沿用旧版游客读取 `default` 工作区的行为；公开合作伙伴回单仍通过独立分享链接访问。\n- **八类岗位虚构任务**：新增 16 条覆盖产品经理、用户运营、数据分析、AI 研究员、HR、开发、UI 设计和影视剪辑的完整任务，包含计划中、进行中、挂起、待验收、已验收、分段工时、等待与验收记录。\n- **可真实打开的演示附件**：生成 4 份 Excel、2 张原创 UI 高保真图、视频分镜与封面、产品 PRD、运营方案、RAG 评测 CSV 和开发说明；附件仅来自演示静态资产，不复用任何真实文件。\n- **隔离测试进入质量门禁**：幂等初始化测试会执行两次演示 Seed，并验证真实空间任务与金额哨兵不变、附件齐全、访问口令保留工作区、R2 路径带租户前缀、向量检索不跨工作区。\n- **真实业务口径不变**：没有修改真实工作区的金额、实际工时、验收日期、实际完成日期、补录映射或结算公式。"
  },
  {
    "id": "CHANGELOG.md#21",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-27 · v0.35.6（登录花园背景降噪）",
    "content": "- **背景视觉强度降低 50%**：在登录花园图上增加半透明纸张层，保留睡莲与水面轮廓，同时避免背景抢过登录面板。\n- **局部微风同步减半**：柳枝和花园边缘动态叠层的不透明度从 68% 调整为 34%，避免两侧动态重新变重。\n- **业务数据口径不变**：本轮没有修改金额、实际工时、验收日期、实际完成日期、补录映射或结算公式。"
  },
  {
    "id": "CHANGELOG.md#22",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-27 · v0.35.5（登录花园微风动画优化）",
    "content": "- **微风不再推动整座花园**：移除整幅背景持续平移和放大的镜头式动画，底图保持稳定，避免登录页产生生硬的缩放感。\n- **局部枝叶自然轻摆**：使用柔和蒙版只让两侧柳枝和花园边缘低幅移动，并采用不等距关键帧打破机械往返节奏。\n- **水波与光影独立呼吸**：水波改为更慢的渐入、扩散和消失，光影只做低幅亮度变化；移动端继续完整覆盖，减少动态效果设置仍然有效。\n- **业务数据口径不变**：本轮没有修改金额、实际工时、验收日期、实际完成日期、补录映射或结算公式。"
  },
  {
    "id": "CHANGELOG.md#23",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-27 · v0.35.4（站内品牌故事入口）",
    "content": "- **补齐“关于 Giverny”入口**：账户菜单新增所有角色均可查看的品牌入口，不再要求用户前往 GitHub 或使用手册理解产品名称。\n- **一屏讲清品牌来源**：复用站内轻量弹窗，说明吉维尼与莫奈、《睡莲》四季色彩、产品理念和品牌 Slogan；仅保留关闭符号，不增加多余操作。\n- **业务数据口径不变**：本轮没有修改金额、实际工时、验收日期、实际完成日期、补录映射或结算公式。"
  },
  {
    "id": "CHANGELOG.md#24",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-26 · v0.35.3（Giverny 品牌体验全链路）",
    "content": "- **统一品牌状态语言**：任务空状态、无进展、进行中、待验收和已验收使用同一套“花园、水面、色彩与创作生长”语义，同时保留任务、工时和金额的直接业务表达。\n- **吉维尼模式更容易理解**：设置页明确说明色彩来自莫奈《睡莲》，并补齐春季浅绿、夏季清水青、秋季暖色和冬季雾蓝的四季灵感。\n- **爱丽丝进入同一品牌世界**：工作助手欢迎页增加轻量睡莲水面动画与创作花园文案；任务项目、历史和执行结果仍保持清晰的工具语言。\n- **对外回单统一落款**：线上回单、合作伙伴分享页、Excel、PDF 和密码重置邮件统一 Giverny 品牌头与“让创作在自己的花园里生长”落款。\n- **品牌资产一致性**：统一 Logo、PNG/SVG favicon、加载页、错误页和网页元信息；旧星光 favicon 已替换为 Giverny 水面字母图形。\n- **可访问性与性能保持**：登录页、加载页和欢迎页动画响应系统“减少动态效果”设置；登录花园背景继续使用压缩后的 WebP。\n- **业务数据口径不变**：本轮没有修改金额、实际工时、验收日期、实际完成日期、补录映射或结算公式。"
  },
  {
    "id": "CHANGELOG.md#25",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-26 · v0.35.2（Giverny 品牌花园登录体验）",
    "content": "- **整页进入莫奈花园**：管理员登录不再只有弹窗顶部的抽象色斑，整层背景使用原创睡莲花园场景，并加入明显的水面波纹、光影移动和景深呼吸动画。\n- **动画不妨碍登录**：登录表单保持稳定清晰，系统开启“减少动态效果”时会自动停用背景动画；背景资源压缩为 WebP，避免登录入口加载过重。\n- **品牌设计文档公开**：README 顶部增加“品牌设计”入口，独立文档说明 Giverny、莫奈《睡莲》、四季主题、状态语言和品牌设计原则。\n- **业务数据口径不变**：本轮没有修改金额、实际工时、验收日期、实际完成日期、补录映射或结算公式。"
  },
  {
    "id": "CHANGELOG.md#26",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-26 · v0.35.1（Agent 流式反馈前移）",
    "content": "- **提问后立即显示进度**：SSE 通道建立后先发送真实的“正在判断需要哪些信息”，不再让用户只看到空光标。\n- **等待过程更有反馈**：云端 Agent 进入实际编排时显示“正在编排”；只有确实走本机 CLI 时才显示连接 Giverny MCP，避免固定模板冒充执行过程。\n- **减少首屏等待**：Token 统计改为异步 Promise，不再阻塞首条流式反馈；增加代理缓冲关闭标记，降低中间层延迟整段输出的概率。\n- **架构守卫**：新增流式反馈门禁，防止未来把路由或统计重新放回首条反馈之前。\n- **业务数据口径不变**：本轮没有修改金额、实际工时、验收日期、实际完成日期、补录映射或结算公式。"
  },
  {
    "id": "CHANGELOG.md#27",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-26 · v0.35.0（Agent 生产可靠性基础设施）",
    "content": "- **附件分析不再只有“自动重试”**：主队列失败超过上限后进入 Cloudflare Queues DLQ，D1 记录死信证据、失败次数和时间；用户可通过现有人工重试入口重新入队，前端和 Agent 都能明确显示“等待人工重试”。\n- **线上基础设施纳入发布门禁**：新增 `infra:sync` / `infra:check`，通过 Cloudflare HTTP API 幂等创建和校准队列、消费者、DLQ 与原生 Tracing；正式发布前自动同步，配置漂移会阻止发布。\n- **Agent 具备原生链路观测**：使用 Cloudflare Workers Tracing 记录理解规划、工具执行、结果组织和附件分析四个业务阶段；只记录角色、操作数量、节点路径和验真状态，不记录问题、回答、任务名称、金额、附件名、API Key 或 Token。\n- **LangGraph 状态可跨请求复盘**：Alice Durable Object 以影子检查点保存规划前后节点路径、操作名、循环次数和事实验真状态，保留最近 200 条；不取代 D1、确认凭证或 Workflow 权威状态，避免双状态源。\n- **模型忠实度保持不变**：用户选择的主模型仍先执行同模型重试；只有明确超时、限额、鉴权、供应商故障或能力不匹配等极端情况才允许备用模型。Dynamic Routing 暂不接管 BYOK 主模型，避免破坏未来租户隔离。\n- **部署链路修复**：统一 Cloudflare API helper，修复静态资源上传地址在抽取后可能失效的问题；永久禁止 Wrangler 的约束继续有效。\n- **业务数据口径不变**：本轮没有修改金额、实际工时、验收日期、实际完成日期、补录映射或结算公式。"
  },
  {
    "id": "CHANGELOG.md#28",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-26 · v0.34.2（Agent 基础对话与事实回答重构）",
    "content": "- **不再把事实清单当成最终回答**：业务工具负责读取，程序负责校验，用户选择的主模型再结合最近对话自然回答；简单问题不再倾倒整页任务明细。\n- **上下文追问回到原业务语境**：“为什么刚才金额不对”“所以重新核对后是什么”会继续核对结算记录和快照，不会因为“为什么”误入产品手册或更新日志。\n- **修复结算导出金额显示为零**：Agent 导出记录同时统一 `amount` / `totalAmount` 契约；正式 D1 中 2026-06-01 至 2026-07-22 快照的数据库汇总、快照汇总和逐行小计均为 ¥9,109.45。\n- **金额质疑必须复算**：先定位最近导出记录，再用对应 `exportId` 逐行核对工时、单价、小计、快照汇总和数据库记录，不能把导出列表中的展示值直接当成最终结论。\n- **思考过程改为用户语言**：等待时先显示轻量动态反馈，随后逐项展示“思考、动作、结果”；LangGraph 节点、工具英文名、协议名称和模型调用次数只保留在管理员审计。\n- **联网查询进入统一编排**：新增受控 `search_web` 能力，天气、新闻和实时公开信息不再错误检索 Giverny 产品知识；站内数据、产品资料和外部网络三类来源严格隔离。\n- **所有业务写入继续强制确认**：任何模型都只能获得当前角色允许的预览能力；签名执行端点不暴露给模型，仍需角色校验、确认卡、签名凭证、Workflow 幂等执行和写后验收。\n- **业务数据口径不变**：本轮修复 Agent 读取字段和回答链路，没有修改任务金额、工时、验收日期、完成日期、补录映射或结算公式。"
  },
  {
    "id": "CHANGELOG.md#29",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-26 · v0.34.1（Agent 生产力结果度量）",
    "content": "- **不再把接口未报错当成工作完成**：设置页明确拆分“接口成功率”和“目标完成率”，只有 LangGraph 生产力闭环以 `complete` 结束才计入真正完成。\n- **首轮完成与补查能力可量化**：新增首轮完成率、补查恢复率、完成 / 待补充 / 失败数量、平均闭环轮次和平均工具调用次数，能够定位是一次做成、补查后完成，还是最终失败。\n- **用户补充后是否解决可追踪**：用匿名会话哈希关联同一对话中的后续轮次，统计“待补充后解决率”，避免 Agent 总让用户补资料却从不把事情做完。\n- **执行审计更接近生产结果**：近期记录直接展示已完成、待补充、失败、闭环轮次、工具次数、确定性证据和终止原因，不再只展示底层请求或验真状态。\n- **隐私边界保持严格**：只保存截断 SHA-256 会话哈希和结构化状态，不保存原始会话 ID、用户问题、回答正文、任务标题或操作草稿；自动化评测继续排除在真实运营指标之外。\n- **质量门禁防止指标退化**：新增确定性指标测试和架构守卫，强制检查 D1 迁移、Worker 持久化、API 契约、设置页展示和正文零存储。\n- **业务数据口径不变**：本轮没有修改金额、实际工时、验收日期、完成日期、补录映射或结算公式。"
  },
  {
    "id": "CHANGELOG.md#30",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-26 · v0.34.0（Agent 生产力执行闭环）",
    "content": "- **Agent 不再停在“选出工具”**：新增第二张 LangGraph 生产力状态图，将首次执行、工具观察、业务验真、补充规划、再执行和终止判定收敛为统一闭环。\n- **删除 Alice 手写三轮补查循环**：架构守卫禁止恢复旧循环；以后执行次数、状态转移和终止条件由 LangGraph 统一管理。\n- **每轮必须有明确结果**：工具任务只能以 `complete`、`needs_input` 或 `failed` 结束，禁止卡在“正在重规划”或工具返回后直接宣称完成。\n- **执行预算成为强制边界**：单轮最多 3 轮、8 次工具调用；超限立即失败停止，防止无限重试、工具滥用和电脑资源失控。\n- **补规划不再为每个工具手写参数分支**：对支持空参数或 schema 默认值的只读能力，观察节点可自动恢复；需要明确对象且无法安全推导时，转为等待用户补充。\n- **防虚构边界扩展到全部预览写入**：不只是新建任务，任何 `confirmation=preview` 的参数都必须提供用户原话依据，避免把“查询长期偏好”误执行为“修改企业记忆”。\n- **生产力可观测**：API 返回 `productivity.status/path/cycles/toolCalls/reason`，可以定位计划正确但执行未完成、证据不足或预算耗尽等具体问题。\n- **质量门禁升级**：所有工具型隔离评测都强制检查 LangGraph 终止状态与轮次/工具预算；Agent 评测 `155/155`、桌面浏览器 `42/42`、移动端 `42/42` 通过。\n- **业务数据口径不变**：本轮没有修改金额、实际工时、验收日期、完成日期、补录映射或结算公式。"
  },
  {
    "id": "CHANGELOG.md#31",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-26 · v0.33.9（Agent 生产力主链转向 LangGraph）",
    "content": "- **不再把“选对工具”当成 Agent 已经会工作**：正式引入 `@langchain/langgraph`，用回合状态图接管理解、能力缩选、直接授权、复杂规划与失败回规划。\n- **明确指令进入单模型快速路径**：“帮我新建一个任务”现在只需一次主模型调用就进入创建预览；复合目标、先查后做、对象不明确或直接方案被拒绝时才调用 Planner。\n- **产品知识不再污染普通业务命令**：任务、进展、验收、财务与附件请求默认禁止产品手册、快捷键、更新日志和全域搜索；只有真正的产品使用问题才开放知识工具。\n- **防止模型替用户虚构任务字段**：单次直接创建的参数必须附带用户原话依据；没有依据的标题、需求、日期或工时会被剔除，由后端返回真实缺失字段，不会提前生成确认卡。\n- **评测改为验收生产力**：新增主模型调用预算、无关检索、缺失字段、禁止虚假确认卡和真实页面反馈门禁；隔离 Agent 评测 `155/155`，桌面浏览器 `42/42`，移动端 `42/42` 通过。\n- **保留 Cloudflare 生产边界**：Cloudflare Agents 继续管理 Durable Object 会话，D1/R2 继续提供权威数据，Workflow 继续负责预览、人工确认、幂等执行和写后验收；未引入 LangSmith Server。\n- **业务数据口径不变**：本轮没有修改金额、实际工时、验收日期、完成日期、补录映射或结算公式。"
  },
  {
    "id": "CHANGELOG.md#32",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-26 · v0.33.8（Agent 主链替换：意图导演与小工具集编排）",
    "content": "- **放弃固定模型面对全量工具的旧方案**：Alice 不再内置 DeepSeek、不再通过 `toolChoice:auto` 在 82 项能力中自由猜测；只保留持久会话、任务引用、确认状态和已校验计划执行。\n- **用户选择的主模型成为真正编排大脑**：主模型先在无工具、无知识上下文中理解整句话，再只针对 0-16 项候选能力制定最小计划；同模型重试与极端故障备用策略保持不变。\n- **产品知识默认彻底隔离**：只有快捷键、使用方法、设置、更新日志、品牌故事和产品规则等问题才能获得 `search_product_help`；普通任务、写入、财务和附件请求即使模型误选，也会被策略层拒绝。\n- **不再为每句新问法打关键词补丁**：新增独立 Intent Director、能力缩选、Zod 参数校验、角色裁剪、知识资格校验和计划二次授权；工具失败会进入验真与重规划，不再让整轮直接崩溃。\n- **多步业务链可以使用前序工具结果**：“先定位最近任务，再读取反馈”等计划会将第一步的真实 taskId 绑定到下一步；结构化附件参数统一使用 POST，全域搜索先提取业务主体并进行 LIKE 安全处理。\n- **结算回单成为正式的即时产物能力**：删除 Agent 内部重复的“导出预览 → 再确认生成”链路；主模型识别到明确日期范围后直接生成未锁定、可审计的 Excel 与线上预览结果卡，不修改任务、工时、金额或验收数据。锁定、删除和分享权限变更仍需人工确认。\n- **处理轨迹来自真实事件**：移除“理解问题 → 制定计划”固定模板和未分类前的关键词假轨迹；界面现在逐步展示 Director 决策、实际工具、缺失字段与验真结果。\n- **质量门禁完整通过**：新增导演层单测与架构守卫，编排确定性断言增至 60 条；隔离 Agent 评测 `155/155`、桌面浏览器 `41/41`、移动端浏览器 `41/41` 全部通过。\n- **业务数据口径不变**：本轮没有修改金额、实际工时、验收日期、完成日期、补录映射或结算公式。"
  },
  {
    "id": "CHANGELOG.md#33",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-26 · v0.33.7（Agent 改进 9～10：对话回归与长期效果评估）",
    "content": "- **真实多轮回归扩展**：从 6 组基础引用扩展到 14 组、28 轮，覆盖任务指代、等待原因、确认卡修改、附件证据、财务复核、正式审计报告、品牌知识和权限边界。\n- **失败指纹不能再假装覆盖**：8 类匿名失败指纹分别绑定固定会话用例；没有回归用例的指纹不能标记“已覆盖”，通过后同时记录回归编号和最后验证版本。\n- **四项长期效果进入运行中心**：新增确认式任务完成率、人工修正率、确定性验真质量和保守估算节省时间，所有指标由程序计算，不由模型自评。\n- **按版本持续观察**：Agent 运行记录写入应用版本，每日保存 30 天滚动快照，可对比不同版本的运行量、任务完成、验真质量和节省时间。\n- **避免小样本误导**：至少积累 30 天、100 次真实运行后才允许形成长期版本结论；不足时明确展示观察天数和样本数。\n- **隐私与业务口径不变**：效果事件只保存匿名类型、主体、版本和结果，不保存问题、回答或确认草稿；本轮没有修改金额、实际工时、验收日期、补录归属、实际完成日期或结算公式。"
  },
  {
    "id": "CHANGELOG.md#34",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-26 · v0.33.6（Agent 改进 6～8：一致性审计、正式交付物与高风险治理）",
    "content": "- **全站一致性审计正式落地**：可手动、交付前或每日定时核查任务状态/日期、活动等待、保存工时与分段汇总、附件/R2、附件分析归属和结算快照结构；每次运行保存审计编号、差异证据与校验值。\n- **审计绝不擅自修数据**：工时差异只作为提示，保存的实际工时继续作为财务锚点；系统不会借审计自动修改金额、工时、验收日期、附件或结算快照。\n- **正式报告绑定权威快照**：新增项目状态报告、验收报告和一致性审计报告；确认前自动审计，确认期间来源变化会拒绝旧凭证。\n- **交付物可追溯可下载**：每份报告保存不可变来源快照、审计运行编号和内容校验值，并提供 HTML 在线查看、PDF 下载、JSON 证据下载与历史查询。\n- **敏感操作统一进入风险案件**：所有 `sensitive` 预览自动记录风险等级、操作主体、不可变证据校验值与保留期限，不再依靠零散警告文案。\n- **高风险操作需要双重确认**：第一次确认只记录用户已阅读风险，第二次确认才批准 Workflow 写入；执行前仍可撤销，已执行操作不会伪装成可以随意回滚。\n- **证据至少保留七年**：高风险案件持续记录待确认、已知情、已执行、失败和已撤销状态；执行前撤销同样保留原因和证据链。\n- **能力与质量门扩展**：统一能力达到 82 项，MCP 读取工具 22 项，签名写入 23 类；Agent 隔离评测 `138/138` 通过。\n- **业务数据口径不变**：本轮没有修改任何金额、实际工时、验收日期、补录归属、实际完成日期或结算公式。"
  },
  {
    "id": "CHANGELOG.md#35",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-26 · v0.33.5（Agent 改进 3～5：计划编辑、主动续接与全域搜索）",
    "content": "- **执行计划可以安全调整**：Alice 可修订尚未执行的步骤、依赖和补偿定义；已经运行、完成、失败、跳过或进入补偿的步骤及其证据保持不可改写。\n- **计划修订继续强制确认**：修订先生成完整未来步骤确认卡，凭证绑定计划 revision、状态和全部步骤快照；确认期间任何会话推进计划，旧凭证都会被拒绝。\n- **“继续”会回到真实停顿点**：Agent 区分待批次确认、暂停、失败、正在运行、下一步可执行和依赖阻塞，不再把“继续推进”当作普通关键词回复。\n- **下一步仍受业务安全链约束**：续接只提出对应的确认动作，实际任务写入仍必须经过业务预览、人工确认、Workflow、审计和写入后独立验收。\n- **全站资料统一检索**：新增任务、附件、正式对话、产品知识和企业记忆五域搜索，合并向量、关键词及结构化结果，并为每条结果保留来源和关联任务。\n- **权限与租户边界保持严格**：全域搜索绑定当前工作区，合作伙伴角色不能读取内部对话和企业记忆；跨租户向量结果和显式数据均被过滤。\n- **能力注册表扩展**：统一能力达到 74 项，MCP 读取工具 19 项；架构守卫、确定性测试和隔离回归覆盖三项能力，防止后续退化。\n- **业务数据口径不变**：本轮没有修改任何金额、实际工时、验收日期、补录归属、实际完成日期或结算公式。"
  },
  {
    "id": "CHANGELOG.md#36",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.33.4（Agent 改进 2：多操作事务与批量执行）",
    "content": "- **多个任务操作可以一次确认**：Alice 可将 2–20 个修改字段、修改状态、记录反馈、追加进展和记录等待操作组成一张批量确认卡，不再要求用户连续确认多张单操作卡。\n- **D1 同域写入全有或全无**：批次在同一个 D1 `batch` 事务中执行，包含确认前快照检查、任务更新、操作审计和批次记录；任一语句失败时整个事务回滚。\n- **并发变化不会覆盖新数据**：预览保存每个任务的完整可变快照指纹；确认前任何任务被其他页面或会话修改，整批操作都会拒绝并标记为已回滚。\n- **批次结果继续独立验收**：提交后重新读取所有受影响任务和 `agent_operation_batches`；只有任务字段、操作数和任务数全部一致，Alice 才会宣称完成。\n- **跨域高风险操作不伪装事务**：结算、完整验收、附件、模型配置等操作仍保持独立确认；需要跨域编排时继续使用可恢复计划和显式补偿，不承诺无法保证的原子性。\n- **工时与金额保护**：改联系人、改字段、改状态、记反馈或记录等待不会触发实际工时重算；只有明确追加计时进展时才按既有规则更新工时。本轮没有修改金额、日期映射或结算公式。"
  },
  {
    "id": "CHANGELOG.md#37",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.33.3（Agent 改进 1：写入后独立验收）",
    "content": "- **接口成功不再等于业务完成**：20 类确认式写入在接口返回后，都会从当前工作区的 D1 权威数据重新读取并逐项核对。\n- **验收失败会立即停住**：Alice 只有在独立验收通过后才会说“已完成”并推进执行计划；核对失败时改为失败状态，列出未通过项并阻断后续步骤。\n- **验收结果可恢复可追溯**：结构化 `postcondition` 与首次写入结果一起保存；同一 Workflow 重放返回原始验收结果，不重复写入也不伪造二次验收。\n- **新能力默认失败关闭**：架构门禁自动核对统一注册表中的全部签名写入端点；未接入独立验收的新端点不能通过发布门禁。\n- **隔离回归扩展**：任务、反馈、状态、字段、进展、等待、验收、结算、附件、计划、提醒、模型路由、主动事项和企业记忆写入均验证 D1 独立验收，并检查幂等重放不改变验收快照。\n- **业务数据口径不变**：本轮没有修改任何金额、实际工时、验收日期、补录归属、实际完成日期或结算公式。"
  },
  {
    "id": "CHANGELOG.md#38",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.33.2（首批能力 5：安全设置 Agent 完成）",
    "content": "- **主备模型可以一次查清**：新增整条模型路由诊断，统一检查当前首选模型、文字/识图四路主备配置、凭证来源、连接结果和近期真实回退原因，不再逐条试错或猜测“可能额度不足”。\n- **主模型优先成为确定性规则**：诊断明确展示主模型目标 99%、一般故障必须由同一模型先尝试 2 次、用户取消绝不触发备用模型，并优先要求修复主模型问题。\n- **配置确认防止覆盖新状态**：模型路由预览绑定修改前的完整脱敏快照；确认期间其他会话修改任一路由或首选模型，旧凭证立即拒绝。\n- **首选模型精确绑定**：配置时选择“设为当前模型”会绑定到本次配置的具体 route，不再模糊切换到该服务商的默认模型。\n- **支持恢复上一份配置**：每次 Agent 修改前自动保存不含密钥的历史快照；恢复前检查目标链路，确认后由 Workflow 恢复，并在恢复前再留一份快照。\n- **密钥边界保持封闭**：API Key 不接受模型输入，不进入历史、确认卡、诊断结果或审计正文；新增、替换和删除 Key 仍只允许在设置页完成。\n- **能力注册表扩展**：统一能力达到 70 项，预览与签名执行各 20 项，MCP 只读工具仍为 17 项。\n- **业务数据口径不变**：本轮没有修改任何金额、实际工时、验收日期、补录归属、实际完成日期或结算公式。"
  },
  {
    "id": "CHANGELOG.md#39",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.33.1（首批能力 4：项目执行 Agent 完成）",
    "content": "- **项目执行状态可直接询问**：工作助手现在可以读取已保存执行计划，准确回答“做到哪一步、下一步是什么、哪个步骤失败、为什么被阻塞”，并展示真实依赖关系与进度计数。\n- **项目上下文一起核对**：执行概览会关联任务当前状态、活动等待、站内提醒和主动风险，不再只返回一张脱离业务现场的步骤清单。\n- **计划管理进入确认式执行**：暂停、恢复、失败步骤重试和取消均先生成预览，用户确认后由 Workflow 持久执行，并记录审计日志。\n- **确认期间变化会被拒绝**：确认凭证绑定工作区、操作者、计划 revision、状态、更新时间和步骤快照；跨租户访问、旧版本操作和凭证重放均被拒绝。\n- **禁止 Agent 伪造完成**：模型工具不提供“完成步骤”能力，真实业务步骤只有在对应业务写入成功后才能推进；取消计划也不会回滚已经落库的业务数据。\n- **能力注册表扩展**：统一能力达到 67 项，MCP 只读工具 17 项，预览与签名执行各 19 项。\n- **业务数据口径不变**：本轮没有修改任何金额、实际工时、验收日期、补录归属、实际完成日期或结算公式。"
  },
  {
    "id": "CHANGELOG.md#40",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.33.0（首批能力 3：日程 Agent（Agenda）完成）",
    "content": "- **日程 Agent 从核心可用升级为完整 Agenda**：可以回答今天、本周或任意 63 天范围内有哪些任务、哪些今天到期、哪些已经逾期、哪些正在等待，以及有哪些站内提醒。\n- **可用时间由程序计算**：支持指定所需时长、工作开始/结束时间、15/30/60 分钟步长、是否包含周末和排除某个任务；当天已经过去的时间不会再作为候选。\n- **复用网站现有日历口径**：有明确开始时刻的任务按“开始时间 + 预估工时”占用时间块；跨日任务作为持续计划展示，不会粗暴地把开始到交付之间的全部时间判为忙碌。\n- **每日负载与冲突可解释**：返回每日忙碌分钟、可用分钟、开始任务、到期任务、提醒和时间块冲突，并保留具体任务 ID、等待原因和对接信息。\n- **排期确认防止覆盖新状态**：预览绑定任务开始、交付、预估工时、状态、结算月份和更新时间；同时绑定相关冲突快照。确认期间任何一侧变化，旧凭证立即拒绝。\n- **提醒边界补齐**：不能创建已经过去的提醒；确认后执行时任务已删除或提醒时间已过，同样拒绝写入。\n- **意图识别不再只看“安排”关键词**：询问“本周有哪些安排”进入 Agenda；“安排下周做三张海报”仍进入新建任务确认，不会误路由。\n- **权限与租户隔离**：Agenda 和排期冲突只对内部工作角色与只读 MCP 开放，不向合作伙伴角色暴露整个工作区日程；跨工作区任务和提醒不能互相读取。\n- **质量门完整通过**：统一能力注册表扩展到 64 项、MCP 只读工具 16 项、能力断言 807 条；Agent 隔离评测 130/130，桌面与移动端浏览器回归各 40/40。\n- **业务数据口径不变**：本轮没有修改任何金额、实际工时、验收日期、补录归属、实际完成日期或结算公式。"
  },
  {
    "id": "CHANGELOG.md#41",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.99（首批能力 1：结算 Agent 完成）",
    "content": "- **结算 Agent 从核心可用升级为完整闭环**：既能生成新回单，也能查询已有回单并直接交付在线预览、合作伙伴分享、Excel 和 PDF 四类入口。\n- **新增确定性结算核对**：程序逐行复算任务数、实际工时、单价、小计和总金额，识别重复任务、行小计错误、快照汇总与数据库记录不一致，不让模型自行心算财务数据。\n- **快照变化可以追溯**：核对已保存回单时，会比较当前业务数据，指出新出现但未进入快照的任务、已退出当前范围的任务，并返回任务 ID、快照校验值、生成时间和锁定状态。\n- **日期覆盖不再靠人工翻记录**：自动识别历史导出范围的日期重叠、未覆盖空档、锁定边界，以及日期和内容完全相同的重复回单。\n- **未锁定记录可安全删除**：Agent 先展示删除预览，人工确认后通过现有 Workflow 删除；确认期间只要快照、锁定或链接状态变化，旧凭证立即失效。\n- **锁定回单继续严格保护**：Agent 不接收管理员密码，也不能删除锁定记录；这类删除只能回到管理界面验证管理员登录密码。\n- **租户、权限和审计同步加固**：结算核对只对财务读取角色开放，写操作只对管理员开放；跨工作区 exportId 返回 404，核对与删除均写入审计记录。\n- **质量门完整通过**：能力注册表扩展到 63 项、MCP 只读工具 15 项；确定性测试 796 条断言、Agent 隔离评测 127/127、桌面 40/40、移动端 40/40 全部通过。\n- **工时与金额不变**：本轮没有修改任何任务金额、工时、验收日期、补录归属、实际完成日期或结算计算公式。"
  },
  {
    "id": "CHANGELOG.md#42",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.98（企业级路线 10：集成、SLO 与治理完成）",
    "content": "- **优先路线 1～10 全部完成**：第 10 项把已有统计升级为可执行生产保护，运行中心统一判定回答成功率、事实验真率、P95 响应、主模型完成率和后台任务稳定性。\n- **主模型 99% 目标成为硬规则**：文字、结构化 JSON 和图片链路都先重试同一模型；只有连续超时、额度限制、供应商故障、鉴权故障、连续无效结果或能力明确不匹配才允许应急回退。\n- **不合规回退会被发现**：用户取消、只失败一次或原因不明不能触发备用模型；每次回退记录故障类别、尝试次数和模型，违反规则会生成生产告警。\n- **发布不再直接切换 100% 流量**：Cloudflare HTTP API 先上传独立候选版本，定向验证健康接口、事实协议、版本号、入口资源和资源可访问性，通过后再正式推广。\n- **候选失败自动恢复**：任一验证失败立即把上一稳定版本恢复为 100% 流量，并阻止后续 GitHub 发布闭环；全流程继续永久禁用 Wrangler。\n- **外部运行告警有安全边界**：新增 HMAC-SHA256 签名 Webhook 连接器，只发送版本、模型、故障类别和时间，不发送用户问题、回答、任务、附件或金额。\n- **管理员可以看懂结果**：设置页新增“生产保护”区，直接说明当前是否允许发布、主模型回退是否合规、错误预算剩余多少以及自动回滚是否启用。\n- **工时与金额不变**：本轮不修改任务金额、工时、验收日期、补录归属、实际完成日期或结算计算口径。"
  },
  {
    "id": "CHANGELOG.md#43",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.97（企业级路线 9：企业知识与记忆系统完成）",
    "content": "- **优先路线第 9 项完成**：新增组织规则、合作伙伴记忆和项目记忆三层长期上下文；任务记忆、个人知识笔记、产品知识和企业记忆保持明确边界。\n- **每条记忆都有可核验来源**：保存来源类型、引用、说明、原文摘要、人工确认状态、有效期、创建人和更新时间；Agent 回答时同步展示来源、版本和有效期。\n- **纠正不再覆盖历史**：旧版本进入“已被纠正”，新版本递增版本号并关联上一版本；失效和软删除退出默认查询，但修订与审计仍保留。\n- **写入继续强制确认**：新增、纠正、失效和删除均支持 Agent 预览、签名确认、Workflow 幂等执行和确认前并发冲突拒绝，模型推测不能直接成为企业事实。\n- **任务中心可直接管理**：新增“企业记忆”页签，可按组织、合作伙伴和项目筛选，查看来源与期限，并用统一站内交互维护记录。\n- **知识数据完成租户隔离**：企业记忆、修订历史和原个人知识笔记全部按工作区过滤；合作伙伴与访客无法读取内部企业记忆。\n- **能力治理同步扩展**：统一能力注册表增至 62 项，MCP 只读工具 14 项，Workflow 确认写入 18 类；新增企业记忆架构、事实、编排、隔离和浏览器回归。\n- **工时与金额不变**：本轮不修改任何任务金额、工时、验收日期、补录归属、实际完成日期或结算计算口径。"
  },
  {
    "id": "CHANGELOG.md#44",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.96（企业级路线 8：主动工作能力完成）",
    "content": "- **优先路线第 8 项完成**：普通每日提醒升级为独立主动事项系统，任务创建、修改和附件上传后即时检测，Cron 继续承担漏扫与恢复兜底。\n- **六类风险进入确定性队列**：覆盖逾期、今天/明天到期、进度 100% 未验收、实际工时超预估 25%、活动等待和验收备注缺口；每条事项带任务事实证据、四级优先级和可执行建议。\n- **去重不再只比较文案**：相同任务与信号只保留一条活动事项；用户处理后，任务源数据未发生变化时不会立即重建；风险条件消失后自动标记解决，新事件发生后才允许复发。\n- **处理闭环可量化**：任务中心新增主动事项页，可执行建议、查看任务、24 小时后提醒、标记已解决或无需处理；汇总待处理、高风险、解决率、忽略率和平均响应时间。\n- **Agent 可查也可确认处理**：新增主动事项确定性查询工具，以及解决/忽略/稍后处理的预览与 Workflow 执行工具；内部风险队列不向合作伙伴角色开放。\n- **低负载开发成为永久纪律**：项目排除 Spotlight 索引，本机 Node 检查限制内存并使用低调度优先级；开发阶段只跑定向测试，完整隔离与浏览器回归仅在发布前串行执行。\n- **能力治理同步扩展**：统一能力注册表增至 59 项，MCP 只读工具 13 项，Workflow 确认写入 17 类，并新增主动工作规则、架构和隔离回归。\n- **工时与金额不变**：本轮不修改任何任务金额、工时、验收日期、补录归属、实际完成日期或结算计算口径。"
  },
  {
    "id": "CHANGELOG.md#45",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.95（企业级路线 7：文件与多模态工作流完成）",
    "content": "- **优先路线第 7 项完成**：统一能力注册表从 50 项扩展到 56 项，文件 Agent 从搜索和上传接力升级为可读取证据、查询分析状态、失败恢复和安全维护元数据的完整工作流。\n- **附件结论必须带证据**：新增附件证据工具，统一返回任务关联、文件元数据、OCR / 提取文字、分析结论和稳定 `[attachment:ID:*]` 引用；Agent 不再只根据文件名猜内容。\n- **PDF、OCR 与 XLSX 可结构化理解**：复用现有后台分析链路；XLSX 解析保留工作表、单元格位置、共享字符串、数值和公式，避免压平 XML 后丢失表格关系。\n- **失败分析可以恢复**：先查询 missing / pending / processing / completed / failed / unsupported 状态，再对缺失或失败分析生成确认草稿；处理中附件拒绝重复入队，执行继续走 Workflow 与幂等记录。\n- **附件维护可确认执行**：支持确认后修改名称、标签、范围和合作伙伴可见性；预览绑定旧状态，确认前发生变化即拒绝旧凭证，真实文件扩展名不可伪造，结算锁定月份不可修改。\n- **合作伙伴可见性统一收口**：附件搜索、任务详情和证据读取共用后端权限过滤；未明确 `visible_to_client` 的内部附件不会通过 Agent、模糊搜索或任务详情泄露给合作伙伴。\n- **浏览器继续安全直传**：大文件二进制仍从已登录浏览器直接进入 R2，不经过模型上下文；模型只接收受控分析证据，API Key 从不进入附件工具参数或结果。\n- **质量门扩展**：能力注册表达到 711 条确定性断言，新增多模态工具测试与架构守卫；隔离 D1/R2/Workflow 验证证据引用、客户隔离、失败重试、并发冲突、扩展名保护和 XLSX 单元格提取，Agent `124/124` 全部通过。\n- **工时与金额不变**：本轮不修改任何任务金额、工时、验收日期、补录归属、实际完成日期或结算计算口径。"
  },
  {
    "id": "CHANGELOG.md#46",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.94（企业级路线 6：业务操作工具完成）",
    "content": "- **优先路线第 6 项完成**：统一能力注册表从 35 项扩展到 50 项，正式补齐结算、排期、附件上传接力、站内提醒和安全模型设置 15 项能力。\n- **结算 Agent 可真正交付文件**：管理员可查询导出记录，按任意日期范围生成确定性结算快照、Excel 和合作伙伴分享链接，并确认后锁定快照或调整链接有效期；不开放 Agent 删除结算记录。\n- **结算确认防脏写**：预览绑定完整回单快照校验值；任务数据在用户确认前发生变化时，旧凭证直接拒绝执行，避免下载内容与确认卡不一致。\n- **日程与提醒进入执行链路**：可查询未闭环任务的排期重叠，确认后调整开始时间、交付时间和预估工时；可创建与任务关联、跨会话恢复的站内定时提醒。\n- **附件上传走浏览器安全接力**：无任务编号的文件可由 Agent 先解析任务归属并返回上传接力；浏览器核对文件清单后直接上传 R2，PDF/Office 等文件二进制与 API Key 不进入模型上下文，明确要求识图的图片继续走受控视觉模型。\n- **模型设置可查、可测、可确认配置**：管理员可读取脱敏后的主备文字/识图路由，测试已安全保存凭证对应的模型，并确认后修改服务商、Base URL 和模型；聊天工具不接受明文 API Key。\n- **权限与 Workflow 全覆盖**：结算和模型设置仅管理员可用；协作者可排期、提醒和上传，viewer 保持只读；五组真实写入全部执行“预览 → 签名确认 → Workflow → 审计”。\n- **质量门扩展**：能力注册表达到 637 条确定性断言，新增业务工具测试与架构守卫；隔离 D1/R2/Workflow 验证 Excel/分享页、快照变更拒绝、排期落库、提醒持久化、密钥脱敏、角色越权和凭证重放，原 Agent `120/120` 继续通过。\n- **工时与金额不变**：本轮不修改任何任务金额、工时、验收日期、补录归属、实际完成日期或结算计算口径。"
  },
  {
    "id": "CHANGELOG.md#47",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.93（企业级路线 5：多步骤任务执行引擎完成）",
    "content": "- **优先路线第 5 项完成**：持续计划从文字步骤清单升级为 D1 持久化执行批次，支持步骤依赖、批次确认、失败停止、重试、暂停/恢复、补偿/回滚编排和跨会话恢复。\n- **依赖图确定性校验**：每个步骤具有稳定 key 和 `dependsOn`；不存在的依赖、自依赖及循环依赖在创建时直接拒绝，根步骤与并行分支按完成结果自动解锁。\n- **批次确认不越权**：新执行批次先进入待确认状态，未确认时任何业务写入都不能推进计划；确认整个批次后，每个真实业务修改仍继续执行“预览 → 确认 → Workflow”。\n- **Workflow 结果回写状态机**：写入开始、成功和失败均回写对应步骤；任一步失败立即停止批次并阻断其他待执行分支，处理后可从失败步骤重试。\n- **暂停与跨会话恢复**：暂停期间拒绝步骤推进；D1 保存步骤状态、尝试次数、失败原因、确认时间和 revision，关闭页面或换会话后继续从原状态恢复。\n- **防止旧会话覆盖**：任务中心更新携带乐观并发 revision，旧窗口操作较早快照时返回冲突，不会覆盖另一会话已经推进的计划。\n- **补偿按反向依赖执行**：完成步骤可以声明补偿动作，下游先补偿、上游后补偿；补偿动作必须已进入统一能力注册表和安全确认链路，不接受模型凭空发明的回滚接口。\n- **任务中心状态可见**：计划卡展示待批次确认、等待依赖、可执行、执行中、失败、补偿中和已补偿状态，并提供批次确认、暂停、重试及补偿入口。\n- **质量门新增执行引擎层**：新增 29 条状态机断言、独立架构守卫及真实 D1/API 隔离流程，覆盖未确认拒绝、依赖解锁、失败停止、重试、暂停/恢复、revision 冲突、下游重开和反向补偿。\n- **工时与金额不变**：本轮只建设 Agent 执行编排，不修改任何任务工时、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#48",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.92（企业级路线 4：权限与租户安全加固完成）",
    "content": "- **优先路线第 4 项完成**：跨租户显式 ID、标题模糊搜索、附件元数据与路径、匿名枚举、提示注入和确认凭证攻击全部进入隔离发布门禁。\n- **确认凭证绑定租户与主体**：第二版 `confirmationToken` 同时签名 action、draft、workspaceId、principalId、jti 和过期时间；错工作区、错主体、换动作、篡改、过期和重放全部拒绝。\n- **Workflow 身份不再丢失**：持久写入使用系统执行角色，但保留原工作区和发起主体；内部二次调用继续携带签名 scope，非默认工作区不会错误写入默认租户。\n- **附件读取边界统一**：源文件和预览共用同一权限函数；匿名用户不能猜附件 ID，内部工具密钥不再构成跨租户豁免，无权访问统一返回不可枚举的 404。\n- **分享链接绑定工作区**：月报和结算分享 token 必须与附件任务处于同一工作区；结算链接还会检查禁用、过期及快照任务关联。\n- **模型工具按角色裁剪**：Alice 只向模型提供当前角色可使用的能力；viewer、client 与 guest 不会看到写入预览，签名执行和内部 Workflow 工具永远不暴露给模型。\n- **提示注入进入不可信边界**：用户消息、任务字段、附件文字、工具结果和附加上下文均不能改变系统权限与确认规则；伪造角色标记、控制字符和上下文闭合标记会被清理。\n- **签名比较加固**：Agent scope 从普通字符串比较升级为 Web Crypto HMAC verify，工作区、主体、角色、runId 或签名任一字段变化都会失败。\n- **质量门新增安全层**：新增 66 条确定性安全断言、架构守卫和真实多租户攻击矩阵；原 Agent 120/120、重启恢复和金额口径保持通过。\n- **工时与金额不变**：本轮只加固权限和租户安全，不修改任何任务工时、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#49",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.91（企业级路线 3：统一 Agent 能力注册表完成）",
    "content": "- **优先路线第 3 项完成**：35 项读取、计划、记忆、后台分析、写入预览、签名执行和内部 Workflow 能力全部进入统一注册表，不再只登记读取工具。\n- **工具契约成为单一来源**：每项能力统一声明输入 schema、HTTP 端点、允许角色、scope、风险等级、确认方式、审计事件、事实来源、任务作用域、轨迹文案和 Runtime 暴露面。\n- **Alice 不再维护旁路定义**：模型工具的 schema、说明、预览与执行配对、任务级判断和思考进度文案均读取注册表；新增能力无需再复制多份常量。\n- **权限与 Workflow 白名单自动派生**：Worker 按注册表中的 method 与 role 授权；九个签名写入端点自动组成 Workflow 白名单，未知端点和只读角色写入继续默认拒绝。\n- **MCP 与 OpenAPI 自动生成**：MCP 从注册表批量挂载八个只读工具；OpenAPI 从 Zod schema 生成全部能力路径，并携带风险、确认和审计策略元数据。\n- **审计事件统一**：任务写入、持续计划和后台分析启动均使用注册表登记的审计事件，避免代码执行成功但审计名称遗漏或分叉。\n- **能力文档与质量门自动化**：新增自动生成的能力清单、架构守卫和 452 条确定性断言；隔离评测增加真实 OpenAPI 清单、策略一致性及角色越权拒绝检查。\n- **工时与金额不变**：本轮只改造 Agent 工具治理，不修改任何任务工时、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#50",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.90（企业级路线 2：无 Wrangler 全链路质量门完成）",
    "content": "- **优先路线第 2 项完成**：Agent 隔离评测、浏览器回归和 GitHub Actions 全部移除 Wrangler 可执行链路，企业路线图正式标记完成。\n- **统一 Miniflare 隔离运行时**：直接使用 Cloudflare workerd 模拟真实 Worker，每次评测独立创建 D1、R2、SQLite Durable Object、Workflow 和静态资源绑定。\n- **Agent 与浏览器共用环境工厂**：两类评测共用 `isolated-runtime`，仅保留各自的用例与端口编排，避免环境口径分叉。\n- **真实写入后重启恢复**：评测创建真实任务并写入 R2 附件，重启整个 workerd 后再核对 D1 任务、R2 文件、登录会话和 operationId 幂等重放。\n- **失败恢复与回收成为必测项**：保留 Workflow 写入、重放、后台任务取消/失败重试；临时存储在正常、异常、信号中断和进程退出时都有清理路径。\n- **架构守卫禁止回退**：扫描 Agent、浏览器、CI 可执行源和质量门命令，旧配置文件、旧辅助脚本或 Wrangler 调用回流时直接阻止发布。\n- **正式发布路径不变**：生产 Worker 仍仅使用 Cloudflare HTTP API Direct Upload，凭证自动刷新，不存在任何 CLI 回退。\n- **工时与金额不变**：本轮只改造评测与发布质量门，不修改任何任务工时、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#51",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.89（企业级路线 1：最终答案逐字段验真完成）",
    "content": "- **优先路线第 1 项完成**：最终答案逐字段验真从“核心保护”升级为统一结构化事实协议，并在企业路线图中正式标记完成。\n- **两条 Agent 链路完全统一**：AliceAgent 与 Worker 兼容聊天、本机 CLI 回接共用同一个事实快照、权威渲染和校验器，不再存在新 Agent 严格、旧链路宽松的问题。\n- **模型不再自由改写业务事实**：模型负责理解和工具规划；最终业务正文由程序依据工具证据生成，完整保留任务名称、文件名、等待原因、金额、工时、日期、状态和数量。\n- **全部业务读取能力已覆盖**：财务、任务搜索、跨任务概况、任务详情、需求人画像、附件、产品说明、能力边界、结算回单、任务记忆和持续计划均有确定性回答区块。\n- **答案必须覆盖全部来源**：事实快照新增带 `sourceTool + path` 的结构化声明；复合问题缺少任一工具事实区块即判定不完整。\n- **旧阻塞兜底纳入协议**：模型规划失败后的任务等待查询也使用统一事实快照，不再维护一套独立文字模板。\n- **生产在线验收成为发布门禁**：正式健康接口运行合成事实自检，验证正确摘要通过、错误工时被拒绝和多来源完整覆盖；部署命令上传后自动执行线上验收。\n- **HTTP 发布凭证可持续续期**：Giverny 独立发布器在每次发布前自动刷新 Cloudflare OAuth 凭证并原子保存，不再因短期令牌过期反复出现 `401`，也不读取 Wrangler 配置路径。\n- **质量门加固**：最终事实保护增至 45 条断言，并覆盖旧 Worker 数据结构、结算回单、任务记忆、持续计划和来源完整性。\n- **工时与金额不变**：本轮只完成 Agent 回答协议，不修改任何任务工时、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#52",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.88（Agent 最终答案关键事实保护）",
    "content": "- **工具成功后仍必须验真正文**：业务问题不再直接采用主模型第一次生成的答案；无论工具是否一次调用齐全，最终正文都必须重新基于确定性工具证据整理。\n- **关键事实由程序逐字段核对**：新增 `agentFactGuard`，保护金额、工时、百分比、日期、状态、任务 ID 和任务/附件数量，模型写出证据中不存在的值时立即拦截。\n- **错误正文不再交给用户**：事实校验失败或主模型二次整理失败时，系统直接使用程序从 D1/R2 证据生成的权威摘要，不恢复使用未经验证的初稿。\n- **复杂任务保留完整验真轨迹**：取消 Agent 轨迹前 10 项截断，事实纠正和确定性回退不会因为步骤较多而从用户界面消失。\n- **企业级路线进入仓库治理**：新增 `AGENT_ENTERPRISE_ROADMAP.md`，持续记录优先路线 1～10 与首批能力 1～6 的状态，后续每次发布同步更新。\n- **质量门加固**：新增 29 条最终事实保护断言和独立架构守卫，直接覆盖阿拉伯数字与中文数字形式的“5 小时写成 3 小时”“1500 元写成 1200 元”、错误日期、状态、任务 ID、附件数量与画像百分比。\n- **工时与金额不变**：本轮只新增回答验真，不修改任何任务工时、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#53",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.87（Agent 复合目标参数作用域）",
    "content": "- **工具不再接收整段混合问题**：新增目标分句器，按标点以及“再、同时、另外、然后”等连接词拆开用户的一句话，每个补查工具只获取与自己目标相关的片段。\n- **姓名提取去除动作前缀**：“分析陈义君的合作画像”现在稳定提取为“陈义君”，不会再把“分析”带进需求人查询参数。\n- **任务标题去除附件与后续要求**：“打开公司产品封套修改的验收附件，并告诉我任务状态”会提取任务标题“公司产品封套修改”，不再夹带附件名称或第二个目标。\n- **财务、产品、任务、附件分别查询**：复合问题中的金额片段、跨任务片段、网站使用片段和附件片段会分别路由，减少工具查空、误匹配和无关结果。\n- **架构守卫防止回退**：新增分句、工具作用域、姓名清洗和任务标题清洗契约；确定性编排断言增至 40 条。\n- **工时与金额不变**：本轮只调整 Agent 补查参数和实体解析，未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#54",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.86（Agent 复合目标拆解与逐项验真）",
    "content": "- **一句话可以包含多个任务**：Agent 现在会同时识别财务、任务、附件、需求人画像、产品说明和写入目标，不再把整句压成一个意图后遗漏其余要求。\n- **每个目标分别验真**：例如“查金额、列延期任务、说明怎么下载回单”必须同时取得财务计算、跨任务聚合和产品知识证据，任何一项都不能用另一项的成功结果代替。\n- **一次动态补齐全部缺口**：编排层合并各目标缺失的只读工具，跳过已有成功证据，最多三轮重试，避免重复查询。\n- **最终答案必须逐项完整回应**：结果整理器会收到目标清单，只能基于确定性证据逐项回答；某一项缺数据时明确指出，不再悄悄省略。\n- **写入和查询可以并行编排**：同一句既要求修改任务又查询当前状态时，写入仍进入人工确认，查询仍读取任务详情。\n- **质量门加固**：确定性编排测试增至 33 条；新增 6 条复合目标评测，完整评测清单达到 108 条单轮用例和 6 组多轮会话；跨任务聚合成功时不再重复执行通用任务搜索。\n- **工时与金额不变**：本轮只调整 Agent 目标拆解、验真和评测，未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#55",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.85（Agent 通用动态重规划）",
    "content": "- **主 Agent 不再只有“卡点补查”特例**：统一动态重规划覆盖金额工时、产品说明、需求人画像、附件、单任务与跨任务问题；模型漏调必需工具时，编排层自动补查。\n- **意图不再由工具选择反推**：新增独立 `inferAgentIntent`，先根据用户整句话区分查数据、问产品、查附件或要写入；即使主模型没调工具，编排层仍然知道必须查什么。\n- **最多三轮补查与重新验真**：工具缺失、首次失败或证据被降级时，Agent 生成补查计划、执行必需只读工具、重新验真；同一工具三次仍失败才停止。\n- **补查后必须重写最终答案**：编排层只把通过验真的工具证据交给主模型第二次整理；第二次失败时不恢复使用旧的未验证初稿。\n- **实体参数由纯程序提取**：新增 `agentEntityResolver`，从“陈义君的用户画像”和“公司产品封套修改目前做到哪”等自然句式中提取需求人姓名与任务标题，避免把整句话当作查询参数。\n- **失败后成功可正确收敛**：同名工具首次失败、后续补查成功时，验真器以最终成功的确定性证据为准，不再被早期失败状态永久卡住。\n- **质量门继续加固**：新增 21 条编排器确定性断言和 6 条“不要查 / 直接猜”反工具规避问法；Agent 评测达到 102 条单轮用例与 6 组多轮会话。\n- **工时与金额不变**：本轮只调整 Agent 意图、只读补查、验真与评测，未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#56",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-25 · v0.32.84（Agent 多轮任务引用与证据一致性）",
    "content": "- **选中任务成为会话状态**：用户点击任务消歧卡或工具唯一定位任务后，AliceAgent 在该会话 Durable Object 中保存 `taskId + title`，关闭再打开仍能延续正确上下文。\n- **指代语自动绑定已选任务**：后续说“这个 / 那个 / 刚才 / 上述 / 当前 / 继续 / 它”时，编排层为详情、记忆、反馈、状态、字段、进展、等待、记录维护、验收和持续计划自动补入 taskId，不再让模型重新猜标题。\n- **工具返回任务 ID 必须可核对**：任务型工具没有返回 taskId，或返回 ID 与请求 ID 不一致时，证据自动降级、计划标记失败并停止采用，避免“拿错项目但回答很流畅”。\n- **任务事实内部一致性校验**：已验收 / 终止 / 不计费任务不得同时存在活动等待；活动等待必须返回原因 / 备注与开始时间，否则不能当作确定性结论。\n- **MCP 补齐跨任务工具**：`query_task_portfolio` 加入 MCP 只读工具列表；MCP 内部调用改用结构化 POST，保留状态数组等复合参数，仍受签名工作区限制。\n- **真正的多轮质量门**：新增 6 组、12 轮会话评测，覆盖详情、显式点选、字段修改、进展、持续计划和任务记忆；评测会检查后续工具实际使用的 taskId，不只检查工具名。\n- **工时与金额不变**：本轮只调整 Agent 会话状态、只读验真、MCP 工具和评测，未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#57",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.83（Agent 跨任务业务查询补强）",
    "content": "- **新增确定性跨任务工具**：`query_task_portfolio` 在 Worker 后台按日期、状态、需求人、对接人和设计类型聚合当前工作区任务，不再让模型用标题关键词猜整体情况。\n- **延期与等待原因可一次查全**：工具直接返回未完成、逾期天数、正在等待的原因 / 备注 / 开始时间 / 已等待时长，以及需求人、对接人和最近进展。\n- **主 Agent 与兼容链路统一**：Cloudflare AliceAgent、旧聊天规划器、OpenAPI 和工具权限表共享同一个后台聚合结果，避免换一个入口就换一套数据口径。\n- **编排层强制验真**：当问题包含“哪些 / 所有 / 全部”与延期、等待、未完成、负责人或汇总意图时，没有跨任务工具证据就不得完成回答，会自动补查。\n- **Agent 评测增至 96 条**：新增 8 种跨任务问法，覆盖延期清单、等待原因、日期范围、需求人、对接人、状态概况和最近进展；新增架构守卫防止任一入口脱节。\n- **租户与数据边界不变**：查询强制使用签名工作区上下文，不返回金额、不开放写入，也不会跨工作区读取任务。\n- **工时与金额不变**：本轮只新增 Agent 只读查询、编排规则和评测，未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#58",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.82（全站空状态系统统一）",
    "content": "- **47 处空状态全部收口**：工作台、任务、文件库、洞察、收入、知识库、设置、工作助手与各类弹窗统一复用 `EmptyState`，不再各自维护“暂无数据”布局。\n- **四级密度适配不同场景**：新增 `feature / panel / compact / inline` 四种密度，从页面主空状态到窄侧栏行内提示都能保持统一层级。\n- **睡莲视觉只服务主空状态**：页面级无内容场景增加轻量睡莲意象，使用主题 token、无阴影和轻动效；系统减少动效时自动停止动画。\n- **加载与无数据语义分离**：后端读取、模型获取和分析等加载过程改用独立 `loading-state`，不再冒充空状态。\n- **架构守卫防止反弹**：新增全量 TSX 与 `src/styles` CSS 扫描，拦截旧空状态类、手写无数据文案、密度层级缺失和减少动效支持回退。\n- **桌面与窄屏完成视觉复核**：空状态与现有纸张表面、4px 圆角和移动端布局保持一致，未增加裸色值、描边堆叠或新的重型依赖。\n- **工时与金额不变**：本轮只调整空状态 UI、样式与质量守卫，未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#59",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.81（生产前端监控完整闭环）",
    "content": "- **异常采集覆盖完整**：在 React 渲染、全局脚本和 Promise 基础上，补齐脚本 / 样式资源失败、动态分包加载失败、API 网络异常、响应解析异常与 5xx；离线状态和正常 4xx 不制造噪声。\n- **真实用户性能正式落库**：使用浏览器原生 `PerformanceObserver` 采集 TTFB、FCP、LCP、INP、CLS 和页面加载耗时，不引入新的重型监控 SDK；同一页面会话重复上报时原位更新。\n- **版本与慢路由可定位**：运行与质量中心展示最近周期的 P75、良好率、最新版本和慢路由；前端错误展开后可查看脱敏后的脚本栈与 React 组件栈。\n- **前端告警接入统一治理**：错误达到 5 / 20 次、核心体验 P75 进入较差区间、慢体验会话达到 25% 时自动生成 warning / critical 告警，并沿用管理员确认与解决流程。\n- **隐私和保留策略固定**：上报严格同源、限制 8/16KB，不保存输入、任务正文、对话、附件、查询参数或外部完整 URL；错误聚合保留 90 天，性能会话保留 30 天，继续按工作区隔离。\n- **Cloudflare 与 D1 双层可观测**：Workers Logs 保持开启并采样 10% 请求；浏览器 RUM 使用第一方 D1，无需公开分析令牌或会话回放服务。\n- **无 Wrangler 的 migration 链路**：新增 Cloudflare D1 HTTP API SQL 执行器，`0029` 性能表与版本 / 路由索引已应用正式 D1；新增监控架构守卫防止采集、保留、告警或 UI 链路回退。\n- **工时与金额不变**：本轮只新增运行监控、性能数据和运维展示，未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#60",
    "sourcePath": "CHANGELOG.md",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.79（正式路由树与路由级懒加载完成）",
    "content": "- **全站统一进入正式路由树**：`createBrowserRouter` 与单一 `RouterProvider` 同时管理后台、普通报告分享页和结算分享页，入口不再用 `window.location.pathname.match` 手动分流。\n- **公开分享页真正按路由加载**：`/share/:token` 与 `/settlement-share/:token` 各自拥有独立懒加载入口，访问后台时不再同步打包和执行两个公开页面模块。\n- **后台视图由路由元数据驱动**：工作台、任务、文件库、洞察、收入、结算、设置和知识库统一在 `router.tsx` 注册；`App.tsx` 不再维护 `viewFromPath`、反向路径表或未知地址纠正副作用，并降至 896 行。\n- **深链接和历史记录补齐**：根路径、旧 `/updates` 和未知地址使用声明式重定向；任务列表 / 日历切换写入浏览器历史，前进与后退能准确恢复对应视图。\n- **新增路由架构与浏览器守卫**：质量门阻止入口重新引入手写 pathname 正则、同步加载公开页或在 `App` 中恢复 URL 映射；桌面和移动回归覆盖重定向、未知地址、深链接及历史导航。\n- **正式发布永久移除 Wrangler**：新增 Cloudflare 官方 HTTP API Direct Upload 发布器，自动上传 Worker 模块与静态资源并继承线上绑定；项目规则和部署文档明确禁止回退 Wrangler，`package.json` 已删除 Wrangler 部署命令。\n- **本地回归移除系统代理干扰**：浏览器与 Agent 隔离评测子进程明确只走本机回环地址，不再继承系统代理变量，减少本地代理链路导致的异常退出。\n- **工时与金额不变**：本轮只调整前端路由和测试启动链路，未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#62",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.78（全站 CSS 业务域模块化完成）",
    "content": "- **巨型 CSS 单文件完成拆分**：原 26,725 行 `App.css` 缩减为 13 行有序入口，521KB 样式规则迁入 `src/styles/` 的 13 个业务域模块。\n- **样式按产品边界归档**：设计 token、应用壳、工作台任务、任务管理、文件预览、通用弹窗、任务表单、爱丽丝、设置、日历洞察、经营回单、进展响应式和知识 AI 各自拥有明确文件。\n- **视觉与级联完全不变**：拆分只发生在 PostCSS 顶层 AST 边界；13 个模块按入口顺序重新拼接后与原文件 521,411 字节完全一致，生产 CSS 产物哈希仍为 `index-48CIUhAu.css`。\n- **新增 CSS 架构守卫**：`App.css` 禁止直接追加规则；模块清单与顺序固定；模块内部禁止嵌套导入；单个样式域超过 4,500 行会阻止发布。\n- **设计规范同步升级**：`AGENTS.md`、`DESIGN.md` 和项目结构文档已改为按业务域搜索、复用和归档样式，并明确 13 个样式域的责任边界。\n- **保留现有全局类契约**：未强行改写两万多行 JSX className，现有共享组件、合作伙伴页面和回单导出继续使用同一套全局视觉语言。\n- **工时与金额不变**：纯前端样式架构调整，未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#63",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.77（业务域状态管理完成）",
    "content": "- **Zustand 状态层完整接入**：新增权限、任务、任务运行态、文件、设置和全局 UI 六个业务域 store，跨视图状态与服务端实体不再散落在 `App` 和工作区 Hook 的闭包中。\n- **主应用状态清零**：`App.tsx` 不再使用 `useState / useReducer`，只负责读取路由、选择状态、计算派生数据和组合页面；主入口进一步降至 935 行。\n- **工作区 Hook 回归编排职责**：`useWorkspaceData` 不再自行保存任务、文件、回单、权限和设置，只负责后端 hydration、身份失效处理、附件运行时与连接状态协调。\n- **按域原子刷新**：一次后端状态刷新收敛为权限、任务、文件、设置四次原子提交，避免逐字段连续更新引发多轮无意义重渲染。\n- **细粒度运行态订阅**：任务时间线、AI 进度评估、作废确认、验收庆祝和附件分析分别进入任务 / 文件运行态，使用 Zustand 浅比较避免无关状态联动。\n- **偏好与启动缓存保持兼容**：任务详情栏和任务行主题继续使用原有本地键；D1 启动快照继续沿用版本号与 30 分钟 TTL，不改变现有秒开和失效策略。\n- **状态架构回退被阻止**：新增状态架构守卫，校验六个 store 均由 Zustand 管理，并禁止 `App.tsx`、`useWorkspaceData` 重新引入跨视图 `useState / useReducer`。\n- **工时与金额不变**：未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#64",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.76（主应用巨型单文件拆分完成）",
    "content": "- **`App.tsx` 巨型入口治理完成**：主入口从 2,346 行降至 978 行，只保留应用装配、跨视图派生数据和页面组合，不再直接承载 D1/R2 装载、任务写入、设置写入或全局快捷键实现。\n- **工作区数据层独立**：首屏快照、身份权限、任务 / 文件 / 回单状态、后端同步和 AI 服务商配置迁入 `useWorkspaceData`。\n- **任务写入域独立**：任务创建、进展与验收、附件上传、AI 进度、并发写入保护、作废恢复和文件操作迁入 `useTaskOperations`，原金额与工时保护逻辑保持不变。\n- **设置与权限域独立**：登录退出、访问口令、备份、单价、回单抬头、计税方式、设计类型和 AI 模型配置迁入 `useSettingsOperations`。\n- **全局交互独立**：命令面板动作、导航快捷键、任务快捷键和月份快捷键迁入 `useAppShortcuts`。\n- **架构回退被阻止**：新增 `architecture:guard`，`App.tsx` 超过 1,000 行时质量门直接失败，避免重新长成巨型入口。\n- **长套件回归更可靠**：桌面与移动回归各拆为两个 18 项独立分片，分别使用独立 D1、模型服务与端口；分片遇到测试基础设施异常时会换新端口最多自动重跑两次，真实断言连续失败仍会阻止发布。\n- **工时与金额不变**：未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#65",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.75（附件与任务时间线运行态独立）",
    "content": "- **附件分析轮询独立**：待处理 / 处理中附件的状态签名、轮询次数、并发保护和结果合并统一进入 `useAttachmentRuntime`。\n- **缩略图自愈独立**：PDF、PSD / AI、Office 和视频预览缺失时的源文件读取、客户端生成、回传与有限重试迁出主应用，所有入口继续共享同一份持久化预览。\n- **任务时间线读取独立**：详情、进展、附件上传后的活动记录刷新统一进入 `useTaskActivity`，较早请求不会覆盖用户刚切换到的新任务。\n- **主入口继续减负**：`App.tsx` 从 2,456 行降至 2,347 行；附件运行副作用与任务时间线读取不再占用主应用闭包。\n- **附件行为保持一致**：AI 分析仍每 4 秒轮询、最多 60 次；缩略图仍每个文件最多尝试 3 次、单轮最多处理 6 个文件。\n- **浏览器回归更稳定**：桌面端与移动端各自启动独立测试 Worker，PDF 首页等待时间与产品 20 秒预览上限对齐，避免长套件资源竞争造成假失败。\n- **工时与金额不变**：未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#66",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.74（Toast 与后端运行态独立）",
    "content": "- **Toast 队列成为独立状态域**：优先级裁剪、错误延长展示、操作按钮、自动消失、手动关闭和定时器回收统一进入 `useToastNotifications`。\n- **后端运行态集中维护**：8 秒慢连接提示、浏览器在线 / 离线监听、R2 真实用量读取和 5 分钟刷新统一进入 `useBackendRuntime`。\n- **刷新与写入链路保持原位**：D1 状态刷新、登录失效处理和所有任务写入仍由主应用掌控，新 Hook 只提供运行状态与通知能力。\n- **主入口继续减负**：`App.tsx` 从 2,523 行降至 2,456 行；生产主包约 390.09KB（gzip 126.02KB）。\n- **可靠性规则不变**：错误 Toast 仍优先保留，连接超过 8 秒仍提示网络问题，R2 用量仍来自后端真实统计。\n- **工时与金额不变**：未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#67",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.73（每日知识与 Agent 通知状态独立）",
    "content": "- **每日知识状态完整迁出**：当前知识、候选队列、历史去重、本地缓存、AI 预取、失败兜底和手动换一条统一进入 `useDailyKnowledge`。\n- **Agent 后台通知独立**：后台分析任务轮询、状态变化判断、完成 / 失败 Toast、定时任务静默处理和已读更新统一进入 `useAgentJobNotifications`。\n- **主应用减少副作用耦合**：移出 4 个 state、7 个 ref 及多段定时器 / 缓存副作用，`App` 只消费当前知识和后台任务结果。\n- **主入口继续减负**：`App.tsx` 从 2,741 行降至 2,523 行；生产主包约 389.79KB（gzip 125.94KB）。\n- **知识与 Agent 行为保持一致**：管理员预取、三次去重请求、离线知识池、工作复盘提醒和分析结果跳转规则不变。\n- **工时与金额不变**：未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#68",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.72（全局浮层编排独立）",
    "content": "- **全局浮层统一编排**：每日知识、命令面板、快捷键、工作助手、语义搜索、新建 / 编辑任务、任务详情、进展 / 验收、文件预览、登录、确认和作废弹窗统一迁入 `AppOverlayLayer`。\n- **权限与写入仍由主应用掌控**：浮层组件只组合现有 UI，任务保存、验收、删除、上传、管理员校验和刷新动作继续由 `App` 注入，没有新增第二套业务链路。\n- **高频操作保持同步**：新建任务与进展 / 验收面板继续同步加载，打开面板后立即粘贴图片的行为不受代码拆分影响；工作助手和语义搜索继续按需加载。\n- **提示层一并收口**：Toast、操作按钮和验收庆祝效果从主入口移入统一浮层，主入口不再直接维护各类弹窗 JSX。\n- **主入口继续减负**：`App.tsx` 从 2,899 行降至 2,741 行；生产主包约 389.19KB（gzip 125.59KB）。\n- **工时与金额不变**：未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。"
  },
  {
    "id": "CHANGELOG.md#69",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.71（工作台展示与统计边界收口）",
    "content": "- **工作台不再依赖反向 UI 插槽**：任务行、右键菜单、快捷操作和任务详情侧栏统一由 `DashboardView` 渲染，主应用只传业务数据与受权限约束的动作。\n- **统计状态进入独立 Hook**：月度工时、计费金额、提醒轮播、类型分布、日趋势、年度统计和收入日分组迁入 `useWorkspaceAnalytics`，视图与业务计算边界更清楚。\n- **主入口继续减负**：`App.tsx` 从 3,222 行降至 2,899 行；工作台内部不再由主入口拼接多个 ReactNode。\n- **金额公式保持原样**：统计 Hook 继续调用 `taskAccounting` 的同一组函数，导入工时、锁定回单和补录映射口径不变。\n- **工时与金额不变**：未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算数据。"
  },
  {
    "id": "CHANGELOG.md#70",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.70（工作台视图整体独立）",
    "content": "- **工作台成为独立视图**：月度统计、每日知识、任务提醒、任务筛选与分页、已验收分组、月度洞察、趋势图和年度统计整体迁入 `DashboardView`。\n- **写入与权限逻辑保持单一来源**：任务右键菜单、任务详情侧栏和所有写入动作继续由主应用生成并作为插槽传入，未复制管理员权限或接口调用。\n- **金额计算与展示彻底分层**：计费工时、月度金额和年度金额仍由 `taskAccounting` 与主业务层计算，工作台视图只消费已计算结果。\n- **主入口继续减负**：`App.tsx` 从 3,444 行降至 3,222 行；生产主包约 385.61KB（gzip 125.34KB）。\n- **工时与金额不变**：未修改任务工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。\n- **完整回归通过**：UI 复用守卫、16 项会计对账、构建、ESLint、隔离 Agent `88/88` 和桌面 / 移动端浏览器 `72/72` 全部通过。"
  },
  {
    "id": "CHANGELOG.md#71",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.69（应用外壳与导航边界拆分）",
    "content": "- **左侧导航独立维护**：品牌区、主导航、账户身份、管理员菜单、R2 真实用量、版本信息和登录 / 退出入口整体迁入 `AppSidebar`。\n- **顶栏独立维护**：页面标题、月份选择、任务日历日 / 周 / 月切换、周期导航、语义搜索、工作助手、快捷键和新建任务入口整体迁入 `AppTopbar`。\n- **权限与快捷键行为保持一致**：游客、合作伙伴、只读成员、协作者和管理员继续沿用原有可见范围，导航快捷键和账户菜单操作路径不变。\n- **主入口继续减负**：`App.tsx` 从 3,616 行降至 3,444 行；生产主包约 383.54KB（gzip 124.46KB）。\n- **工时与金额不变**：未修改任务、工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。\n- **完整回归通过**：UI 复用守卫、16 项会计对账、构建、ESLint、隔离 Agent `88/88` 和桌面 / 移动端浏览器 `72/72` 全部通过。"
  },
  {
    "id": "CHANGELOG.md#72",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.68（进展与验收工作流整体组件化）",
    "content": "- **进展 / 验收面板整体独立**：分段计时、等待记录、修改建议、普通进展、验收进展、附件处理、AI 命名、AI 验收备注和任务体感反馈整体迁入 `TaskProgressModal`。\n- **验收数据契约收口**：验收工时、备注、反馈、计时段、等待段、验收文件和任务修订字段统一使用共享 `AcceptancePayload`，主应用与进展面板不再依赖同一巨大闭包。\n- **时长展示使用单一实现**：进展时间线与面板共用 `durationDisplay`，避免后续拆分时出现同一时长两套文案算法。\n- **高频粘贴保持即时可用**：进展 / 验收面板保持同步加载，打开后即可在任意位置粘贴图片，不引入懒加载事件空窗。\n- **主入口大幅减负**：`App.tsx` 从 6,373 行降至 3,616 行，本轮减少 2,757 行；生产主包约 382.42KB（gzip 123.61KB）。\n- **工时与金额不变**：未修改进展计时、等待时长、验收工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。\n- **完整回归通过**：UI 复用守卫、16 项会计对账、构建、ESLint、隔离 Agent `88/88` 和桌面 / 移动端浏览器 `72/72` 全部通过。"
  },
  {
    "id": "CHANGELOG.md#73",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.67（新建任务整体组件化）",
    "content": "- **新建 / 编辑任务整体独立**：任务字段、设计类型、合作伙伴附件、AI 文案与分类建议、AI 工时建议、语音排期、补录和不计费设置整体迁入 `NewTaskModal`，不再共享 `App.tsx` 的巨大闭包。\n- **高频入口保持即时响应**：新建任务组件独立但同步加载，避免懒加载窗口吞掉用户在弹窗刚打开时执行的 Command+V；图片继续进入合作伙伴附件，文本继续进入任务需求。\n- **共享规则进一步收口**：AI 建议采用 / 编辑 / 拒绝判断迁入 `aiLearning`，设计类型分组展开与颜色归一化迁入 `designTypeGroups`，主应用和任务弹窗使用同一实现。\n- **主入口继续减负**：`App.tsx` 从 7,748 行降至 6,373 行，本轮减少 1,375 行；生产主包约 382.42KB（gzip 124.23KB）。\n- **工时与金额不变**：未修改任务数据、实际工时、预估工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。\n- **完整回归通过**：133 文件 UI 复用守卫、16 项会计对账、构建、ESLint、隔离 Agent `88/88` 和桌面 / 移动端浏览器 `72/72` 全部通过。"
  },
  {
    "id": "CHANGELOG.md#74",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.66（工作助手整体独立与按需加载）",
    "content": "- **工作助手成为独立业务组件**：对话、项目归档、临时对话、历史搜索、模型选择、本机 CLI、附件上传、后台分析、任务计划和任务记忆整体迁入 `ChatPanel`。\n- **首屏不再加载工作助手**：`ChatPanel` 通过 `React.lazy` 仅在打开工作助手时下载，形成约 60.94KB 的独立包。\n- **主包显著降低**：首屏主包从约 482.46KB 降至 381.84KB，减少约 100.62KB，gzip 从约 154.25KB 降至 123.80KB。\n- **助手功能完整保留**：云端会话同步、向量 / 普通搜索、临时对话不落盘、项目管理、流式分析、写入前确认、附件结果和回单下载交互保持不变。\n- **主入口大幅减负**：`App.tsx` 从 9,109 行降至 7,748 行，本轮减少 1,361 行。\n- **工时与金额不变**：未修改 Agent 编排、任务写入、实际工时、预估工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。\n- **完整回归通过**：130 文件 UI 复用守卫、16 项会计对账、构建、ESLint、隔离 Agent `88/88` 和重启后的桌面 / 移动端浏览器 `72/72` 全部通过。"
  },
  {
    "id": "CHANGELOG.md#75",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.65（Agent 确认与分析过程组件拆分）",
    "content": "- **写入前确认卡独立**：Agent 新建任务、修改字段、记录进展与验收操作的草稿核对、差异展示、编辑与 Workflow 确认迁入 `AgentApprovalCard`。\n- **排期编辑能力原样保留**：确认卡继续复用 `MonthPicker`、`PlanDateTimeField` 和 `VoiceScheduleButton`，保留开始时间、预估工时、交付时间的语音识别与联动。\n- **任务候选选择独立**：Agent 对多个相似任务的消歧选择迁入 `AgentTaskSelectionCard`，继续要求用户明确选中后才执行。\n- **分析步骤渐进展示独立**：工具标记清理、当前分析步骤、已完成步骤和最终状态迁入 `AgentExecutionTimeline`，仍先逐项展示处理过程再显示答案。\n- **主入口继续减负**：`App.tsx` 从 9,506 行降至 9,109 行，本轮减少 397 行；主包约 482.46KB。\n- **工时与金额不变**：未修改 Agent 工具编排、任务数据、实际工时、预估工时、计费、金额、验收日期、补录归属或结算口径。\n- **完整回归通过**：129 文件 UI 复用守卫、16 项会计对账、构建、ESLint、隔离 Agent `88/88` 和桌面 / 移动端浏览器 `72/72` 全部通过。"
  },
  {
    "id": "CHANGELOG.md#76",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.64（Agent 结果界面与通用交互拆分）",
    "content": "- **Agent 附件结果独立维护**：真实附件列表、Microsoft Excel 回单标识、在线预览、下载与附件文件映射迁入 `AgentAttachmentResults`。\n- **回单预览交互原样保留**：完整 Excel 回单继续使用共享 `SettlementReceipt`，保留关闭、放大、缩小、1:1、适合窗口和横纵滚动。\n- **后台分析卡片独立**：排队、资料汇总、报告生成、完成状态与重试 / 取消交互迁入 `AgentAnalysisTaskCard`，任务中心与卡片共用同一状态文案。\n- **Toast 与快捷键规则收口**：错误优先级、最多四条可见 Toast、自动语义分类与月份数字键映射迁入独立通用模块。\n- **主入口继续减负**：`App.tsx` 从 9,821 行降至 9,506 行，本轮减少 315 行；主包约 482.46KB。\n- **工时与金额不变**：未修改实际工时、预估工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。\n- **完整回归通过**：126 文件 UI 复用守卫、16 项会计对账、构建、ESLint、隔离 Agent `88/88` 和桌面 / 移动端浏览器 `72/72` 全部通过。"
  },
  {
    "id": "CHANGELOG.md#77",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.63（上传与任务上下文链路独立）",
    "content": "- **图片上传链路单一模块**：大文件限制、附件命名、粘贴截图命名、图片压缩 Worker、主线程排队降级、缩略图与 Base64 转换迁入 `fileUpload`。\n- **压缩与响应性保持不变**：继续使用 2400px 归档上限、480px 缩略图、后台 Worker 串行队列和主线程失败回退，多张高分辨率验收图不阻塞备注输入。\n- **任务样本分析独立**：同类工时偏差、修改信号和已验收状态归一化迁入 `taskContextInsights`，仍只使用已验收、非补录、工时完整的有效样本。\n- **Agent 任务证据整理独立**：任务附件去重、审计动态摘要、进展附件归属和需求文案清理迁入 `taskAssistantContext`，不再由前端组件闭包内部拼接。\n- **主入口跌破一万行**：`App.tsx` 从 10,345 行降至 9,821 行，本轮减少 524 行；主包约 482.46KB。\n- **工时与金额不变**：未修改实际工时、预估工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。\n- **完整回归通过**：120 文件 UI 复用守卫、16 项会计对账、构建、ESLint、隔离 Agent `88/88` 和桌面 / 移动端浏览器 `72/72` 全部通过。"
  },
  {
    "id": "CHANGELOG.md#78",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.62（附件预览生成链路独立）",
    "content": "- **预览生成单一模块**：PDF、PSD、视频首帧、DOCX、PPTX、XLSX 和文本封面的上传前预览迁入 `attachmentPreview`，任务、验收和文件库继续复用同一链路。\n- **文件名规则共享**：基础文件名与扩展名拆分迁入 `fileName`，AI 命名、附件替换、缩略图和预览文件继续使用同一规则。\n- **现有容错完整保留**：PDF 超时、Office 离屏渲染失败回退、空白画布检测、对象 URL 释放及 Excel 首表行列限制保持不变。\n- **主入口继续减负**：`App.tsx` 从 10,590 行降至 10,345 行，主包约 482.45KB。\n- **工时与金额不变**：未修改附件源文件、任务数据、工时、金额、验收日期、补录归属、实际完成日期或结算口径。\n- **完整回归通过**：117 文件 UI 复用守卫、16 项会计对账、构建、ESLint、隔离 Agent `88/88` 和桌面 / 移动端浏览器 `72/72` 全部通过。"
  },
  {
    "id": "CHANGELOG.md#79",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.61（知识内容库与聊天展示组件拆分）",
    "content": "- **每日知识内容成为独立数据源**：完整知识内容池迁入 `data/dailyKnowledgePool`，主应用与轮换算法只读取这一份内容，没有复制或改写任何文章。\n- **聊天答案展示独立维护**：Markdown、表格、链接、运行步骤、最终答案和 `<think>` 内容隔离迁入 `ChatContent`；仍只向用户展示可核验的处理步骤和最终答案。\n- **模型标签规则单一来源**：Provider 名称、路由名称、OpenRouter 模型名和当前首选模型标签迁入 `chatModelPresentation`，继续严格尊重用户选择的主模型。\n- **主入口显著减负**：`App.tsx` 从 11,103 行降至 10,590 行，本轮减少 513 行；主包约 482.45KB，保持基本稳定。\n- **工时与金额不变**：未修改实际工时、预估工时、计费、金额、验收日期、补录归属、实际完成日期、结算口径、DeepSeek 思考模式或 12,000 Token 预算。\n- **完整回归通过**：115 文件 UI 复用守卫、16 项会计对账、构建、ESLint、隔离 Agent `88/88` 和桌面 / 移动端浏览器 `72/72` 全部通过。"
  },
  {
    "id": "CHANGELOG.md#80",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.60（工作助手历史与知识轮换边界）",
    "content": "- **会话历史持久化单一来源**：工作助手的本地历史、对话项目、云端记录合并、删除写回和模型选择迁入 `conversationCache`；临时对话继续不落盘，历史记录继续保留原始时间与消息。\n- **每日知识轮换算法独立**：历史避让、队列去重、随机补位和刷新恢复迁入 `dailyKnowledgeCatalog`，缓存容量与当前文章 / 队列存储 key 保持不变。\n- **附件文本解析集中复用**：TXT、Markdown、CSV、DOCX、PDF 和 PPTX 的前端文字提取迁入 `attachmentText`，继续保留 16,000 字限制、PDF 前 20 页和 Office XML 解析规则。\n- **主入口继续减负**：`App.tsx` 从 11,330 行降至 11,103 行；主包约 482.47KB，体积基本持平，本轮收益集中在历史一致性与模块维护边界。\n- **工时与金额不变**：未修改实际工时、预估工时、计费、金额、验收日期、补录归属、实际完成日期、结算口径或主模型路由。\n- **完整回归通过**：112 文件 UI 复用守卫、16 项会计对账、构建、ESLint、隔离 Agent `88/88` 和桌面 / 移动端浏览器 `72/72` 全部通过。"
  },
  {
    "id": "CHANGELOG.md#81",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-24 · v0.32.59（草稿缓存边界与完整门禁稳定性）",
    "content": "- **进展草稿职责隔离**：进展文字、时间草稿、待上传附件和稳定条目 ID 迁入 `progressDraftCache`，保存成功时只清理当前进展草稿，不再与启动快照共用模糊的清理函数。\n- **启动快照与页面草稿独立维护**：带版本号和 30 分钟 TTL 的工作台启动快照迁入 `stateCache`；新建任务草稿和每日知识历史 / 队列分别迁入独立模块，保留原有存储 key、恢复规则与容量限制。\n- **主入口继续减负**：`App.tsx` 从约 11,562 行降至约 11,330 行，主包由约 482.44KB 降至约 482.30KB；业务组件不再直接持有多套全局缓存实现。\n- **完整门禁不再假死**：浏览器和隔离 Agent 评测共用统一的 Wrangler D1 子进程管理；Wrangler 明确完成命令后会安全结束残留进程组，普通 Node 评测仍严格检查真实退出码。\n- **工时与金额不变**：本次只调整缓存和测试边界，未修改实际工时、预估工时、计费、金额、验收日期、补录归属、实际完成日期或结算口径。\n- **完整回归通过**：109 文件 UI 复用守卫、16 项会计对账、构建、ESLint、隔离 Agent `88/88` 和桌面 / 移动端浏览器 `72/72` 全部通过。"
  },
  {
    "id": "CHANGELOG.md#82",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.58（工时输入与排期派生规则复用）",
    "content": "- **工时输入解析单一来源**：分钟、小数小时、`1:30`、中文小时 / 分钟和英文单位的解析与直观格式化迁入 `durationInput`。\n- **排期派生规则统一**：新建任务与记录进展共用分钟归一化、开始 / 结束时间差和日期部分替换规则，避免两个表单逐渐出现不同口径。\n- **交互语义不变**：仍支持 `15 分钟`、`30 分钟`、`1 小时`、`1.5`、`0.25`、`1:30` 等既有输入方式，三项排期的派生字段逻辑保持不变。\n- **工时与金额不变**：本次为既有纯函数等价迁移，未修改实际工时、预估工时、计费、金额、验收日期、补录归属或结算口径。\n- **回归门禁**：构建、ESLint、UI 复用守卫、16 项会计对账和桌面 / 移动端专项 `8/8` 通过。"
  },
  {
    "id": "CHANGELOG.md#83",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.57（进展时间段规则集中复用）",
    "content": "- **时间段规则单一来源**：进展与验收使用的默认时间草稿、开始 / 结束时间推导、跨日分钟戳、冲突检测和最近可用时段迁入 `timeEntryDraft`。\n- **进展弹窗继续减耦**：`TaskProgressModal` 只消费共享计算结果，不再在组件内部保存一套时间段算法，为后续独立组件拆分建立稳定边界。\n- **工时与金额不变**：本次为既有纯函数等价迁移，未修改实际工时、预估工时、计费、金额、验收日期、补录归属或结算口径。\n- **回归门禁**：构建、ESLint、UI 复用守卫、16 项会计对账和进展 / 验收桌面与移动端专项 `6/6` 通过。"
  },
  {
    "id": "CHANGELOG.md#84",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.56（进展附件预览组件拆分）",
    "content": "- **附件缩略图与预览独立维护**：进展附件的缩略图、悬停预览、完整预览和对象 URL 清理迁入 `PendingAttachmentPreview`，PDF、图片、Office 和视频继续使用统一阅读器。\n- **附件草稿类型集中**：上传进度、AI 命名、压缩预览、上传结果和验收范围统一收敛到共享 `PendingProgressAttachment` 类型。\n- **继续复用原压缩链路**：缩略图组件通过显式回调调用既有图片准备逻辑，没有复制图片压缩、源文件或上传方案。\n- **工时与金额不变**：未修改任务数据、工时、金额、验收日期、补录归属、实际完成日期或结算口径。\n- **浏览器门禁恢复稳定**：针对 Wrangler 4.113 在 Node 25 下完成本地 D1 初始化后不退出的问题，评测启动器会识别成功标记并安全结束残留进程组；附件专项 `4/4`、桌面 / 移动端完整浏览器门禁 `72/72` 通过。\n- **验证说明**：UI 复用守卫、构建、ESLint、16 项会计对账和完整浏览器门禁通过。"
  },
  {
    "id": "CHANGELOG.md#85",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.55（月份与设计类型组件复用）",
    "content": "- **月份选择器独立维护**：月份导航与年份 / 月份浮层迁入 `MonthPicker`，工作台和结算相关入口继续使用统一交互。\n- **设计类型选择器独立维护**：新建任务的设计类型分组、子分类和已选状态迁入 `NewTaskDesignTypeSelector`，避免表单内重复实现。\n- **组件边界继续收敛**：两个输入组件不再依赖 `App.tsx` 闭包，主入口继续减负。\n- **工时与金额不变**：本次只调整组件边界，未修改任务数据、工时、金额、验收日期、补录归属、实际完成日期或结算口径。\n- **回归门禁**：UI 复用守卫、ESLint、16 项会计对账和 72 项桌面 / 移动端浏览器用例通过。"
  },
  {
    "id": "CHANGELOG.md#86",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.54（任务详情弹窗组件拆分）",
    "content": "- **任务详情弹窗独立维护**：将任务摘要、状态、工时、等待记录、验收入口和编辑入口迁入 `TaskDetailModal`，减少主应用中的大型 UI 闭包。\n- **继续复用既有业务规则**：详情弹窗直接使用统一的任务到期判断、任务进度、实际工时、等待时长和结算月份规则，没有复制计算逻辑。\n- **工时与金额不变**：本次只调整组件边界，未修改任务数据、工时、金额、验收日期、补录归属、实际完成日期或结算口径。\n- **回归门禁**：UI 复用守卫、ESLint、16 项会计对账和 72 项桌面 / 移动端浏览器用例通过。"
  },
  {
    "id": "CHANGELOG.md#87",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.53（任务页模块化与按需加载）",
    "content": "- **任务管理按需加载**：任务列表 / 日历视图迁入 `TasksView`，仅进入任务页面时加载，日历继续保持独立分包，减少首屏解析和主应用重渲染压力。\n- **任务展示规则集中复用**：日期格式、排期倒计时、临期判断、补录日期展示、设计类型配色和任务列表右键空白区判断迁入 `taskListPresentation`，工作台、任务管理、任务详情不再各自维护同类规则。\n- **进展洞察提示独立复用**：`TaskContextInsightBadge` 成为共享组件，工作台和任务管理继续使用同一种管理员提示样式。\n- **工时与金额不变**：本次仅调整组件边界、懒加载和展示逻辑位置，未修改任务数据、工时、金额、验收日期、补录归属、实际完成日期或结算口径。\n- **回归门禁**：UI 复用守卫、构建、ESLint、16 项会计对账、88 项 Agent 评测和 72 项桌面 / 移动端浏览器用例全部通过。"
  },
  {
    "id": "CHANGELOG.md#88",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.52（预览与图片优化链路检修）",
    "content": "- **图片优化失败可安全回退**：无法解码或压缩的图片不再把 Promise rejection 泄漏到全局错误监控，系统会保留原文件继续上传，并由界面反馈后续状态。\n- **PDF 阅读器清理更稳**：关闭或切换 PDF 预览时，PDF.js worker 的销毁 Promise 增加安全收尾，避免取消中的 worker 产生未处理异常。\n- **回归夹具修正**：将 PDF / 图片预览用例中的损坏 PNG 替换为合法夹具，避免测试误报产品级前端异常。\n- **维护项**：未修改 PDF 分页、缩放、缩略图、任务数据、工时、金额、结算日期或模型路由。"
  },
  {
    "id": "CHANGELOG.md#89",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.51（任务侧栏与附件展示组件拆分）",
    "content": "- **任务详情侧栏独立维护**：将工作台与任务管理共用的进展、反馈、等待记录、实时等待计时、进度评估和任务信息侧栏迁入 `DashboardTaskSidebar`，两处入口继续复用同一个组件。\n- **附件预览入口统一**：附件悬停缩略图和 PDF / PSD / Office / 视频回退预览迁入共享组件；PDF 首页生成迁入 `pdfPreview` 工具，保持原有超时与失败回退行为。\n- **展示规则单一来源**：反馈来源称呼、验收附件判断、记录倒序、时间范围和等待时长迁入 `taskPresentation`，避免任务页和工作台形成第二套显示口径。\n- **主入口继续减负**：`App.tsx` 从 13,608 行降至约 12,802 行；未修改任务数据、工时、金额、验收日期、补录归属、结算口径或 AI 模型路由。\n- **回归门禁**：UI 复用守卫、TypeScript 构建与 ESLint 通过，继续执行任务、附件和验收流程回归。"
  },
  {
    "id": "CHANGELOG.md#90",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.50（语音排期组件独立复用）",
    "content": "- **语音排期能力独立成共享组件**：将录音、浏览器实时听写、云端转写、5 秒整理反馈、采集完成、关闭和结果应用整体迁入 `VoiceScheduleButton`，新建任务、记录进展和验收继续使用同一条链路。\n- **时间字段格式化继续单一来源**：中文工时展示迁入共享日期时间工具，语音结果预览与业务表单继续复用同一套格式化规则。\n- **维护边界更清晰**：主入口移除约 413 行语音状态机代码，减少顶层闭包耦合；未修改语音接口、DeepSeek / 备用模型路由、开始时间、交付时间、工时计算或任何结算数据。\n- **回归门禁**：UI 复用守卫、TypeScript 构建、ESLint 与现有语音浏览器用例继续通过。"
  },
  {
    "id": "CHANGELOG.md#91",
    "sourcePath": "CHANGELOG.md",
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.49（全站日期时间组件独立复用）",
    "content": "- **日期时间选择器成为真正共享组件**：将全站 `PlanDateTimeField` 从巨型 `App.tsx` 迁入独立组件，任务新建 / 编辑、进展、验收、结算范围和链接有效期继续使用同一套日历与时间列。\n- **日期格式规则集中复用**：日期时间输入规范化与展示格式迁入共享 `dateTime` 工具，主应用、结算页和日期组件不再各自保存实现。\n- **结算页移除临时组件传参**：`ReportsView` 直接导入共享日期组件，不再由主应用通过 props 传递组件定义，模块边界更清晰。\n- **主文件继续减负**：`App.tsx` 从约 14,423 行降至约 14,020 行；日期组件独立约 393 行，主入口保持约 492.15KB。\n- **交互专项回归**：桌面 / 移动端覆盖新建任务日历越界选择、语音排期、结算自定义日期、分享范围与 Excel 回单，未改变现有交互。\n- **维护项**：未修改任务日期值、工时、6 月 / 7 月金额、补录归属、实际完成日期、税务公式或结算回单。"
  },
  {
    "id": "CHANGELOG.md#92",
    "sourcePath": "CHANGELOG.md",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.46（结算计算单一来源与对账门禁）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.45（任务日历拆分与日期规则收敛）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.44（设置整页拆分与首屏减重）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.43（设置依赖解耦与知识弹窗拆分）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.42（收入视图拆分与金额对账门禁）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.41（文件库整页拆分与按需加载）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.40（正式路由接管与历史状态收敛）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.39（设置模块拆分与按需加载）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.38（生产错误监控与可观测性）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.37（任务基础组件与空状态统一）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.36（语义搜索与知识库按需拆分）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.35（快捷交互组件拆分）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.34（文件预览弹窗与类型工具拆分）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.33（文件预览阅读器拆分）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.32（弹窗与文件菜单批量拆分）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.31（任务右键菜单与进度规则拆分）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-23 · v0.32.30（弹窗与月份选择组件拆分）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.29（趋势图组件拆分）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.28（图表组件拆分与 CI 稳定）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.27（运行韧性与基础组件）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.26（中等屏幕侧栏收缩）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.25（面板圆角收敛为 4px）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.24（导出记录动作与快照范围对齐）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.23（导出记录轻量化与自定义导出口径统一）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.22（结算导出与合作伙伴链接完整收口）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.21（合作伙伴时间线与回单预览重构）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.20（回单项目卡片与验收附件彻底对齐）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.19（项目交付改为四列紧凑画廊）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.18（合作伙伴回单按项目归档交付证据）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.17（任务实际完成日期恢复真实口径）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.16（分享与锁定彻底解耦）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.15（结算导出、分享与 Agent 完整闭环）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.14（自定义日期实时驱动回单预览）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.13（结算回单完全采用正式模板）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-22 · v0.32.12（Excel 下载与结算回单并轨）",
  ...
  },
  {
    ...
    "sourceLabel": "Giverny 更新日志",
    "category": "更新日志",
    "heading": "2026-07-21 · v0.32.11（模型弹窗结构对齐）",
