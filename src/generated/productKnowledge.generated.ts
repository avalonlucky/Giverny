...
export const productKnowledgeDocuments: ProductKnowledgeDocument[] = [
  ...
  },
  {
    ...
    "category": "运营手册",
    "heading": "9. 生产保护与受控发布",
    "content": "运行中心会把回答成功率、事实验真率、P95 响应、主模型完成率和后台任务失败率统一判定为正常、接近红线、突破红线或样本观察中。突破红线时直接显示“已阻止发布”，不再让管理员自己从多张统计卡猜测系统是否稳定。\n\n正常情况下至少 99% 请求必须由用户选择的主模型完成。主模型第一次失败后先由同一个模型重试；只有连续超时、额度限制、供应商服务异常、鉴权故障、连续无效结果或能力明确不匹配才允许启动应急模型。用户取消、单次失败和原因不明不能触发备用模型。\n\n正式发布通过 Cloudflare Versions 与 Deployments HTTP API 进行候选验证。候选版本必须通过健康接口、事实协议、版本号和静态资源一致性核对后才接管全部流量；失败时自动恢复上一稳定版本。数据库和 R2 不随代码回滚，迁移必须继续保持向后兼容。\n\n管理员可配置签名治理 Webhook 接收应急回退等运行事件。事件不包含问题、回答、任务、附件、金额或其他业务内容。完整规则见 `docs/AGENT_GOVERNANCE.md`。"
  },
  {
