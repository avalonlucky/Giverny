...
export const productKnowledgeDocuments: ProductKnowledgeDocument[] = [
  ...
  },
  {
    ...
    "content": "工作助手现在由统一 Agent 编排层组织，不会把大模型的一次回答直接当成最终事实。它会先理解问题、制定步骤，再按当前登录工作区调用任务、附件、财务或产品说明工具，最后核对工具证据后回答。\n\n如果工具临时失败或验真发现缺少关键证据，Agent 会在同一业务目标下补充最小必要证据；仍无法取得可靠证据时会明确停止或请你补充，不会用猜测填空。管理员可在「设置 → 模型 → AI 运行中心 → Agent 执行审计」查看 ADK 协调与审核模型、工具状态、确定性证据和审核结果。审计不保存问题和回答正文，并按当前工作区隔离。\n\n- 查询金额、工资或工时：以网站计算结果为准，模型不能自行改数字。\n- 查询具体任务卡点或延期：读取任务详情和当前等待记录后回答。\n- 修改数据：先展示草稿，只有明确确认后才执行。\n- 模型边界：工作助手的协调与证据审核模型由 Google ADK Runtime 配置决定；设置页的文字/识图主备模型继续用于任务命名、文案、附件理解等非 Agent 功能。\n- 本机 CLI：「设置 → 本机 CLI」仍可配对、扫描和诊断本机连接；配置生产 ADK 后，它不参与网站文本对话路由。\n\n---"
  },
  {
    "id": "使用手册.md#4",
    "sourcePath": "使用手册.md",
