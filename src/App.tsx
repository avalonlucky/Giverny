...
function PendingAttachmentThumbnail({
  ...
  const previewUrl = useMemo(() => isImage ? URL.createObjectURL(attachment.file) : undefined, [attachment.file, isImage])
...
function App() {
  ...
  const taskItemsRef = useRef<Task[]>([])
            ...
            <section className="panel task-panel dashboard-task-panel">
              ...
              <div className="task-list" onContextMenu={openDashboardCreateMenu}>
                ...
                {visibleTasks.map((task) => {
                  ...
                  const canRecordProgress = isTaskStarted(task)
                  ...
                  >
                    ...
                    <div className="task-row-end">
                      ...
                      <div className="task-row-actions" aria-label="任务快捷操作">
                        ...
                        <button type="button" className="icon-button" title={canRecordProgress ? '记录进展' : '改为进行中后可记录进展'} aria-label={canRecordProgress ? '记录进展' : '改为进行中后可记录进展'} disabled={!canRecordProgress} onClick={(event) => { event.stopPropagation(); handleOpenTaskProgress(task.id) }}>
                          <BarChart3 size={15} />
                        </button>
                        <button
                          type="button"
...
function TaskContextMenu({
  ...
  const canRecordProgress = isTaskStarted(menu.task)
  const hasAcceptanceProgress = (menu.task.timeEntries ?? []).some((entry) => entry.isAcceptanceProgress)
  const hasAcceptanceClosure = menu.task.status === '已验收' || hasAcceptanceProgress
    ...
    <div className="task-context-menu" style={{ left: menu.x, top: menu.y }} role="menu">
      ...
      {!isVoided && (
        <>
          ...
          <button type="button" disabled={!canRecordProgress} title={canRecordProgress ? '记录进展' : '改为进行中后可记录进展'} onClick={() => run(() => onOpenProgress(menu.task))}>
            <BarChart3 size={15} />
            记录进展
          </button>
...
function DashboardTaskSidebar({
  ...
  const displayedProgress = canRecordProgress ? snappedProgress : 0
      ...
      {activeTab === 'info' ? (
        ...
        <section className="dashboard-side-section dashboard-side-progress-section" role="tabpanel">
          <div className="dashboard-side-progress">
            ...
            <div className="dashboard-side-progress-scale">
              {[0, 20, 40, 60, 80, 100].map((value) => (
                <button
                  ...
                  title={canRecordProgress ? `设置进度为 ${value}%` : '任务仍在计划中，进度保持为 0'}
            ...
            {!canRecordProgress && <p className="dashboard-side-muted dashboard-side-planned-note">任务仍在计划中。改为进行中后，才会开始记录真实进展、等待与生命周期。</p>}
          </div>
          ...
          {progressPane === 'progress' ? (
            <div className="dashboard-side-subsection dashboard-side-record-pane" role="tabpanel">
              <div className="dashboard-side-subsection-title">
                <span>进展 · 分段计时</span>
                <button type="button" className="text-button dashboard-side-action" disabled={!canRecordProgress} title={canRecordProgress ? '记录进展' : '改为进行中后可记录进展'} onClick={() => onOpenProgress(task.id, 'progress')}>
                  <Plus size={15} />
                  记录进展
                </button>
              </div>
                  ...
                  <div className="dashboard-side-timeline">
                    {shouldShowAcceptanceSummary && (
                      <article className="dashboard-side-time-item dashboard-side-acceptance-item">
                        ...
                        <p>{task.acceptanceNote?.trim() || '已完成验收确认。'}</p>
                        <em>不新增计时 · 已进入验收闭环</em>
                    ...
                    {shownTimeEntries.map((entry) => {
                      ...
                      const entryFiles = files.filter((file) => file.taskId === task.id && file.entryId === entry.id && !file.deletedAt)
                      return (
                        <article className="dashboard-side-time-item" key={entry.id}>
                          ...
                          <p>{entry.isAcceptanceProgress ? (task.acceptanceNote?.trim() || entry.note || '已完成验收确认。') : (entry.note || '未填写具体内容')}</p>
            ...
            >
              ...
              <div className="task-row-end">
                ...
                <div className="task-row-actions" aria-label="任务快捷操作">
                  ...
                  <button type="button" className="icon-button" title={canRecordProgress ? '记录进展' : '改为进行中后可记录进展'} aria-label={canRecordProgress ? '记录进展' : '改为进行中后可记录进展'} disabled={!canRecordProgress} onClick={(event) => { event.stopPropagation(); openProgress(task) }}>
                    <BarChart3 size={15} />
                  </button>
                  <button
                    type="button"
...
function TaskProgressModal({
  ...
  const addReplacementExistingAttachment = async (fileList: FileList | null) => {
    ...
    try {
      ...
      if (isAcceptanceMode && existingFile.scope === 'acceptance' && onUploadAcceptanceFile) {
        await onUploadAcceptanceFile(task.id, file)
      } else {
  ...
  const acceptancePreviewTimeEntries = shouldIncludeAcceptanceDraftEntry
  ...
  const canConfirmAcceptance = (acceptanceLockedHours > 0 || isAcceptanceRevisionMode || !countAcceptanceTime) && !isSaving && Boolean(onConfirmAcceptance) && (!countAcceptanceTime || !draftConflict)
  ...
  const saveProgress = async () => {
    ...
    setTimeEntryError('')
    try {
      const shouldAllowAcceptedTimeEdit = isEditingEntry && Boolean(editingEntry?.isAcceptanceProgress)
      const nextEntry = buildDraftTimeEntry({ isAcceptanceProgress: shouldAllowAcceptedTimeEdit })
      if (nextEntry) {
  ...
  const confirmAcceptanceFromProgress = async () => {
    ...
    setTimeEntryError('')
    try {
      const nextEntry = shouldIncludeAcceptanceDraftEntry ? buildDraftTimeEntry({ isAcceptanceProgress: true }) : null
      if (nextEntry) {
        const conflict = comparableEntries.find((entry) => timeEntriesOverlap(nextEntry, entry))
        if (conflict) {
          setTimeEntryError(`这个时间段和 ${formatEntryDateTimeRange(task, conflict)} 已有记录重叠，请改到前后相邻的空档。`)
      ...
      for (const attachment of pendingAttachments) {
        ...
        try {
          if (onUploadAcceptanceFile) {
            await onUploadAcceptanceFile(task.id, uploadFile)
          } else {
            await onUploadImage(task.id, uploadFile, undefined, nextEntry?.id)
          }
          finalizedUploadedNames.push(uploadFile.name)
      ...
      // 累计工时
      const nextTimeEntries = nextEntry ? [...acceptanceTimeEntries, nextEntry] : acceptanceTimeEntries
      ...
      onConfirmAcceptance(task, {
        ...
        acceptanceFiles: Array.from(new Set([...(task.acceptanceFiles ?? []), ...finalizedUploadedNames])),
      })
      clearDraftCache(progressDraftKey)
      progressAttachmentDraftCache.delete(progressDraftKey)
      onClose()
    ...
    <ModalShell className="task-action-modal task-progress-modal progress-lite-modal" labelledBy="task-progress-title" onClose={onClose}>
      <header className="progress-lite-header">
        <div>
          ...
          <small>{isEditingEntry ? '修改这段记录的内容和时间' : isWaitingMode ? '记录非工作的等待时间段，仅用于洞察分析，不计入结算工时' : isAcceptanceRevisionMode ? `${task.title} · 修改验收备注、附件与验收进展工时` : isAcceptanceMode ? `${task.title} · 记录最后进展、验收备注和交付附件` : `${task.title} · 按时间段计时，工时自动累计并计入结算`}</small>
        </div>
        <button className="icon-button modal-close-button" aria-label="关闭" title="关闭" onClick={onClose}>
          <X size={18} />
        </button>
      ...
      <div className={`progress-lite-body ${isWaitingMode ? 'waiting-mode' : ''} ${isAcceptanceMode ? 'acceptance-mode' : ''}`}>
        {isWaitingMode ? (
          ...
          <>
            {onConfirmAcceptance && !isEditingEntry && !isAcceptanceRevisionMode && (
              <div
              ...
              >
                ...
                <em>{isAcceptanceMode ? '提交后完成验收闭环' : '打开后记录验收收尾'}</em>
              </div>
            )}
            {isAcceptanceMode && (
            ...
            >
              ...
              {pendingAttachments.length > 0 && (
                <div className="progress-pending-attachments">
                  ...
                  <div className="progress-attachment-thumbs" aria-label="新增附件缩略图">
              ...
              {pendingAttachments.length > 0 && (
                <div className="progress-attachment-list">
                  {pendingAttachments.map((attachment) => (
                    <article className="progress-attachment-draft" key={attachment.id}>
                      <Paperclip size={15} />
                      <div className="progress-attachment-main">
      ...
      <footer className="modal-footer">
        <button className="ghost-button" onClick={onClose}>取消</button>
        {isAcceptanceMode && onConfirmAcceptance && !isEditingEntry ? (
          <button
...
function InsightsView({
  ...
  const contactProfileRows = useMemo(() => {
    type ContactProfile = {
      name: string
      projects: number
      hours: number
    ...
    const contactByTask = new Map<number, string>()
    ...
    periodTasks.forEach((task) => {
      const name = task.contact || task.requester || '未填写'
      contactByTask.set(task.id, name)
      const c = ensure(name)
      c.projects += 1
    ...
    const revisionRe = /反馈|修改|调整|重做|改稿|返工|第.{0,2}[轮版次稿]/
    periodUpdates.forEach((update) => {
      const name = contactByTask.get(update.taskId)
      if (!name) return
      const c = map.get(name)
      if (!c) return
      if (revisionRe.test(update.body || '') || revisionRe.test(update.title || '')) c.revisionMentions += 1
    })
    periodFiles.forEach((file) => {
      const name = contactByTask.get(file.taskId)
      if (!name) return
      const c = map.get(name)
      if (!c) return
    ...
    completedAnalyses.forEach((analysis) => {
      const name = contactByTask.get(analysis.taskId)
      if (!name) return
      const c = map.get(name)
      if (!c) return
  ...
  }, [periodTasks, periodUpdates, periodFiles, completedAnalyses])
  const selectedProject = selectedInsightKind === 'pd' ? projectDiagnosisRows[Number(selectedInsightValue)] : undefined
  const selectedContact = selectedInsightKind === 'cp' ? contactProfileRows[Number(selectedInsightValue)] : undefined
  useEffect(() => {
    let ignore = false
      ...
      <div className="insight-reference-layout">
        <aside className="insight-tree" aria-label="洞察目录">
          ...
          <div className="insight-tree-group">
            ...
            {contactProfileRows.map((item, index) => (
              <button
                type="button"
        ...
        <section className="insight-document">
            ...
            <>
              ...
              <div className="pdiag">
                <div className="pdiag-head">
                  ...
                  <span>{selectedProject.task.contact || selectedProject.task.requester || '未填写'}</span>
          ...
          {selectedInsightKind === 'cp' && selectedContact && (() => {
            ...
            // 与「你其他对接人」对比，用于判断耗时偏长/偏短、需求难易
            const hpp = c.hours / Math.max(c.projects, 1)
            const cohortHpp = contactProfileRows.length > 0
              ? contactProfileRows.reduce((sum, row) => sum + row.hours / Math.max(row.projects, 1), 0) / contactProfileRows.length
              : hpp
            // 该对接人的特征画像：把数据翻译成对「这个人」的判断
            ...
            if (topTags.some(([tag]) => tag === '定价偏低')) adviceLines.push('历史标记「定价偏低」：该对接人项目建议重新评估单价。')
            ...
            <>
              <div className="sec-head">
                <h2>对接人画像 · {c.name}</h2>
                <p>从历史任务总结「和这个人合作是什么样的」，指导下次报价、排期与协作</p>
              </div>
              <div className="cp">
                ...
                <div className="cp-sub">这位对接人的特征</div>
            ...
            {billableTasks.map((task, index) => (
              receiptTemplate === 'detail' ? (
                <tr key={task.id}>
                  ...
                  <td>{task.contact || task.requester || '—'}</td>
                  <td>{task.status}</td>
