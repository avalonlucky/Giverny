...
/**
 * 从 LLM Director 决策映射到 AgentIntent 分类。
 * 这是意图分类的唯一权威来源——取代旧的正则推断。
 */
export function directorDecisionToIntents(decision: AgentDirectorDecision): AgentIntent[] {
  const intents: AgentIntent[] = []
  const add = (intent: AgentIntent) => { if (!intents.includes(intent)) intents.push(intent) }
  if (decision.isWrite) add('write')
  for (const domain of decision.domains) {
    switch (domain) {
      case 'finance': add('finance'); break
      case 'tasks': add('task_data'); break
      case 'files': add('attachment'); break
      case 'calendar': add('task_data'); break
  ...
  if (decision.requiresProductKnowledge && !intents.includes('product_help')) add('product_help')
  if (intents.length === 0) add('general')
  return intents
}

/**
 * 旧版正则意图推断——仅作为 Director 不可用时的降级路径保留。
 * 正常流程应使用 directorDecisionToIntents()。
 */
...
/**
 * 声明式验证规则表——每条规则定义：触发条件 + 需要的工具 + 失败提示。
 * 新增验证只需在表中追加一行，无需修改验证引擎逻辑。
 */
type VerificationRule = {
  when: (q: string, intents: AgentIntent[]) => boolean
  tools: string[]
  tool: string
  issue: string
}
...
const verificationRules: VerificationRule[] = [
  { when: (_q, i) => i.includes('finance'), tools: ['query_month_finance', 'query_settlement_exports', 'reconcile_settlement_export', 'generate_settlement_receipt', 'manage_settlement_export_preview'], tool: 'query_month_finance', issue: '金额或工时结论没有经过财务计算工具。' },
  { when: (_q, i) => i.includes('product_help'), tools: ['search_product_help', 'search_workspace'], tool: 'search_product_help', issue: '产品说明没有经过官方产品知识工具核对。' },
  { when: (_q, i) => i.includes('person_profile'), tools: ['get_requester_profile'], tool: 'get_requester_profile', issue: '需求人画像没有读取当前工作区的历史任务证据。' },
  { when: (q) => /(?:任务|项目)\s*#\s*\d+/.test(q) && /(?:查|看|打开|读取|详情|进展|状态|等待|卡在|未解决)/.test(q), tools: ['get_task_detail'], tool: 'get_task_detail', issue: '显式任务 ID 的详情问题没有读取对应任务详情。' },
  { when: (q, i) => i.includes('task_data') && /(?:这个任务|那个任务|这个项目|那个项目|刚才那个|上述任务|当前任务).*(?:详情|进展|状态|等|卡|为什么|现在)/.test(q), tools: ['get_task_detail'], tool: 'get_task_detail', issue: '显式任务 ID 的详情问题没有读取对应任务详情。' },
  { when: (q) => /(?:日程|安排|空闲|空档|有空|时间槽|时间段|什么时候能安排|哪天能安排|本周计划|今天计划|明天计划)/.test(q) && !/(?:安排|计划).*(?:做|制作|设计|新建|创建|新增)/.test(q), tools: ['query_agenda'], tool: 'query_agenda', issue: '日程或可用时间结论没有读取 Agenda。' },
  { when: (q) => /(?:主动事项|风险待办|优先级|最该|优先处理|提醒处理效果|误报率|解决率)/.test(q), tools: ['query_proactive_work'], tool: 'query_proactive_work', issue: '主动工作结论没有读取优先级、证据和处理效果。' },
  { when: (q) => !/(?:做到哪一步|当前步骤|为什么.{0,8}(?:卡住|阻塞)|失败步骤|下一步是什么)/.test(q) && /(?:继续|接着|续接|往下推进|执行下一步).*(?:计划|项目|任务|执行)|(?:计划|项目|任务|执行).*(?:继续|接着|续接|往下推进|执行下一步)/.test(q), tools: ['query_plan_continuation'], tool: 'query_plan_continuation', issue: '计划续接没有从真实停顿点生成下一步建议。' },
  { when: (q) => /(?:执行计划|任务计划|项目计划|计划步骤|当前步骤|下一步|做到哪一步|为什么.{0,8}(?:卡住|阻塞)|依赖.{0,8}(?:什么|谁)|失败步骤)/.test(q), tools: ['query_project_execution', 'query_plan_continuation'], tool: 'query_project_execution', issue: '项目执行进度、依赖或下一步结论没有读取已保存的执行计划。' },
  { when: (q) => /(?:全站|整个网站|所有地方|到处|跨(?:任务|附件|对话|知识)|不记得.*在哪).*(?:搜|查|找)|(?:统一搜索|全域搜索)/.test(q), tools: ['search_workspace'], tool: 'search_workspace', issue: '跨域查找没有经过全域统一搜索。' },
  { when: (q) => /(?:全站|数据|任务|附件|工时|结算).*(?:一致性|数据审计|矛盾|对不上|丢失|快照损坏)|(?:一致性审计)/.test(q), tools: ['audit_workspace_consistency'], tool: 'audit_workspace_consistency', issue: '数据一致性结论没有经过全站确定性审计。' },
  { when: (q) => /(?:查询|查看|找|已有).*(?:正式交付物|项目状态报告|验收报告|审计报告)/.test(q), tools: ['query_formal_deliverables'], tool: 'query_formal_deliverables', issue: '正式交付物查询没有读取持久化产物清单。' },
  { when: (q) => /(?:查询|查看|哪些|有没有).*(?:高风险操作|风险案件|审批证据)/.test(q), tools: ['query_high_risk_actions'], tool: 'query_high_risk_actions', issue: '高风险操作结论没有读取审批与证据案件。' },
  { when: (q) => /(?:组织规则|公司规则|团队规则|合作伙伴.{0,12}(?:偏好|约定|记忆)|项目.{0,12}(?:约定|决策|记忆)|企业记忆|长期规则|之前记住|历史决策)/.test(q), tools: ['query_enterprise_memory'], tool: 'query_enterprise_memory', issue: '组织、合作伙伴或项目记忆结论没有读取企业分层记忆及来源。' },
  { when: (q) => /(?:主模型|备用模型|大模型|模型路由).*(?:不可用|失败|异常|故障|回退|回落|切换)|(?:为什么|为何).*(?:备用模型|模型).*(?:启动|切换|不可用)/.test(q), tools: ['diagnose_ai_routing'], tool: 'diagnose_ai_routing', issue: '模型主备链路结论没有经过配置、连接状态和近期回退记录诊断。' },
  { when: (_q, i) => i.includes('attachment'), tools: ['search_attachments', 'search_workspace', 'prepare_attachment_upload', 'inspect_attachment_evidence', 'query_attachment_analysis'], tool: 'search_attachments', issue: '附件结论没有经过真实附件查询。' },
  { when: (q, i) => i.includes('attachment') && /(?:内容|文字|OCR|错别字|质量|问题|需求.*(?:一致|匹配)|分析结论)/i.test(q), tools: ['inspect_attachment_evidence'], tool: 'inspect_attachment_evidence', issue: '附件内容或质量结论没有读取可引用的文件证据。' },
]

export function verifyAgentAnswer(turn: AgentTurn, precomputedIntents?: AgentIntent[]): AgentVerification {
  const issues: string[] = []
  const requiredTools: string[] = []
  const detectedIntents = precomputedIntents ?? inferAgentIntents(turn.question, turn.intent)
  ...
  for (const rule of verificationRules) {
    if (rule.when(turn.question, detectedIntents) && !hasDeterministicTool(...rule.tools)) {
...
export function completeAgentTurn(turn: AgentTurn, answer: string, precomputedIntents?: AgentIntent[]): AgentTurn {
  const next = { ...turn, answer: answer.trim(), phase: 'verify' as AgentTurnPhase }
  const verification = verifyAgentAnswer(next, precomputedIntents)
  return {
    ...next,
    verification,
