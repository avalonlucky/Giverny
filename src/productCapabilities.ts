...
const featureCapabilities: ProductCapability[] = [
  ...
  },
  {
    ...
    category: '产品版本',
    title: '最近更新',
    summary: '当前版本是 v0.34.2，最近重点重构了 Agent 的自然对话、上下文追问、结算事实复核、动态过程反馈和受控联网查询。',
    details: [
      'v0.34.2：业务工具读取并核验事实后，由用户选择的主模型结合上下文自然回答；新增回答相关性检查，避免问金额却倾倒任务明细、问主题却回复更新日志。',
      'v0.34.2：修复结算导出金额字段映射；金额质疑会重新核对导出记录、快照和逐行小计，不再把缺失展示字段当成零。',
      'v0.34.2：思考过程逐项显示“思考、动作、结果”，隐藏编排节点、工具英文名和实现协议；天气、新闻等实时问题进入受控联网查询，不检索产品手册。',
      'v0.34.1：运行中心区分接口成功与目标完成，并统计首轮完成、补查恢复和待补充后解决情况。',
      'v0.34.0：LangGraph 生产力闭环统一执行、观察、补查和终止状态，并限制单轮工具预算。',
      'v0.33.9：明确命令进入主模型快速路径，普通业务命令不再默认检索快捷键、产品手册或更新日志。',
      'v0.33.8：主模型负责理解与最小工具规划，程序负责能力授权、参数校验、权限确认和结果验真。',
