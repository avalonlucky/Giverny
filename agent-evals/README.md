...
这组回归用例覆盖月份查询、收入工时、任务详情、创建与修改预览、六类后台分析、云端会话、任务通知、任务消歧和安全边界。
...
隔离评测会创建临时本地 D1，导入 `fixture.sql` 的匿名任务，启动本地 Worker、Cloudflare Workflow 与 OpenAI-compatible 模拟模型，执行全部 70 条工具链用例、MCP 鉴权/工具协议回归、确认写入 Workflow、operationId 幂等重放、云端会话迁移与恢复、任务未读状态，以及后台分析的完成、取消和重试后删除临时目录。它不会读取或修改正式 D1，也不会调用外部模型。
