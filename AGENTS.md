...
- 网站更新采用“两阶段发布”：先完成本地验证并部署正式站供用户验收；用户明确确认满意后，再完成代码提交、推送、版本 tag 和 GitHub Release。
- 线上验收阶段允许继续迭代并重复部署，不要求每次调整都生成 GitHub commit / tag / Release；但版本号与 `CHANGELOG.md` 仍需跟随正式站更新，避免线上版本无法识别。
...
- 用户确认满意后，不得遗忘 GitHub 发布闭环；最终版本、代码、tag 和 Release 必须对齐。
...
- [ ] 如果用户已确认满意，是否完成 GitHub commit + tag + Release？
