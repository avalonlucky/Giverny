...
<p align="center">
  <strong>把任务流转、协同记录、工时沉淀、交付验收和月度结算放进同一条可追溯链路。</strong>
</p>
...
> **一句话：** Giverny 是一个面向公司、兼职团队和设计服务协作场景的任务流转与结算工作台，把“需求在聊天里、工时在 Excel、文件在网盘、验收靠口头、月底手工对账”的混乱流程，收敛成一条可追溯的业务链路。
...
Giverny 不是又一个泛用项目管理工具。它更像一个“协同任务 + 工时证据 + 交付验收 + 月度结算”的轻量运营后台，适合需要把设计、内容、运营、外包或内部协作任务从需求到结算完整留痕的团队。

它关心的不是“任务有没有被拖到某个看板列”，而是每一项工作最终能不能回答清楚：

- 这件事是谁提的、谁对接、什么时候交付？
- 过程里做了哪些推进，上传过哪些文件？
- 实际投入了多少工时，哪些工时可以结算？
- 最终由谁验收，验收依据是什么？
- 月底给客户或公司对账时，能不能一键拿出任务、工时、文件和金额？

## 核心能力

| 能力 | 解决的问题 |
| --- | --- |
| 任务流转 | 从计划中、进行中、待验收到已验收，完整记录任务状态 |
| 过程协同 | 在任务右侧记录进展、附件、时间段和动态时间轴 |
| 工时管理 | 所有统计、收入和结算只基于实际工时，不用预计时间凑数 |
| 交付验收 | 通过终审面板核对基础信息、进度、分段工时、附件和备注 |
| 月度结算 | 按结算月份汇总工时、收入、验收情况和年度趋势 |
| 客户对账 | 生成只读分享链接，让甲方只看月报、任务明细和交付文件 |
| 文件归档 | 把过程文件、验收附件和最终稿沉淀到任务生命周期里 |
| 快捷操作 | 用命令面板和键盘快捷键快速新建任务、记录进展、进入文件库或设置 |

## 适合这些场景

- 公司内部设计、内容、运营、市场任务需要跨人协同和月度复盘。
- 兼职设计师或外包团队需要记录实际工时、交付文件和结算依据。
- 负责人需要按月核对“做了什么、花了多久、交付了什么、该结算多少”。
- 甲方或协作方需要一个只读页面查看月报和交付内容，而不是翻聊天记录。
