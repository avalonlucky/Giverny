...
- `agent-evals/`：125 条固定回归用例覆盖查询、附件搜索与证据读取、主动事项、完整任务生命周期写入、持续计划、任务记忆、同名消歧、六类后台分析和安全边界。
...
- Agent 工具能力注册表：`src/agentToolRegistry.ts` 统一登记 59 项读取、计划、记忆、结算、排期、附件、多模态分析、主动事项、提醒、安全设置、后台分析、写入预览、签名执行与内部 Workflow 能力。输入 schema、角色、scope、风险、确认关系、审计事件、任务作用域、执行轨迹、MCP 暴露面、Workflow 白名单和 OpenAPI 元数据均从这里派生；自动生成清单见 [`AGENT_CAPABILITY_REGISTRY.md`](./AGENT_CAPABILITY_REGISTRY.md)。
...
- 远程 MCP：`/mcp` 使用 Streamable HTTP 暴露十三个只读工具，与爱丽丝共用 `src/agentToolRegistry.ts`；除附件证据外，`query_proactive_work` 返回内部主动事项、优先级、证据和处理效果，合作伙伴角色不可读取。MCP 仅接受独立的 `MCP 只读`口令，该口令不能登录网站或访问写入工具。
- 持久写入：十七类确认操作均由 `AgentWriteWorkflow` 等待人工批准后执行，覆盖任务生命周期、结算导出/分享管理、排期调整、站内提醒、模型路由配置、附件分析恢复、附件元数据修改和主动事项处理；步骤支持重试，`agent_write_operations` 缓存完成结果，重复恢复不会重复写入。
