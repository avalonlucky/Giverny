...
工作助手中的手动模型选择是当前站点的运行时最高优先级，不只影响这一轮聊天。助手菜单读取模型中心里已启用、已保存 Key 且成功加载模型的服务商，并以各服务商指定的默认模型作为候选；选择通义千问、豆包、Kimi、Gemini、OpenAI、OpenRouter 或其他已配置服务商后，工作助手规划与回答、新建任务需求优化、进展 / 验收备注、AI 工时、整体进度、洞察诊断和附件命名等文字能力都会先调用该服务商默认模型。正常情况下必须使用用户所选主模型；只有主模型经过同模型重试仍出现鉴权、额度、超时、供应商故障或无效结构等极端失败，才允许切换备用模型，并记录实际模型与切换原因。

若所选服务商默认模型支持多模态输入，站内识图能力也会先使用同一模型，再回退识图主 / 备用模型。不支持识图的文字模型不会覆盖视觉路线。选择「自动」会清除运行时覆盖，恢复本机 CLI、站内 Agent 与模型设置的自动分流。运行时选择独立保存在设置中，不会改写文字主 / 备用、识图主 / 备用的长期配置；站内写入始终由 Worker 做权限校验和确认。
