...
工作助手中的手动模型选择属于单轮最高优先级覆盖：规划工具与组织最终答案均先使用所选模型；调用失败后才进入文字主模型、文字备用模型和 Workers AI 的全站回退链路。「自动」模式才按照本机 CLI、站内 Agent 与模型设置自动分流。若所选端点属于通义千问、豆包、Kimi、Gemini、OpenAI、OpenRouter 或其他兼容多模态输入的模型，图片理解也先尝试该端点；失败后才进入识图主 / 备用链路。站内写入始终由 Worker 做权限校验和确认。
