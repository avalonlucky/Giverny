...
工作助手中的手动模型选择是当前站点的运行时最高优先级，不只影响这一轮聊天。助手菜单读取模型中心里已启用、已保存 Key 且成功加载模型的服务商，并以各服务商指定的默认模型作为候选；选择通义千问、豆包、Kimi、Gemini、OpenAI、OpenRouter 或其他已配置服务商后，工作助手规划与回答、新建任务需求优化、进展 / 验收备注、AI 工时、整体进度、洞察诊断和附件命名等文字能力都会先调用该服务商默认模型；失败后才进入设置中的文字主模型、文字备用模型和 Workers AI 回退链路。
