...
当前 Bridge Runtime 版本为 `0.3.0`。工作助手收到请求后，Worker 会用登录身份和 `browserDeviceKey` 精确匹配当前电脑，再将命令放入该设备的短期队列。Bridge 领取命令后启动已选 CLI，逐步回传可展示的执行事件和最终回答。

Codex CLI 会把首次执行返回的会话 ID 绑定到当前工作助手对话。后续追问使用 `codex exec resume` 续接同一会话，只发送本轮新消息，不再重复冷启动完整上下文。首次问题仍需要建立 CLI 会话，连续追问会更快并保留上下文。
...
- 图片理解；
- Bridge 离线或低于 `0.3.0`、CLI 不可用、本机超时或执行失败时的自动回退。
