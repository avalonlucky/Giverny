...
## 2026-07-16 · v0.28.41 AI 工时可靠性与反馈校准

- 历史工时样本新增时间衰减，近期任务优先，缺失实际交付时间时回退到预计交付或开始时间。
- 任务保存会把 AI 建议和最终采用值写入现有学习事件；同类至少 3 次后启用受控个人采用校准。
- 工时面板新增常规区间、历史命中率、不确定因素和「偏低 / 合适 / 偏高」原因反馈。
- 同一次建议的重复反馈按 suggestion id 去重；完整结果继续写入既有 `basis_json`，无新增 migration。
- 隔离 Agent 评测增加工时专项检查，原有 74/74 门禁继续通过。
