...
## 2026-07-16 · v0.28.37 附件预览自适应

- `FilePreviewModal` 的图片预览改为可收缩 Grid 轨道，并以弹窗可用宽高为双重上限。
- 横图、竖图、长图均保持原比例完整显示，调整弹窗尺寸时实时适配；工作助手、文件库、进展和验收共用该行为。
- Agent 继续通过结构化 `attachments` 返回真实文件，正文继续使用 GFM 富文本渲染。
- 隔离发布门禁保持 74/74；本版本无新增 D1 migration。
