...
test('工作助手的明确创建指令直接进入业务流程且不虚构字段', async ({ page }) => {
  ...
  await expect(timeline.locator('> summary')).toContainText('模型推理')
  await timeline.locator('> summary').click()
  // 回答完成后推理全文收进嵌套折叠：上千字留在展开状态会把执行过程挤走。
  await timeline.locator('.thinking-reasoning-fold > summary').click()
  await expect(timeline.locator('.thinking-reasoning')).toContainText('用户问的是这个项目最新那一版')
...
test('工作助手分开展示真实模型推理与执行过程且不暴露框架名', async ({ page }) => {
  await page.getByRole('button', { name: '打开工作助手' }).click()
  const dialog = page.getByRole('dialog', { name: '爱丽丝' })
  const input = dialog.getByPlaceholder('向爱丽丝提问…')
  await input.fill('你帮我新建一个任务')
  ...
  await expect(timeline.locator('> summary')).toContainText('模型推理')
  await timeline.locator('> summary').click()
  await expect(timeline.locator('.thinking-reasoning-fold')).not.toHaveAttribute('open', '')
  await timeline.locator('.thinking-reasoning-fold > summary').click()
  await expect(timeline.locator('.thinking-reasoning')).toContainText('用户问的是这个项目最新那一版，我先看附件里的版本号')
...
test('模型推理与执行步骤不会互相混入或重复显示', async ({ page }) => {
  ...
  await timeline.locator('> summary').click()
  await timeline.locator('.thinking-reasoning-fold > summary').click()
  await expect(timeline.locator('.thinking-reasoning')).not.toContainText('正在读取任务详情')
  await expect(timeline.locator('.execution-progress .thinking-line', { hasText: '正在读取任务详情' })).toHaveCount(1)
})
