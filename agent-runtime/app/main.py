...
SYSTEM_PROMPT = (
    ...
    "- 创建任务、记录反馈、修改状态、修改字段、追加进展都必须遵循：先调用 *_preview 工具生成草稿和 confirmationToken，把预览展示给用户；只有用户明确确认后，才调用对应执行工具。\n"
    "- 不要自己编造 confirmationToken；执行工具只能使用 preview 工具返回的 token。\n"
